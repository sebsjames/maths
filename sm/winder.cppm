...
export namespace sm
{
     ...
     *
     * Implements the axis-crossing algorithm to compute the winding number. see
     * https://www.engr.colostate.edu/~dga/documents/papers/point_in_polygon.pdf for some extra info
     *
     * This class is specialised so that the container which contains the path coordinates can be
     * any of the straightforward STL containers such as std::vector or std::list (but not
     * std::map). The coordinate should be some type which has one of the following: .x and .y
     * attributes, .x() and .y() methods or the ability to access members in an array-like fashion
     * (std::vector or sm::vec). For example:
     *
     *\code{c++}
    ...
    public:
        ...
        int wind (const T& px)
        {
            ...
            auto biter = this->boundary.begin();
            while (biter != this->boundary.end()) {

                sm::vec<T_el, 2> pt = this->shiftpt (px, *biter);
                if (pt[1] * pt_last[1] < T_el{0}) {
                    ...
                    if (r > T_el{0}) { // crosses positive x axis
                        if (pt_last[1] < T_el{0}) {
                            if constexpr (debug_mode == true) { std::cout << "+x,"; }
                            wd += 1;
                        } else {
                            if constexpr (debug_mode == true) { std::cout << "-x,"; }
                            wd -= 1;
                        ...
                        if (pt[1] > T_el{0}) {
                            if constexpr (debug_mode == true) { std::cout << "PL+.5,"; }
                            wd += 0.5;
                        } else {
                            if constexpr (debug_mode == true) { std::cout << "PL-.5,"; }
                            wd -= 0.5;
                        ...
                        if (pt_last[1] < T_el{0}) {
                            if constexpr (debug_mode == true) { std::cout << "PT+.5,"; }
                            wd += 0.5;
                        } else {
                            if constexpr (debug_mode == true) { std::cout << "PT-.5,"; }
                            wd -= 0.5;
    ...
    private:
        ...
        // Returns (bp - px) (for just the first two elements) in a vec<T_el, 2>
        sm::vec<T_el, 2> shiftpt (const T& px, const T& bp)
        {
            if constexpr (debug_mode == true) { std::cout << "Shifting boundary point " << bp << std::endl; }
            ...
            if constexpr (array_access_possible<T>::value == true) {
                ...
                if constexpr (debug_extra == true) { std::cout << "T array access possible\n"; }
                pt[0] = bp[0] - px[0];
                pt[1] = bp[1] - px[1];
            } else {
                ...
                if constexpr (has_subtraction<T>::value == true) {
                    ...
                    if constexpr (debug_extra == true) { std::cout << "T has xy members\n"; }
                    pt[0] = bp.x - px.x;
                    pt[1] = bp.y - px.y;
                } else {
                    []<bool flag = false>() { static_assert(flag, "Can't subtract one coordinate from another."); }();
