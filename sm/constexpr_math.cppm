 ...
 *
 * Constexpr math in the sm namespace as sm::cem
 *
...
export module sm.constexpr_math;

import sm.mathconst;

// C++23: 202302L C++20: 202002L
export namespace sm::cem // ConstExpr Math
{
    template<typename T>
    ...
    {
        constexpr bool is_odd (const std::int64_t x) noexcept { return (x & 1U) != 0; }
        constexpr bool is_even (const std::int64_t x) noexcept { return !is_odd (x); }
    }
    ...
    namespace internal // any_nan
    {
        ...
        constexpr bool any_nan (const T1 x, const T2 y) noexcept {  return (cem::isnan(x) || cem::isnan(y)); }
    }
    ...
    template<typename T>
    constexpr bool isfinite (const T x) noexcept { return (!cem::isnan(x)) && (!cem::isinf(x)); }

    // signum
    template<typename T> constexpr std::int32_t sgn (const T x) noexcept { return (x > T{0} ? 1 : (x < T{0} ? -1 : 0)); }
    ...
    {
        template<typename T>
        constexpr std::int64_t find_exponent (const T x, const std::int64_t exponent) noexcept
        {
            return (x < T{1e-03}  ? find_exponent(x * T{1e+04}, exponent - std::int64_t{4}) :
                    x < T{1e-01}  ? find_exponent(x * T{1e+02}, exponent - std::int64_t{2}) :
                    x < T{1} ? find_exponent(x * T{10}, exponent - std::int64_t{1}) :
                    x > T{10} ? find_exponent(x / T{10}, exponent + std::int64_t{1}) :
                    x > T{1e+02} ? find_exponent(x / T{1e+02}, exponent + std::int64_t{2}) :
                    x > T{1e+04} ? find_exponent(x / T{1e+04}, exponent + std::int64_t{4}) : exponent);
        }
    }
    ...
    {
        template<typename T>
        constexpr T log_cf_main (const T xx, const std::int32_t depth_end) noexcept
        {
            constexpr std::int32_t log_max_iter_small = 25;
            std::int32_t depth = log_max_iter_small - 1;
            T res = T(2 * (depth + 1) - 1);
        ...
        constexpr long double log_mantissa_integer (const std::int32_t x) noexcept
        {
        ...
        {
            return (log_main(x / T(static_cast<std::int32_t>(x))) + T(log_mantissa_integer(static_cast<std::int32_t>(x))));
        }

        template<typename T>
        constexpr T log_breakup (const T x) noexcept
        {
            return (log_mantissa (mantissa(x)) + sm::mathconst<T>::ln_10 * T(find_exponent (x, 0)));
        }

        template<typename T>
        constexpr T log_check (const T x) noexcept
        {
            return (cem::isnan(x) ? std::numeric_limits<T>::quiet_NaN() :
                    x < T(0) ? std::numeric_limits<T>::quiet_NaN() :
                    std::numeric_limits<T>::min() > x ? -std::numeric_limits<T>::infinity() :
                    std::numeric_limits<T>::min() > cem::abs(x - T{1}) ? T{0} :
    ...
    {
        template<typename T>
        constexpr return_t<T> log10_check (const T x) noexcept { return return_t<T>(log(x) / sm::mathconst<T>::ln_10); }
    }
    template<typename T>
    ...
    namespace internal // pow_integral
    {
        ...
        {
            return pow_integral_compute (base, static_cast<std::int64_t>(exp_term));
        }
    ...
    {
        template<typename T>
        constexpr std::int32_t ceil_resid (const T x, const T x_whole) noexcept { return ((x > T{0}) && (x > x_whole)); }
        template<typename T>
        ...
        {
            return (cem::abs(x) >= 8388608.f ? x : ceil_int(x, float(static_cast<std::int32_t>(x))));
        }
        template<>
        constexpr double ceil_check_internal<double> (const double x) noexcept
        { return (cem::abs(x) >= 4503599627370496. ? x : ceil_int(x, double(static_cast<std::int64_t>(x)))); }
        template<>
        constexpr long double ceil_check_internal<long double> (const long double x) noexcept
        { return (cem::abs(x) >= 9223372036854775808.l ?  x : ceil_int(x, ((long double)static_cast<std::uint64_t>(cem::abs(x))) * sgn(x))); }
        template<typename T>
        constexpr T ceil_check (const T x) noexcept
        {
            return (cem::isnan(x) ? std::numeric_limits<T>::quiet_NaN() :
                    !cem::isfinite(x) ? x :
                    std::numeric_limits<T>::min() > cem::abs(x) ? x : ceil_check_internal(x));
        }
    }
    ...
    {
        template<typename T>
        constexpr std::int32_t floor_resid (const T x, const T x_whole) noexcept { return ((x < T{0}) && (x < x_whole)); }
        template<typename T>
        ...
        {
            return (cem::abs(x) >= 8388608.f ? x : floor_int (x, float(static_cast<std::int32_t>(x))));
        }
        template<>
        constexpr double floor_check_internal<double> (const double x) noexcept
        {
            return (cem::abs(x) >= 4503599627370496. ? x : floor_int (x, double(static_cast<std::int64_t>(x))));
        }
        template<>
        constexpr long double floor_check_internal<long double> (const long double x) noexcept
        {
            return (cem::abs(x) >= 9223372036854775808.l ? x : floor_int (x, ((long double)static_cast<std::uint64_t>(cem::abs(x))) * sgn(x)));
        }
        template<typename T>
        constexpr T floor_check (const T x) noexcept
        {
            return (cem::isnan(x) ? std::numeric_limits<T>::quiet_NaN() : !cem::isfinite(x) ? x :
                    std::numeric_limits<T>::min() > cem::abs(x) ? x : floor_check_internal(x));
        }
    }
    template<typename T>
    ...
    {
        template<typename T>
        constexpr T trunc_int (const T x) noexcept { return (T(static_cast<std::int64_t>(x))); }
        template<typename T>
        constexpr T trunc_check_internal (const T x) noexcept { return x; }
        template<>
        constexpr float trunc_check_internal<float> (const float x) noexcept { return (cem::abs(x) >= 8388608.f ? x : trunc_int(x)); }
        template<>
        constexpr double trunc_check_internal<double>(const double x) noexcept { return (cem::abs(x) >= 4503599627370496. ? x : trunc_int(x)); }
        template<>
        constexpr long double trunc_check_internal<long double>(const long double x) noexcept
        {
            return( cem::abs(x) >= 9223372036854775808.l ? x : ((long double)static_cast<std::uint64_t>(cem::abs(x))) * sgn(x) );
        }
        ...
        constexpr T trunc_check (const T x) noexcept {
            return (cem::isnan(x) ? std::numeric_limits<T>::quiet_NaN() :
                    !cem::isfinite(x) ? x :
                    std::numeric_limits<T>::min() > cem::abs(x) ? x : trunc_check_internal(x) );
        }
    }
    template<typename T>
    ...
    namespace internal // round
    {
        ...
        {
            return (cem::abs(x - internal::floor_check(x)) >= T{0.5} ? internal::floor_check(x) + sgn(x) : internal::floor_check(x) );
        }
        template<typename T>
        constexpr T round_check_internal (const T x) noexcept { return x; }
        template<>
        constexpr float round_check_internal<float> (const float x) noexcept { return (cem::abs(x) >= 8388608.f ? x : round_int(x)); }
        template<>
        constexpr double round_check_internal<double> (const double x) noexcept { return (cem::abs(x) >= 4503599627370496. ? x : round_int(x)); }
        template<>
        constexpr long double round_check_internal<long double>(const long double x) noexcept { return (cem::abs(x) >= 9223372036854775808.l ? x : round_int(x)); }
        template<typename T>
        constexpr T round_check (const T x) noexcept
        {
            return (cem::isnan(x) ? std::numeric_limits<T>::quiet_NaN() : !cem::isfinite(x) ? x :
                    std::numeric_limits<T>::min() > cem::abs(x) ? x : sgn(x) * round_check_internal(cem::abs(x)));
        }
    }
    template<typename T>
    ...
    {
        template<typename T>
        constexpr std::int64_t find_whole (const T x) noexcept
        {
            return (cem::abs(x - cem::floor(x)) >= T{0.5} ? static_cast<std::int64_t>(cem::floor(x) + sgn(x)) :
                    static_cast<std::int64_t>(cem::floor(x)));
        }

        template<typename T>
        constexpr T find_fraction (const T x) noexcept
        {
            return (cem::abs(x - cem::floor(x)) >= T{0.5} ? x - cem::floor(x) - sgn(x) : x - cem::floor(x));
        }
    }
    ...
    {
        template<typename T>
        constexpr T exp_cf_recur (const T x, const std::int32_t depth_end) noexcept
        {
            constexpr std::int32_t exp_max_iter_small = 25;
            std::int32_t depth = exp_max_iter_small - 1;
            T res = T{1};
            while (depth > depth_end - 1) {
        ...
        {
            return (static_cast<T>(pow_integral (sm::mathconst<T>::e, find_whole(x))) * exp_cf (find_fraction(x)));
        }

        template<typename T>
        constexpr T exp_check(const T x) noexcept
        {
            return (cem::isnan(x) ? std::numeric_limits<T>::quiet_NaN() : internal::is_neginf(x) ? T{0} :
                    std::numeric_limits<T>::min() > cem::abs(x) ? T{1} : internal::is_posinf(x) ? std::numeric_limits<T>::infinity() :
                    cem::abs(x) < T{2} ? exp_cf(x) : exp_split(x));
        }
    ...
    {
        template<typename T>
        constexpr T atan_series_order_calc (const T xx, const T x_pow, const std::uint32_t order) noexcept
        { return (T{1}/( T((order-1)*4 - 1) * x_pow ) - T{1}/( T((order-1)*4 + 1) * x_pow * xx)); }
        template<typename T>
        constexpr T atan_series_order (const T x, const T x_pow, const std::uint32_t order_begin, const std::uint32_t max_order) noexcept
        {
            if (max_order == 1) { return sm::mathconst<T>::pi_over_2 - T{1} / x_pow; }

            T xx = x * x;
            T res = atan_series_order_calc (xx, pow (x, 4 * max_order - 5), max_order);
            std::uint32_t depth = max_order - 1;
            while (depth > order_begin) {
                res += atan_series_order_calc (xx, pow (x, 4 * depth - 5), depth);
                --depth;
            }
            res += sm::mathconst<T>::pi_over_2 - T{1} / x;
            return res;
        }
        template<typename T>
        ...
        template<typename T>
        constexpr T atan_cf_recur (const T xx, const std::uint32_t depth_begin, const std::uint32_t max_depth) noexcept
        {
            std::uint32_t depth = max_depth - 1;
            T res = T(2 * (depth + 1) - 1);
        ...
        {
            return (cem::isnan(x) ? std::numeric_limits<T>::quiet_NaN() :
                    std::numeric_limits<T>::min() > cem::abs(x) ? T{0} : x < T(0) ? -atan_begin(-x) : atan_begin(x));
        }
    }
    template<typename T>
    ...
    namespace internal // atan2
    {
        ...
        constexpr T atan2_compute (const T y, const T x) noexcept
        {
            return (any_nan(y, x) ? std::numeric_limits<T>::quiet_NaN() :
                    std::numeric_limits<T>::min() > cem::abs(x) ? std::numeric_limits<T>::min() > cem::abs(y) ? neg_zero(y) ? neg_zero(x) ? -sm::mathconst<T>::pi :
                    - T{0} : neg_zero(x) ? sm::mathconst<T>::pi : T{0} :
                    y > T{0} ? sm::mathconst<T>::pi_over_2 : -sm::mathconst<T>::pi_over_2 :
                    x < T{0} ? y < T{0} ? atan(y/x) - sm::mathconst<T>::pi :
                    atan(y/x) + sm::mathconst<T>::pi :
                    atan(y/x));
        }
        template<typename T1, typename T2, typename TC = common_return_t<T1, T2>>
        constexpr TC atan2_type_check (const T1 y, const T2 x) noexcept { return atan2_compute (static_cast<TC>(y), static_cast<TC>(x)); }
    }
    template<typename T1, typename T2>
    constexpr common_return_t<T1, T2> atan2 (const T1 y, const T2 x) noexcept
    {
        if (std::is_constant_evaluated()) {
            return internal::atan2_type_check (y, x);
        } else {
    ...
    {
        template<typename T>
        constexpr T sqrt_recur (const T x, const T xn, const std::int32_t count) noexcept
        {
            constexpr std::int32_t sqrt_max_iter = 100;
            return (cem::abs(xn - x/xn) / (T{1} + xn) < std::numeric_limits<T>::min() ? xn :
        ...
        constexpr T sqrt_check (const T x) noexcept
        {
            return (cem::isnan (x) ? std::numeric_limits<T>::quiet_NaN() :
                    ...
                    std::numeric_limits<T>::min() > cem::abs(x) ? T{0} :
                    std::numeric_limits<T>::min() > cem::abs(T{1} - x) ? x : sqrt_simplify (x, T{1}));
        }
    }
    template<typename T>
    ...
    namespace internal
    {
        ...
        {
            return (std::numeric_limits<T>::min() > cem::abs (x - sm::mathconst<T>::pi_over_2) ? T{1.633124e+16} :
                    tan_series_exp_long (x - sm::mathconst<T>::pi_over_2));
        }
        template<typename T>
        constexpr T tan_cf_recur (const T xx, const std::int32_t depth, const std::int32_t max_depth) noexcept
        {
        ...
        template<typename T>
        constexpr T tan_begin (const T x, const std::int32_t count = 0) noexcept
        {
            return (x > sm::mathconst<T>::pi ? count > 1 ? std::numeric_limits<T>::quiet_NaN() :
                    tan_begin (x - sm::mathconst<T>::pi * cem::floor (x / sm::mathconst<T>::pi), count+1 ) :
                    tan_cf_main(x));
        }
        template<typename T>
        ...
        {
            return (cem::isnan(x) ? std::numeric_limits<T>::quiet_NaN() :
                    std::numeric_limits<T>::min() > cem::abs(x) ? T{0} : x < T{0} ? -tan_begin (-x) : tan_begin (x));
        }
    }
    template<typename T>
    ...
    namespace internal // cos
    {
        ...
        {
            return (cem::isnan(x) ? std::numeric_limits<T>::quiet_NaN() :
                    std::numeric_limits<T>::min() > cem::abs(x) ? T{1} :
                    std::numeric_limits<T>::min() > cem::abs(x - sm::mathconst<T>::pi_over_2) ? T{0} : // special cases: pi/2 and pi
                    std::numeric_limits<T>::min() > cem::abs(x + sm::mathconst<T>::pi_over_2) ? T{0} :
                    std::numeric_limits<T>::min() > cem::abs(x - sm::mathconst<T>::pi) ? T{-1} :
                    std::numeric_limits<T>::min() > cem::abs(x + sm::mathconst<T>::pi) ? T{-1} : cos_compute (cem::tan (x/T{2})));
        }
    }
    template<typename T>
    ...
    namespace internal // sin
    {
        ...
        constexpr T sin_check (const T x) noexcept
        {
            return (cem::isnan(x) ? std::numeric_limits<T>::quiet_NaN() :
                    std::numeric_limits<T>::min() > cem::abs(x) ? T{0} :
                    std::numeric_limits<T>::min() > cem::abs(x - sm::mathconst<T>::pi_over_2) ? T{1} :
                    std::numeric_limits<T>::min() > cem::abs(x + sm::mathconst<T>::pi_over_2) ? T{-1} :
                    std::numeric_limits<T>::min() > cem::abs(x - sm::mathconst<T>::pi) ? T{0} :
                    std::numeric_limits<T>::min() > cem::abs(x + sm::mathconst<T>::pi) ? T{-0} : sin_compute (cem::tan (x / T{2})));
        }
    }
    template<typename T>
    ...
    namespace internal // acos
    {
        ...
        {
            return (cem::abs(x) > T{1} ? std::numeric_limits<T>::quiet_NaN() :
                    std::numeric_limits<T>::min() > cem::abs(x -  T{1}) ? T{0} :
                    std::numeric_limits<T>::min() > cem::abs(x) ? sm::mathconst<T>::pi_over_2 : atan (cem::sqrt(T{1} - x * x) / x));
        }
        template<typename T>
        constexpr T acos_check (const T x) noexcept
        {
            return (cem::isnan(x) ? std::numeric_limits<T>::quiet_NaN() :
                    x > T{0} ? acos_compute(x) : sm::mathconst<T>::pi - acos_compute(-x));
        }
    }
    template<typename T>
    ...
    namespace internal // asin
    {
        ...
        constexpr T asin_compute (const T x) noexcept
        {
            return (x > T{1} ? std::numeric_limits<T>::quiet_NaN() :
                    std::numeric_limits<T>::min() > cem::abs(x -  T{1}) ? sm::mathconst<T>::pi_over_2 :
                    std::numeric_limits<T>::min() > cem::abs(x) ? T{0} : atan (x/sqrt(T{1} - x * x)));
        }

        template<typename T>
        constexpr T asin_check(const T x) noexcept
        {
            return (cem::isnan(x) ? std::numeric_limits<T>::quiet_NaN() : x < T{0} ? -asin_compute(-x) : asin_compute(x));
        }
    }
    template<typename T>
