...
export namespace sm::algo
{
    ...
    T centroid (const Container<T, Allocator>& coords)
    {
    ...
    template<typename T>
    sm::vec<T, 2> centroid2D (const std::vector<sm::vec<T, 2>> points)
    {
        sm::vec<T, 2> centroid;
        centroid[0] = T{0};
        centroid[1] = T{0};
    ...
    //! x1,y1,x2,y2,etc
    template<typename T>
    sm::vec<T, 2> centroid2D (const std::vector<T> points)
    {
        sm::vec<T, 2> centroid;
        centroid[0] = T{0};
        centroid[1] = T{0};
    ...
    template<typename T>
    std::array<T,3> centroid3D (const std::vector<T> points)
    {
    ...
    template<typename T>
    std::array<T,3> centroid3D (const std::array<T, 12> points)
    {
        std::array<T,3> centroid;
        centroid[0] = T{0};
        centroid[1] = T{0};
