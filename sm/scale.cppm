...
export namespace sm
{
    ...
    /*!
     * \brief Base class for sm::scale_impl
     *
     ...
     * double, etc or a type which can contain a vector such as std::array, sm::vec or
     * std::vector.
     *
    ...
    struct scale_impl_base
    {
        ...
        requires sm::is_copyable_container<Container>::value && sm::is_copyable_container<OContainer>::value
         ...
         * \param output The scaled output
         */
        void transform (const std::span<T> data, std::span<S> output)
        {
            std::size_t dsize = data.size();
            if (output.size() != dsize) {
                throw std::runtime_error ("scale_impl_base::transform(): Ensure data.size()==output.size()");
            ...
            typename std::span<S>::iterator oi = output.begin();
            while (di != data.end()) { *oi++ = this->transform_one (*di++); }
        }
        ...
        /*!
         * \brief Inverse transform a span of scalars or vectors.
         */
        void inverse (const std::span<S> data, std::span<T> output)
        {
            std::size_t dsize = data.size();
            if (output.size() != dsize) {
                throw std::runtime_error ("scale_impl_base::inverse(): Ensure data.size()==output.size()");
            ...
            typename std::span<S>::iterator di = data.begin();
            typename std::span<T>::iterator oi = output.begin();
            while (di != data.end()) { *oi++ = this->inverse_one (*di++); }
        }
         ...
         * will be something like \c std::vector<float> or \c std::list<sm::vec<double,2>>
         */
        template <typename Container> requires sm::is_copyable_container<Container>::value
        ...
        void set_type (scaling_function t)
        {
        ...
        scaling_function get_type() { return this->type; }
     ...
     * vector such as std::array or sm::vec.
     *
     ...
     *
     * \tparam ntype The 'number type' as contained in sm::number_type::value. 1 for vectors, 0
     ...
     * sm::vec or std::vector. It is from the type \a T that ntype is
     ...
     * \sa scale_impl_base
     */
    template <std::int32_t ntype = 0, typename T=float, typename S=float>
    struct scale_impl : public scale_impl_base<T, S>
    {
        ...
        S_el get_params (std::uint32_t idx) { return this->params[idx]; }

        //! Get all the params
        sm::vvec<S_el> get_params() { return this->params; }
    ...
    struct scale_impl<1, T, S> : public scale_impl_base<T, S>
    {
        ...
        //! \param idx The index into #params
        //! \return The specified element of #params
        S get_params (std::uint32_t idx) { return this->params[idx]; }
    ...
    protected:
        ...
        void compute_scaling_log (T input_min, T input_max)
        {
            ...
            if constexpr (sm::number_type<T>::value == 1) {
     ...
     */
    template<typename T, typename S>
    struct scale_impl<2, T, S> : public scale_impl<1, T, S>
    {
    ...
    class scale_impl<3, T, S> : public scale_impl_base<T, S> {
        void compute_scaling_linear ([[maybe_unused]] T input_min, [[maybe_unused]] T input_max) {
            []<bool flag = false>() { static_assert(flag, "sm::scale does not support scaling vectors of complex numbers"); }();
        }
    };
    ...
    class scale_impl<-1, T, S> : public scale_impl_base<T, S> {
        void compute_scaling_linear ([[maybe_unused]] T input_min, [[maybe_unused]] T input_max) {
            []<bool flag = false>() { static_assert(flag, "sm::scale does not support scaling non-number types"); }();
        }
    };
    ...
    struct scale : public scale_impl<sm::number_type<T>::value, T, S> {};
