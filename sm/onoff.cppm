...
export namespace sm::algo
{
    ...
    template<typename T, std::int32_t w, bool horz_wrap=true>
    void oncentre_offsurround (const sm::vvec<T>& data, sm::vvec<T>& result)
    {
        ...
        std::int32_t lastrow_index = static_cast<std::int32_t>(data.size()) - w;
        ...
        // Top row
        for (; i < static_cast<std::int32_t>(data.size())-1; ++i) { // last row
            result[i] -= (data[i-1] + data[i+1] + data[i-w+1] + data[i-w] + data[i-w-1]) / T{5};
        }
        // TR pixel
        if constexpr (horz_wrap) {
