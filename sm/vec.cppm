...
namespace sm
{
     ...
     * such as std::int32_t, long long int and so on. Thus, a typical (and in fact, the default)
     * signature would be:
     *
     * vec<float, 3> v;
     *
     ...
     *
     * Because sm::vec extends std::array, it works best when compiled with a
     ...
     * and sm::vec does not add any of its own constructors. Prior to c++-17,
     ...
     */
    export template <typename S, std::size_t N> struct vec;
     ...
     */
    export template <typename S, std::size_t N> std::ostream& operator<< (std::ostream&, const vec<S, N>&);

    export template <typename S=float, std::size_t N=3>
    struct vec : public std::array<S, N>
    {
        ...
        // Unit x vector
        template <std::size_t Ny = N> requires (Ny > 0)
        static constexpr vec<S, Ny> ux() { return vec<S, Ny>{ S{1} }; }

        // Unit y vector
        template <std::size_t Ny = N> requires (Ny > 1)
        static constexpr vec<S, Ny> uy() { return vec<S, Ny>{ S{0}, S{1} }; }

        // Unit z vector
        template <std::size_t Ny = N> requires (Ny > 2)
        static constexpr vec<S, Ny> uz() { return vec<S, Ny>{ S{0}, S{0}, S{1} }; }

        // Unit vector in the Dim dimension. For example, u<0>() and ux() are equivalent
        template <std::size_t Dim, std::size_t Ny = N>
        static constexpr vec<S, Ny> u()
        {
            vec<S, Ny> unit = {};
            unit[Dim] = S{1};
            return unit;
        ...
        // Another unit vector, but this one takes dim as an argument
        template <std::size_t Ny = N>
        static constexpr vec<S, Ny> u (const std::size_t dim)
        {
            vec<S, Ny> unit = {};
            unit[dim] = S{1};
            return unit;
        ...
        /*!
         * Set from a tokenized string (commas by default). Works for simple types S
         *
         * This function throws exceptions, which may originate from std::stof (and friends) due to
         * bad input. For example, vec<float, 3>::set_from_str ("1,2,f") will throw an error because
         * the third element is a bad numeric string. However, you *can* leave elements
         * empty. vec<float, 3>::set_from_str ("1,,3") should result in a vec containing {1,0,3}
         */
        void set_from_str (const std::string& s, const std::string sep = ",")
        {
            if constexpr (false == (std::is_same_v<std::decay_t<S>, float> || std::is_same_v<std::decay_t<S>, double>
                                    || std::is_same_v<std::decay_t<S>, std::int32_t> || std::is_same_v<std::decay_t<S>, std::int64_t>
                                    || std::is_same_v<std::decay_t<S>, std::uint32_t> || std::is_same_v<std::decay_t<S>, std::uint64_t>)) {
                // Causes compile-time error. If you find you need to set from string with S==std::uint8_t, or
                // short 16 or something, then add the types here. For now stick to 32/64 bit types
                []<bool flag = false>() { static_assert(flag, "set_from (string type) only for simple scalar types S"); }();
            }

            if (sep.empty()) { return; } // Need to know separator

            *this = {}; // Zero first

            std::string::size_type ssz = sep.size(), a = 0, b = 0;
            std::size_t i = 0;
            while (a < s.size() && (b = s.find (sep, a)) != std::string::npos && i < N) {
                // convert s.substr (a, b-a) to type S and insert into *this[i++]
                std::string sub = s.substr (a, b-a);
                if (sub.empty()) {
                    (*this)[i++] = S{0};
                } else {
                    if constexpr (std::is_same_v<std::decay_t<S>, float>) {
                        (*this)[i++] = std::stof (s.substr (a, b - a));
                    } else if constexpr (std::is_same_v<std::decay_t<S>, double>) {
                        (*this)[i++] = std::stod (s.substr (a, b - a));
                    } else if constexpr (std::is_same_v<std::decay_t<S>, std::int32_t>) {
                        (*this)[i++] = std::stol (s.substr (a, b - a));
                    } else if constexpr (std::is_same_v<std::decay_t<S>, std::int64_t>) {
                        (*this)[i++] = std::stoll (s.substr (a, b - a));
                    } else if constexpr (std::is_same_v<std::decay_t<S>, std::uint32_t>) {
                        (*this)[i++] = std::stoul (s.substr (a, b - a));
                    } else if constexpr (std::is_same_v<std::decay_t<S>, std::uint64_t>) {
                        (*this)[i++] = std::stoull (s.substr (a, b - a));
                    } // there's no else
                }
                a = b + ssz;
            }
            // Last element probably has no separator
            if (a < s.size() && i < N) {
                b = s.size();
                if constexpr (std::is_same_v<std::decay_t<S>, float>) {
                    (*this)[i++] = std::stof (s.substr (a, b - a));
        ...
        //! Set from a tokenized string provided as std::string (commas by default). Works for simple types S
        void set_from (const std::string& s, const std::string sep = ",") { this->set_from_str (s, sep); }
        //! Set from a tokenized string provided as const char* (commas by default). Works for simple types S
        void set_from (const char* s, const std::string sep = ",") { this->set_from_str (s, sep); }
        ...
        constexpr void arange (const Sy start, const Sy2 stop, const Sy2 increment) noexcept
        {
            ...
            S num = sm::cem::ceil((stop - start) / increment);
            if (num > S{0}) {
        ...
        {
            if constexpr (std::is_same_v<S, T> == true) {
                return *this;
            } else {
                vec<T, N> v = { T{0} };
                v += *this;
        ...
        //! Return this vec in single precision, std::int32_t format
        constexpr vec<std::int32_t, N> as_int() const noexcept { return this->as<std::int32_t>(); }
        constexpr vec<std::int32_t, N> as_int32_t() const noexcept { return this->as<std::int32_t>(); }

        //! Return this vec in single precision, std::uint32_t format
        constexpr vec<std::uint32_t, N> as_uint() const noexcept { return this->as<std::uint32_t>(); }
        constexpr vec<std::uint32_t, N> as_uint32_t() const noexcept { return this->as<std::uint32_t>(); }
        ...
        {
            ss.precision (std::numeric_limits<S>::max_digits10);
            bool first = true;
            for (auto i : *this) {
                if (first) {
                    first = false;
                ...
                if constexpr (std::is_same<S, std::uint8_t>::value == true || std::is_same<S, char>::value == true) {
                    ss << static_cast<std::int32_t>(i);
                } else { ss << i; }
            }
        }
        ...
        constexpr void renormalize() noexcept
        {
            ...
            const Sy denom = sm::cem::sqrt (std::accumulate (this->begin(), this->end(), Sy{0}, add_squared));
            if (denom != Sy{0}) {
        ...
        {
            static_assert ((N%2==0), "N must be even to call sm::vec::rotate_pairs");
            S tmp_el = S{0};
         ...
         * max. Strictly, the range is [min, max)
         */
        void randomize_normal (S _mean, S _sd) noexcept
        {
         ...
         */
        static constexpr S unit_thresh() noexcept
        {
        ...
        constexpr bool checkunit() const noexcept
        {
            ...
            if (sm::cem::abs(metric) > sm::vec<Sy>::unit_thresh()) {
                return false;
            }
            return true;
        }
        ...
        constexpr Sy length() const noexcept
        {
            ...
            if constexpr (std::is_integral<std::decay_t<Sy>>::value == true) {
                return static_cast<Sy>(std::round(sm::cem::sqrt(std::accumulate(this->begin(), this->end(), Sy{0}, add_squared))));
            } else {
                return sm::cem::sqrt(std::accumulate(this->begin(), this->end(), Sy{0}, add_squared));
            }
        }
         ...
         *
         * If S typed elements are sm::vecs or sm::vvecs, then return the sum of the squares
        ...
        {
            auto abs_compare = [](S a, S b) { return (sm::cem::abs(a) < sm::cem::abs(b)); };
        ...
        constexpr bool has_inf() const noexcept
        {
            if constexpr (std::numeric_limits<S>::has_infinity) {
                return std::any_of (this->cbegin(), this->cend(), [](S i){return sm::cem::isinf(i);});
            } else {
                return false;
            }
        }
        ...
        constexpr bool has_nan() const noexcept
        {
            if constexpr (std::numeric_limits<S>::has_quiet_NaN || std::numeric_limits<S>::has_signaling_NaN) {
                return std::any_of (this->cbegin(), this->cend(), [](S i){return sm::cem::isnan(i);});
            } else {
                return false;
            }
        }
        ...
        constexpr void replace_nan_with (const S replacement) noexcept
        {
            ...
            for (auto& i : *this) { if (sm::cem::isnan(i)) { i = replacement; } }
        }
        ...
        constexpr void replace_nan_or_inf_with (const S replacement) noexcept
        {
            ...
            for (auto& i : *this) { if (sm::cem::isnan(i) || sm::cem::isinf(i)) { i = replacement; } }
        }
        ...
        //! Return the variance of the elements. Passing template arg correction = -1
        //! estimates a population variance from the sample
        template<typename Sy=S, std::int64_t correction = 0>
        constexpr Sy variance() const noexcept
        {
            ...
            std::int64_t sz = this->size();
            Sy variance = sos_deviations / (sz + correction);
            return variance;
        }
        ...
        constexpr Sy std() const noexcept
        {
            ...
            return sm::cem::sqrt (this->variance<Sy>());
        }
        ...
        {
            vec<S, N> rtn{};
            auto raise_to_p = [p](S elmnt) -> S { return static_cast<S>(sm::cem::pow (elmnt, p)); };
            std::transform (this->begin(), this->end(), rtn.begin(), raise_to_p);
            return rtn;
        }
        //! Raise each element to the power p
        constexpr void pow_inplace (const S& p) noexcept { for (auto& i : *this) { i = static_cast<S>(sm::cem::pow (i, p)); } }
        ...
        constexpr vec<S, N> pow (const vec<Sy, N>& p) const noexcept
        {
            auto pi = p.begin();
            vec<S, N> rtn{};
            auto raise_to_p = [pi](S elmnt) mutable -> S { return static_cast<S>(sm::cem::pow (elmnt, (*pi++))); };
            std::transform (this->begin(), this->end(), rtn.begin(), raise_to_p);
            return rtn;
        }
        template<typename Sy=S>
        ...
        {
            auto pi = p.begin();
            for (auto& i : *this) { i = static_cast<S>(sm::cem::pow (i, (*pi++))); }
        }
        ...
        {
            vec<S, N> rtn{};
            auto _floor = [](S elmnt) -> S { return (sm::cem::floor (elmnt)); };
            std::transform (this->begin(), this->end(), rtn.begin(), _floor);
            return rtn;
        }
        constexpr void floor_inplace() noexcept { for (auto& i : *this) { i = sm::cem::floor(i); } }

        //! Return the floor-or-ceiling of the vector's elements - i.e. apply trunc()
        constexpr vec<S, N> trunc() const noexcept
        {
            vec<S, N> rtn{};
            auto _trunc = [](S elmnt) -> S { return (sm::cem::trunc(elmnt)); };
            std::transform (this->begin(), this->end(), rtn.begin(), _trunc);
            return rtn;
        }
        constexpr void trunc_inplace() noexcept { for (auto& i : *this) { i = sm::cem::trunc(i); } }

        //! Return the ceiling of the vec
        constexpr vec<S, N> ceil() const noexcept
        {
            vec<S, N> rtn{};
            auto _ceil = [](S elmnt) -> S { return (sm::cem::ceil (elmnt)); };
            std::transform (this->begin(), this->end(), rtn.begin(), _ceil);
            return rtn;
        }
        constexpr void ceil_inplace() noexcept { for (auto& i : *this) { i = sm::cem::ceil(i); } }
        ...
        {
            vec<S, N> rtn{};
            auto sqrt_element = [](S elmnt) -> S { return static_cast<S>(sm::cem::sqrt (elmnt)); };
            std::transform (this->begin(), this->end(), rtn.begin(), sqrt_element);
            return rtn;
        }
        //! Replace each element with its own square root
        constexpr void sqrt_inplace() noexcept { for (auto& i : *this) { i = static_cast<S>(sm::cem::sqrt (i)); } }
        ...
        {
            vec<S, N> rtn{};
            auto log_element = [](S elmnt) -> S { return static_cast<S>(sm::cem::log (elmnt)); };
            std::transform (this->begin(), this->end(), rtn.begin(), log_element);
            return rtn;
        }
        //! Replace each element with its own natural log
        constexpr void log_inplace() noexcept { for (auto& i : *this) { i = static_cast<S>(sm::cem::log (i)); } }
        ...
        {
            vec<S, N> rtn{};
            auto log_element = [](S elmnt) -> S { return static_cast<S>(sm::cem::log10 (elmnt)); };
            std::transform (this->begin(), this->end(), rtn.begin(), log_element);
            return rtn;
        }
        //! Replace each element with its own log to base 10
        constexpr void log10_inplace() noexcept { for (auto& i : *this) { i = static_cast<S>(sm::cem::log10 (i)); } }
        ...
        {
            vec<S, N> rtn{};
            auto exp_element = [](S elmnt) -> S { return static_cast<S>(sm::cem::exp (elmnt)); };
            std::transform (this->begin(), this->end(), rtn.begin(), exp_element);
            return rtn;
        }
        //! Replace each element with its own natural exponential
        constexpr void exp_inplace() noexcept { for (auto& i : *this) { i = static_cast<S>(sm::cem::exp (i)); } }
        ...
        {
            vec<S, N> rtn{};
            auto abs_element = [](S elmnt) -> S { return static_cast<S>(sm::cem::abs(elmnt)); };
            std::transform (this->begin(), this->end(), rtn.begin(), abs_element);
            return rtn;
        }
        //! Replace each element with its own absolute value
        constexpr void abs_inplace() noexcept { for (auto& i : *this) { i = static_cast<S>(sm::cem::abs(i)); } }
        ...
        /*!
         * Use something like this as a compare function when storing sm::vecs in
         * a std::set:
         *
         ...
         * unique vecs.
         *
         * This can also be an issue with using a sm::vec as a key to an std::map. So similarly
         * do something like this to create a map with a key type sm::vec<std::int32_t, 2> and a value
         * type std::string:
         *
         *    auto _cmp = [](sm::vec<std::int32_t,2> a, sm::vec<std::int32_t,2> b){return a.lexical_lessthan(b);};
         *    std::map<sm::vec<std::int32_t, 2>, std::string, decltype(_cmp)> themap(_cmp);
         */
        template<typename Sy=S>
         ...
         */
        template<typename Sy=S>
        bool lexical_lessthan_beyond_epsilon (const vec<Sy, N>& rhs, const std::int32_t n_eps = 1) const noexcept
        {
        ...
        //! C++ modules change
        //! With modules (or g++-14?) we apparently no longer inherit operator!=
        constexpr bool operator!= (const vec<S, N>& rhs) const noexcept
        {
            std::uint32_t ndiff = 0u;
            for (std::uint32_t i = 0u; i < N && ndiff == 0u; ++i) { ndiff += (*this)[i] == rhs[i] ? 0u : 1u; }
            return ndiff > 0u;
        }
         ...
         * the angle (in radians) about the z axis (range [0, 2pi]) and phi is the azimuthal angle
         * (range [0, pi]).  This is the naming convention in mathematical texts. YOU MAY NEED TO
         * SWITCH theta AND phi because some functions, including boost::math for its spherical
         * harmonics uses a different convention, swapping theta and phi (the 'physics' convention)!
         */
        template <std::size_t Ny = N> requires (Ny == 3)
        constexpr vec<S, Ny> cartesian_to_spherical() const noexcept
        {
            ...
            S rho = this->length();
            spherical[0] = rho;                                      // rho
            spherical[1] = sm::cem::atan2 ((*this)[1], (*this)[0]);  // theta
            spherical[2] = sm::cem::acos ((*this)[2] / rho);         // phi
            return spherical;
        }

        /*!
         * Convert spherical coordinates (rho, theta, phi) to 3D Cartesian (x,y,z) where theta is
         * the angle in radians about the z axis (range [0, 2pi]) and phi is the azimuthal angle
         * (range [0, pi]).  This is the naming convention in mathematical texts, rather than the
         * 'physics' convention.
         */
        template <std::size_t Ny = N> requires (Ny == 3)
        constexpr vec<S, Ny> spherical_to_cartesian() const noexcept
        {
            vec<S, Ny> cartesian = {S{0}};
            // Assuming *this is (maths convention) spherical, convert to cartesian coordinates.
            const S rho = (*this)[0];
            const S theta = (*this)[1];
            const S phi = (*this)[2];
            // Project rho onto the plane
            S rho_proj = rho * sm::cem::sin (phi);
            // x and y come from theta and the rho projection
            cartesian[0] = rho_proj * sm::cem::sin (theta);
            cartesian[1] = rho_proj * sm::cem::cos (theta);
            // z from phi and rho
            cartesian[2] = rho * sm::cem::cos (phi);
            return cartesian;
        }

        /*!
         * Return the magnitude of the angle (in radians) between this vector and the other. Works
         * for any N.
         */
        constexpr S angle (const vec<S, N>& other) const noexcept
        {
            S cos_theta = this->dot(other) / (this->length() * other.length());
            return sm::cem::acos (cos_theta);
        }

        /*!
         * Return the angle in radians between this vector and the other. Works for any N.
         *
         * axis is the axis of rotation, so this angle IS signed, positive if 'other' is at a
         * positive right-handed angle wrt *this. axis does not need to be *exactly* the axis of
         * rotation, though it could be.
         */
        ...
        {
            S _angle = sm::cem::atan2 ((*this)[1], (*this)[0]);
            return _angle;
        }
        ...
        {
            S l = this->length();
            (*this)[0] = sm::cem::cos (_ang);
            (*this)[1] = sm::cem::sin (_ang);
            (*this) *= l > S{0} ? l : S{1};
        }
        ...
        constexpr void operator+= (const S& s) noexcept
        {
            auto add_s = [s](S elmnt) { return elmnt + s; };
            std::transform (this->begin(), this->end(), this->begin(), add_s);
        }
...
# endif
        ...
        //! Define a hash function for sm::vec that can be passed to unordered_map to use vec as a key
        // e.g.: std::unordered_map<sm::vec<float, 2>, std::int32_t, sm::vec<float, 2>::hash> vec_keyed_unordered_map;
        struct hash
        {
            std::size_t operator()(const vec<S, N>& v) const
            {
                if constexpr (N > 0u) {
                    std::size_t h = std::hash<S>{}(v[0]);
    ...
    export template <typename T, typename S, std::size_t N> requires std::is_arithmetic_v<T>
