...
export module sm.algo.edgeconv_2d;
...
export namespace sm::algo
{
     ...
     * rectangle.
     */
    template<typename T, std::int32_t w, bool invert_vert_edges=false, bool invert_horz_edges=false>
    void edgeconv_2d (const sm::vvec<T>& data, sm::vvec<T>& v_edges, sm::vvec<T>& h_edges)
    {
        ...
        std::int32_t lastrow_index = data.size() - w;

        for (std::int32_t i = 0; i < static_cast<std::int32_t>(data.size()); ++i) {
    ...
    template<typename T, std::int32_t w, std::int32_t h, bool invert_vert_edges=false, bool invert_horz_edges=false>
    void edgeconv_2d (const sm::vec<T, w*h>& data, sm::vec<T, w*h>& v_edges, sm::vec<T, w*h>& h_edges)
    {
        std::int32_t lastrow_index = (w*h) - w;
