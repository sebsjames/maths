 ...
 *
 * Wrappers around the HDF5 C API that play well with sm::vec and sm::vvec
 */
module;
...
export namespace sm
{
    ...
    /*!
     * The internal file access mode, specifying whether the data file should be opened
     * for reading, writing (truncating the file at the outset) or for read/write.
     */
    ...
    private:
        ...
        file_access_mode file_access = file_access_mode::truncate_write;
        ...
        hid_t open_dataset (const char* path, hid_t dtype_id, hid_t space_id)
        {
            ...
            if (this->file_access == file_access_mode::read_write) {
                if (dataset_id < 0) {
            ...
            if (dataset_id < 0) {
                std::stringstream ee;
                ee << __PRETTY_FUNCTION__ << ": H5Dcreate2 for path " << path
                   << " dtype_id " << dtype_id << " and space_id " << space_id
                   << " failed (it returned dataset_id = " << dataset_id << ")";
                throw std::runtime_error (ee.str());
            }
            return dataset_id;
        }
        ...
        void check_dataset_space_2_dims (hid_t dataset_id, hsize_t dim0, hsize_t dim1)
        {
            if (dataset_id < 0) {
                std::stringstream ee;
                ee << "In:\n" << __PRETTY_FUNCTION__
                   << "\nError: dataset_id is not valid (should be >= 0)";
                throw std::runtime_error (ee.str());
            }
            ...
            if (space_id >= 0) {
                ...
                std::cout  << __PRETTY_FUNCTION__ << ": No existing space opening dataset_id " << dataset_id << "; (space_id = "
                           << space_id << ") continue...\n";
            }
        }
        ...
        {
            if (dataset_id < 0) {
                if (this->on_read_error_action == read_error_action::carry_on) {
                    // Return with no action; vals will not be modified.
                    return -1;
                } else if (this->on_read_error_action == read_error_action::info) {
                    std::stringstream ii;
                    ii << "Info: " << path << " does not exist in this Hdf5 file";
                    std::cout << ii.str() << std::endl;
                    return -1;
                } else if (this->on_read_error_action == read_error_action::warning) {
                    std::stringstream ww;
                    ww << "Info: " << path << " does not exist in this Hdf5 file";
                    std::cerr << ww.str() << std::endl;
                    return -1;
                } else { // read_error_action::exception
                    std::stringstream ee;
    ...
    public:
        ...
        /*!
         * Construct, defining file access with the sm::file_access_mode enum.
         */
        hdfdata (const std::string fname,
                 const file_access_mode _file_access,
        ...
        void init (const std::string fname,
                   ...
                   const bool show_hdf_internal_errors = false)
        {
            if (file_id >= 0) { throw std::runtime_error ("This hdfdata object was already set up."); }

            this->_init (fname, _file_access, show_hdf_internal_errors);
        }
                   ...
                   const bool show_hdf_internal_errors = false)
        {
            ...
            file_access_mode fa_native = file_access_mode::read_only;
            if (_file_access == std::ios::in) {
                fa_native = file_access_mode::read_only;
            } else if (_file_access == std::ios::out) {
                fa_native = file_access_mode::read_write;
            } else if (_file_access == (std::ios::out | std::ios::trunc)) {
                fa_native = file_access_mode::truncate_write;
            } else {
                std::stringstream ee;
                ee << "hdfdata: Your ios_base::openmode options are: ios::in (for read_only); "
                   << "ios::out|ios::trunc (for truncate_write) or ios::out (for read_write)";
                throw std::runtime_error (ee.str());
            }

            this->_init (fname, fa_native, show_hdf_internal_errors);
        }
    ...
    private:
                    ...
                    const bool show_hdf_internal_errors)
        {
            ...
            if (this->file_access == file_access_mode::read_only) {
                // std::cout << "Open read-only\n";
                this->file_id = H5Fopen (fname.c_str(), H5F_ACC_RDONLY, H5P_DEFAULT);
            } else if (this->file_access == file_access_mode::read_write) {
    ...
    public:
         ...
         * which is why this is a read_error_action.
         */
        read_error_action on_read_error_action = read_error_action::info;
        ...
        void read_contained_vals (const char* path, Container<T, Allocator>& vals)
        {
            ...
            if constexpr (
                          ...
                          || std::is_same<std::decay_t<T>, sm::vec<float, 2>>::value == true
                          || std::is_same<std::decay_t<T>, sm::vec<double, 2>>::value == true
                          || std::is_same<std::decay_t<T>, sm::vec<int, 2>>::value == true
                          || std::is_same<std::decay_t<T>, sm::vec<long long int, 2>>::value == true
                          || std::is_same<std::decay_t<T>, sm::vec<unsigned int, 2>>::value == true
                          || std::is_same<std::decay_t<T>, sm::vec<unsigned long long int, 2>>::value == true
            ...
            if constexpr (std::is_same<std::decay_t<T>, float>::value == true
                          ...
                          || std::is_same<typename std::decay<T>::type, sm::vec<float,2>>::value == true
                ...
                status = H5Dread (dataset_id, H5T_NATIVE_FLOAT, H5S_ALL, H5S_ALL, H5P_DEFAULT, &(invals[0]));
                                 ...
                                 || std::is_same<typename std::decay<T>::type, sm::vec<double,2>>::value == true
                ...
                status = H5Dread (dataset_id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, &(invals[0]));
                                 ...
                                 || std::is_same<typename std::decay<T>::type, sm::vec<int,2>>::value == true
                ...
                status = H5Dread (dataset_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, &(invals[0]));
                                 ...
                                 || std::is_same<typename std::decay<T>::type, sm::vec<short int,2>>::value == true
                ...
                status = H5Dread (dataset_id, H5T_NATIVE_SHORT, H5S_ALL, H5S_ALL, H5P_DEFAULT, &(invals[0]));
                                 ...
                                 || std::is_same<typename std::decay<T>::type, sm::vec<unsigned int,2>>::value == true
                ...
                status = H5Dread (dataset_id, H5T_NATIVE_UINT, H5S_ALL, H5S_ALL, H5P_DEFAULT, &(invals[0]));
                                 ...
                                 || std::is_same<typename std::decay<T>::type, sm::vec<unsigned short int,2>>::value == true
                ...
                status = H5Dread (dataset_id, H5T_NATIVE_USHORT, H5S_ALL, H5S_ALL, H5P_DEFAULT, &(invals[0]));
                                 ...
                                 || std::is_same<typename std::decay<T>::type, sm::vec<unsigned long long int,2>>::value == true
                ...
                status = H5Dread (dataset_id, H5T_NATIVE_ULLONG, H5S_ALL, H5S_ALL, H5P_DEFAULT, &(invals[0]));
                                 ...
                                 || std::is_same<typename std::decay<T>::type, sm::vec<long long int,2>>::value == true
        ...
        //! read_contained_vals for an array<T,N> (and by extension, a sm::vec<T,N>)
        template<typename T, std::size_t N>
        ...
        template<typename T, std::size_t N>
        void read_contained_vals (const char* path, sm::vvec<sm::vec<T, N>>& vals)
        {
            hid_t dataset_id = H5Dopen2 (this->file_id, path, H5P_DEFAULT);
            if (this->check_dataset_id (dataset_id, path) == -1) { return; }
            hid_t space_id = H5Dget_space (dataset_id);
        ...
        {
            std::vector<std::string> pbits = sm::util::string_to_vector (path, "/");
         ...
         * void add_contained_vals (const char*, const std::vector<sm::vec<T, N>>&)
         */
        template <typename T>
        ...
        template<typename T>
        void add_contained_vals (const char* path, const sm::vvec<sm::vvec<T>>& vals)
        {
            if (vals.empty()) { return; }
            this->process_groups (path);
        ...
        //! add_contained_vals for vector<sm::vec<T, N>>
        template<typename T, std::size_t N>
        void add_contained_vals (const char* path, const std::vector<sm::vec<T, N>>& vals)
        {
            if (vals.empty()) { return; }
            this->process_groups (path);
            hsize_t dim_vecNdcoords[2];
        ...
        // Code duplication of the above template function for std::vector<sm::vec<T,N>> :(
        template<typename T, std::size_t N>
        void add_contained_vals (const char* path, const sm::vvec<sm::vec<T, N>>& vals)
        {
            if (vals.empty()) { return; }
            this->process_groups (path);
            hsize_t dim_vecNdcoords[2];
