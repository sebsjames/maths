...
namespace sm
{
    ...
    export
    {
        ...
        public:
            ...
            //! fixed seed
            rand_uniform (std::uint32_t _seed) noexcept
            {
            ...
            //! This constructor gives RN generator which works in range [a,b)
            rand_uniform (T a, T b, std::uint32_t _seed) noexcept
            {
                this->generator.seed (_seed);
                typename std::uniform_real_distribution<T>::param_type prms (a, b);
                this->dist.param (prms);
     ...
     */
    export template <typename T = double, typename E = std::mt19937_64> requires std::is_floating_point_v<std::decay_t<T>>
    class rand_normal
    {
    private:
        //! random device to provide a seed for the generator
    ...
    public:
        ...
        rand_normal (std::uint32_t _seed) noexcept
        {
            this->generator.seed (_seed);
            typename std::normal_distribution<T>::param_type prms (T{0}, T{1});
            this->dist.param (prms);
        ...
        //! This constructor gives RN generator with mean \a mean and standard deviation \a sigma
        rand_normal (T mean, T sigma, std::uint32_t _seed) noexcept
        {
            this->generator.seed (_seed);
            typename std::normal_distribution<T>::param_type prms (mean, sigma);
            this->dist.param (prms);
    ...
    public:
        ...
        rand_lognormal (std::uint32_t _seed) noexcept
        {
            this->generator.seed (_seed);
            typename std::lognormal_distribution<T>::param_type prms (T{0}, T{1});
            this->dist.param (prms);
        ...
        rand_lognormal (T mean, T sigma, std::uint32_t _seed) noexcept
        {
            this->generator.seed (_seed);
            typename std::lognormal_distribution<T>::param_type prms (mean, sigma);
            this->dist.param (prms);
     ...
     * long (plus (u)int16/32/64_t)
     *
     * \tparam T The type of the random number to be generated
     *
     * \tparam E The pseudo-random number generator engine.
     */
    export template <typename T = std::int32_t, typename E = std::mt19937> requires std::is_integral_v<std::decay_t<T>>
    class rand_poisson
    {
    private:
        //! random device to provide a seed for the generator
    ...
    public:
        ...
        rand_poisson (std::uint32_t _seed) noexcept
        {
            this->generator.seed (_seed);
            typename std::poisson_distribution<T>::param_type prms (T{0});
            this->dist.param (prms);
        ...
        //! This constructor gives RN generator with mean \a mean.
        template<typename F = T> requires std::is_arithmetic_v<F>
        rand_poisson (F mean, std::uint32_t _seed) noexcept
        {
            this->generator.seed (_seed);
            typename std::poisson_distribution<T>::param_type prms (mean);
            this->dist.param (prms);
     ...
     * \tparam E The pseudo-random number generator engine.
     */
    export template <typename T = double, typename E = std::mt19937> requires std::is_floating_point_v<std::decay_t<T>>
    ...
    public:
        ...
        rand_exponential (std::uint32_t _seed) noexcept
        {
            this->generator.seed (_seed);
            typename std::exponential_distribution<T>::param_type prms (T{1});
            this->dist.param (prms);
        ...
        rand_exponential (T rate, std::uint32_t _seed) noexcept
        {
            this->generator.seed (_seed);
            typename std::exponential_distribution<T>::param_type prms (rate);
            this->dist.param (prms);
     ...
     * \tparam E The pseudo-random number generator engine.
     */
    export template <typename T = double, typename E = std::mt19937> requires std::is_floating_point_v<T>
    ...
    public:
        ...
        //! Sets fixed seed \a _seed.
        rand_pareto (std::uint32_t _seed) noexcept : sm::rand_exponential<T, E>(T{1}, _seed) {}
        ...
        rand_pareto (T alpha, std::uint32_t _seed) noexcept : sm::rand_exponential<T, E>(alpha, _seed) {}
        //! This constructor gives RN generator with alpha \a alpha, sigma \a sigma. and a given seed
        rand_pareto (T alpha, T _sigma, std::uint32_t _seed) noexcept : sm::rand_exponential<T, E>(alpha, _seed)
        {
            this->sigma = _sigma;
        }
        //! Copy constructor copies the distribution parameters
    ...
    public:
        ...
        rand_string (const std::uint32_t l) noexcept : length(l) { this->setup_rng(); }
        //! Construct with given length \a l and character group \a _cg.
        rand_string (const std::uint32_t l, const chargroup& _cg) noexcept : length(l), cg(_cg) { this->setup_rng(); }
        ...
        std::string get() const noexcept
        {
            ...
            for (std::uint32_t i = 0U; i < this->length; ++i) {

                // Get a random number
                std::uint16_t rn = this->rng->get();
        ...
        std::string get (const std::uint32_t l) noexcept
        {
            this->length = l;
            return this->get();
        }
    ...
    private:
        ...
        void setup_rng() noexcept
        {
            ...
            this->rng = std::make_unique<rand_uniform<std::uint16_t>>(0, this->numchars()-1);
        }

        //! Return the number of characters total in each chargroup
        std::uint16_t numchars() const noexcept
        {
            std::uint16_t n = 0;
            switch (this->cg) {
        ...
        std::unique_ptr<rand_uniform<std::uint16_t>> rng;
