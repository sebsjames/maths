...
export namespace sm
{
    ...
    class anneal
    {
        ...
        static constexpr std::uint32_t min_steps_to_reanneal = 10;
    ...
    public: // Algorithm parameters to be adjusted by user before calling anneal::init()
        ...
        //! the algorithm has finished.
        std::uint32_t f_x_best_repeat_max = 10;
        ...
        std::uint32_t reanneal_after_steps = 100;
    ...
    public: // Parameter vectors and objective fn results need to be client-accessible.
        ...
        std::uint32_t f_x_best_repeats = 0;
    ...
    public: // User-accessible methods.
                ...
                const sm::vvec<sm::vec<T,2>>& param_ranges)
        {
            ...
            this->state = anneal_state::need_to_init;
        }
        ...
        void init()
        {
            ...
            this->k_f = static_cast<std::uint32_t>(this->n.exp().mean());
            ...
            this->state = anneal_state::need_to_compute;
        }
        ...
        void step()
        {
            ...
            if (this->stop_check()) {
                this->state = anneal_state::ready_to_stop;
                return;
            }

            if (this->state == anneal_state::need_to_compute_set) {
                this->complete_reanneal();
                this->state = anneal_state::need_to_step;
            }
            ...
            if (this->enable_reanneal && this->reanneal_test()) {
                ...
                this->state = anneal_state::need_to_compute_set;
            } else {
                this->state = anneal_state::need_to_compute;
            }
        }
    ...
    protected: // Internal algorithm methods.
        ...
        sm::vvec<T> generate_delta_parameter (const sm::vvec<T>& x_start) const
        {
            ...
            for (std::uint32_t i = 0; i < this->D; ++i) {
        ...
        void complete_reanneal()
        {
            ...
            if (T_re > T{0}) {
                std::uint32_t k_re = static_cast<std::uint32_t>(((this->T_0/T_re).log() / this->c).pow(D).mean());
                if (display_reanneal) {
            ...
            this->k_cost = static_cast<std::uint32_t>(eps + (tvdb3/this->c_cost).pow(this->D).mean());
