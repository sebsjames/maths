 ...
 *
 * \file
 * \brief An N dimensional vector class template, sm::vvec which derives from std::vector.
 *
 * \author Seb James
 * \date May 2020
 */
...
export namespace sm
{
     ...
     * on.
     *
     * This class is better for writing neural networks than sm::vec, whose size
    ...
    struct vvec : public std::vector<S, Al>
    {
        ...
        //! Should a function (linspace) respect the endpoint?
        enum class endpoint { no, yes };
         ...
         * sm::vvec, sm::vec etc and even with std::set (though not with std::map).
         *
         ...
         */
        template <typename C>
        requires (sm::is_copyable_container<C>::value && !std::is_same<std::decay_t<C>, S>::value)
        void set_from (const C& c)
        {
            this->resize (c.size());
        ...
        template <typename C>
        requires (sm::is_copyable_container<C>::value && std::is_same<std::decay_t<C>, S>::value)
        void set_from (const C& v) noexcept { std::fill (this->begin(), this->end(), v); }

        //! Set all elements from the value type v.
        template <typename Sy=S> requires (!sm::is_copyable_container<Sy>::value)
        void set_from (const Sy& v) noexcept { std::fill (this->begin(), this->end(), v); }
        ...
        void set_from_onelonger (const C& v) noexcept
        {
        ...
        //! \return this vvec in single precision, std::int32_t format
        vvec<std::int32_t> as_int() const { return this->as<std::int32_t>(); }
        vvec<std::int32_t> as_int32_t() const { return this->as<std::int32_t>(); }

        //! \return this vvec in single precision, std::uint32_t format
        vvec<std::uint32_t> as_uint() const { return this->as<std::uint32_t>(); }
        vvec<std::uint32_t> as_uint32_t() const { return this->as<std::uint32_t>(); }
         ...
         * rounding errors.
         */
        template <endpoint ep = endpoint::yes, typename Sy=S, typename Sy2=S>
        void linspace (const Sy start, const Sy2 stop, const std::size_t num = 0u)
        {
            if (num > 0u) { this->resize (num); }
            std::size_t sz = this->size();
            S _stop = static_cast<S>(stop);
            if constexpr (ep == endpoint::no) {
                // If num == 0 there was no resize and so size() could return 0
                if (sz > 0u) { _stop -= _stop / sz; }
            }
            S increment = (sz == 1u) ? S{0} : (_stop - static_cast<S>(start)) / (sz - 1u);
            for (std::size_t i = 0u; i < sz; ++i) { (*this)[i] = start + increment * i; }
        }
         ...
         * leading_spaces: How many leading spaces to insert after a newline
         */
        void str_comma_separated (std::stringstream& ss,
                                  const std::uint32_t num_in_line=std::numeric_limits<std::uint32_t>::max(),
                                  const char sep = ',', const std::uint32_t leading_spaces = 0) const
        {
            ...
            std::string nothing = "";
            std::string end_line = "\n";
            for (std::uint32_t sp = 0; sp < leading_spaces; ++sp) { end_line += " "; }
            for (auto i : *this) {
                if (first) {
                    // Test for char/std::uint8_t and cast to std::int32_t to avoid being interpreted as ascii
                    if constexpr (std::is_same<S, std::uint8_t>::value == true || std::is_same<S, char>::value == true) {
                        ss << std::setw(w) << static_cast<std::int32_t>(i);
                    } else { ss << std::setw(w) << i; }
                    first = false;
                    if (elementcount == num_in_line - 1) { ss << std::setw(0) << end_line; }
                } else {
                    ss << std::setw(0) << sep << std::setw(0) << (elementcount%num_in_line==0 ? end_line : nothing);
                    if constexpr (std::is_same<S, std::uint8_t>::value == true || std::is_same<S, char>::value == true) {
                        ss << std::setw(w) << static_cast<std::int32_t>(i);
                    } else { ss << std::setw(w) << i; }
                }
         ...
         * "(5,4,5,5,40)".
         */
        std::string str (const std::uint32_t num_in_line=std::numeric_limits<std::uint32_t>::max()) const
        {
            std::stringstream ss;
            ss.precision (std::numeric_limits<S>::max_digits10);
            ss << "(";
            this->str_comma_separated (ss, num_in_line, ',', 1);
            ss << ")";
            return ss.str();
        }

        //! Output the vector in a form suitable to paste into MATLAB or Octave
        std::string str_mat (const std::uint32_t num_in_line=std::numeric_limits<std::uint32_t>::max()) const
        {
            std::stringstream ss;
            ss.precision (std::numeric_limits<S>::max_digits10);
            ss << "[";
         ...
         */
        std::string str_numpy (const std::uint32_t num_in_line=std::numeric_limits<std::uint32_t>::max()) const
        {
            std::stringstream ss;
            ss.precision (std::numeric_limits<S>::max_digits10);
            ss << "np.array((";
            this->str_comma_separated (ss, num_in_line, ',', 10);
            ss << "))";
            return ss.str();
        }

        /*!
         * Output in format the same as if the array were printed with a Python print command
         */
        std::string str_pythprint (const std::uint32_t num_in_line=6) const
        {
            std::stringstream ss;
            ss << "[";
            ss.precision (8);
        ...
        std::string str_initializer (const std::uint32_t num_in_line=std::numeric_limits<std::uint32_t>::max()) const
        {
            std::stringstream ss;
            ss.precision (std::numeric_limits<S>::max_digits10);
            ss << "{";
         ...
         */
        void randomize_normal (S _mean, S _sd)
        {
        ...
        void shuffle()
        {
            std::random_device rd; // we have access to these via #include <sm/random>
        ...
        void rotate_pairs()
        {
            std::size_t N = this->size();
            if (N%2!=0) {
                throw std::runtime_error ("vvec size must be even to call sm::vvec::rotate_pairs");
            }
            S tmp_el = S{0};
        ...
        bool checkunit() const noexcept
        {
            ...
            if (std::abs(metric) > sm::vvec<S>::unit_thresh()) {
                return false;
            }
            return true;
        }
        ...
        //! (sm::vvec<sm::vec<T, N>>). Note that the enclosed non-scalar thing must have
        //! function length_lessthan (as sm::vec does).
        template <typename Sy=S> requires sm::is_copyable_fixedsize<std::decay_t<Sy>>::value
        S max() const noexcept
        {
        ...
        //! vvec of vecs version of argmax returns the index of the maximum length sm::vec
        template <typename Sy=S> requires sm::is_copyable_fixedsize<std::decay_t<Sy>>::value
        std::size_t argmax() const noexcept
        {
            auto themax = std::max_element (this->begin(), this->end(), [](S a, S b){return a.length_lessthan(b);});
        ...
        sm::range<S> range() const noexcept
        {
            ...
            if constexpr (test_for_nans) {
                if (this->has_nan()) {
                    ...
                    sm::vvec<S> sans_nans = this->prune_nan();
        ...
        /*!
         * vvec of vecs version of range().
         *
         * \return a range with min containing the minimum value for each dimension and max
         * containing the max value for each dimension. This gives the N-dimensional volume that
         * contains all the coordinates in the vvec.
         *
         * Same as vvec<>::extent
         */
        template <typename Sy=S> requires sm::is_copyable_fixedsize<std::decay_t<Sy>>::value
        ...
        template <typename Sy=S>
        sm::vvec<std::size_t> find (const Sy& val) const
        {
            sm::vvec<std::size_t> indices;
            std::size_t i = 0;
            for (i = 0; i < this->size(); i++) {
        ...
        template<bool test_for_nans = false, typename Sy=S, std::int64_t correction = 0>
        Sy variance() const noexcept
        {
            ...
            Sy variance = sos_deviations / (sz + correction - n_nans);
            return variance;
        }

        //! \return the mean(rtn[0]) and variance(rtn[1]) of the elements. Passing
        //! template arg correction = -1 estimates a population variance from the sample
        template<bool test_for_nans = false, typename Sy=S, std::int64_t correction = 0>
        sm::vvec<Sy> mean_variance() const noexcept
        {
            if (this->empty()) { return sm::vvec<Sy>{}; }
            Sy _mean = this->mean<test_for_nans, Sy>();
            Sy sos_deviations = Sy{0};
            std::size_t n_nans = 0u;
            for (S val : *this) {
            ...
            std::int64_t sz = this->size();
            Sy variance = sos_deviations / (sz + correction - n_nans);
            return sm::vvec<Sy>{ _mean, variance };
        }
        ...
        //! \return mean (rtn[0]) and standard deviation (rtn[1]) of the elements
        template<bool test_for_nans = false, typename Sy=S>
        sm::vvec<Sy> mean_std() const noexcept
        {
            if (this->empty()) { return sm::vvec<Sy>{}; }
            sm::vvec<Sy> ms = this->mean_variance<test_for_nans, Sy>();
            ms[1] = std::sqrt (ms[1]);
            return ms;
        }

        //! \return the sum of the elements. If elements are of a constrained type, you can call this something like:
        //! vvec<std::uint8_t> uv (256, 10);
        //! std::uint32_t thesum = uv.sum<false, std::uint32_t>();
        ...
        //! this something like:
        //! vvec<std::uint8_t> uv (256, 10);
        //! std::uint32_t theproduct = uv.product<std::uint32_t>();
        template<bool test_for_nans = false, typename Sy=S>
        ...
        {
            vvec<S> rtn(this->size());
            const S c = S{1} / std::sqrt (sm::mathconst<S>::two_pi * sigma * sigma);
            auto _element = [sigma, mu, c](S i) { return c * std::exp ((i - mu) * (i - mu) / (S{-2} * sigma * sigma)); };
            std::transform (this->begin(), this->end(), rtn.begin(), _element);
            return rtn;
        }
        void gauss_inplace (const S sigma, const S mu = S{0})
        {
            const S c = S{1} / std::sqrt (sm::mathconst<S>::two_pi * sigma * sigma);
            for (auto& i : *this) { i = c * std::exp ((i - mu) * (i - mu) / (S{-2} * sigma * sigma)); }
        }
        ...
        vvec<S> smooth_gauss (const S sigma, const std::uint32_t n_sigma) const
        {
            sm::vvec<S> filter;
        ...
        template<wrapdata wrap = wrapdata::none>
        void smooth_gauss_inplace (const S sigma, const std::uint32_t n_sigma)
        {
            sm::vvec<S> filter;
            S hw = std::round (sigma * n_sigma);
            std::size_t elements = static_cast<std::size_t>(2 * hw) + 1;
        ...
        {
            std::int32_t sz = this->size();
            std::int32_t osz = sz;           // osz is size of output vvec
            std::int32_t kw = kernel.size(); // kernel width
            std::int32_t zki = 0;            // zero of the kernel index (or how many steps 'right'
            ...
            for (std::int32_t i = 0; i < osz; ++i) {
                ...
                S sum = S{0};
                for (std::int32_t j = 0; j < kw; ++j) {
                    // ii is the index into the data by which kernel[j] should be multiplied
                    std::int32_t ii = i - j + zki; // -j effectively 'flips' the kernel, as is required by
        ...
        void convolve_inplace (const vvec<S>& kernel)
        {
            ...
            std::int32_t osz = sz;            // osz is size of output vvec
            std::int32_t kw = kernel.size();  // kernel width
            std::int32_t zki = 0;             // zero of the kernel index
            if constexpr (centre == centre_kernel::yes) { zki = kw / 2; }
        ...
        //
        // Use something like this as a compare function when storing sm::vvec in
        // a std::set:
        //
         ...
         */
        template<typename Sy=S>
        bool lexical_lessthan_beyond_epsilon (const vvec<Sy>& rhs, const std::int32_t n_eps = 1) const
        {
        ...
        //! C++ modules change
        //! With modules we apparently no longer inherit operator!=
        constexpr bool operator!= (const vvec<S>& rhs) const noexcept
        {
            if (this->size() != rhs.size()) { return true; }
            std::uint32_t ndiff = 0u;
            for (std::uint32_t i = 0u; i < this->size() && ndiff == 0u; ++i) { ndiff += (*this)[i] == rhs[i] ? 0u : 1u; }
            return ndiff > 0u;
        }
         ...
         */
        template <typename Sy=S>
        requires (std::is_scalar_v<std::decay_t<Sy>> || sm::is_copyable_fixedsize<std::decay_t<Sy>>::value)
        ...
        void operator+= (const S& s) noexcept
        {
            auto add_s = [s](S elmnt) -> S { return elmnt + s; };
            std::transform (this->begin(), this->end(), this->begin(), add_s);
        }
        ...
        {
            std::size_t sz = this->size();
            std::size_t sz_new = sz + a.size();
            this->reserve (sz_new); // necessary to avoid stringop-overflow warning on g++-15
            this->resize (sz_new);
