...
export namespace sm
{
     ...
     *
     * This could be re-written with template <typename T, std::uint32_t N> where N is the
    ...
    public:
        ...
        sm::vvec<std::uint32_t> vertex_order;
    ...
    private:
        ...
        void compute_x0()
        {
            ...
            for (std::uint32_t i = 0; i<this->n; ++i) { // *excluding* i==n
        ...
        void allocate()
        {
            ...
            std::uint32_t i = 0;
            for (std::uint32_t& vo : this->vertex_order) { vo = i++; }
        }
    };
