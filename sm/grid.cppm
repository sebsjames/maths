 ...
 * \file
 *
 * This file contains the definition for sm::grid, a simple Cartesian Grid
 * class.
 *
 * \author Seb James
 * \date February 2024
...
export namespace sm
{
     ...
     *
     * Note that you can change the grid parameters at runtime. For example, with grid, you could
     * model an expanding domain by changing grid::dx over time. You could model a shift in the
     * cooordinates that your grid maps by changing grid::g_offset.
     *
    ...
    {
    private:
        // Members that are template arguments in sm::grid. Set only via constructor or setters

        //! w Number of elements that the grid is wide
        I w = I{1};
        //! h Number of elements that the grid is high
        I h = I{1};
        //! dx A two element sm::vec providing the horizontal distance between
        ...
        gridorder order = sm::gridorder::bottomleft_to_topright;
    ...
    public:
        ...
        //! Setter for dx
        void set_dx (const sm::vec<C, 2> _dx) { this->dx = _dx; this->init(); }
        //! Setter for g_offset
        void set_offset (const sm::vec<C, 2> _offset) { this->offset = _offset; this->init(); }

        //! Setter for most of the grid parameters to be carried out all in one function
        void set_grid_params (const sm::vec<I, 2> dims,
                              const sm::vec<C, 2> spacing,
                              const sm::vec<C, 2> grid_offset)
        {
            this->w = dims[0];
            this->h = dims[I{1}];
            this->dx = spacing;
        ...
        sm::vec<I, 2> get_dims() const { return sm::vec<I, 2>{this->w, this->h}; }
        sm::vec<C, 2> get_dx() const { return this->dx; }
        sm::vec<C, 2> get_offset() const { return this->offset; }
        ...
        static sm::vec<I, 2> suggest_dims (const I num_elements, const bool allow_extra = false)
        {
            sm::vec<I, 2> w_h = { std::numeric_limits<I>::max(), std::numeric_limits<I>::max() };
            ...
            if (!factors.empty()) {
                sm::vvec<C> factors_minus_sqrt = factors.template as<C>() - std::sqrt(static_cast<C>(num_elements));
                factors_minus_sqrt.abs_inplace();
                std::size_t j = factors_minus_sqrt.argmin();
                if (j < factors.size()) {
                ...
                if (num_elements <= one_by_most) {
                    ...
                    for (I j = num_elements + I{1}; j < num_elements + possible_additional; ++j) {
                        w_h = sm::grid<I, C>::suggest_dims (j, false);
                        if (w_h != sm::vec<I, 2>{ std::numeric_limits<I>::max(), std::numeric_limits<I>::max() }) {
                            // success!
                            break;
                        }
                    }
        ...
        bool rowmaj() const
        {
            return (this->order == sm::gridorder::bottomleft_to_topright
                    || this->order == sm::gridorder::topleft_to_bottomright) ? true : false;
        }
        ...
        {
            std::stringstream ss;
            if (order == sm::gridorder::bottomleft_to_topright) {
            ...
            } else if (order == sm::gridorder::topleft_to_bottomright) {
            ...
            } else if (order == sm::gridorder::bottomleft_to_topright_colmaj) {
            ...
            } else if (order == sm::gridorder::topleft_to_bottomright_colmaj) {
        ...
        //! Default constructor does nothing. Call init() before use.
        grid(){}
        ...
        grid (const I _w, const I _h,
              ...
              const gridorder _order = sm::gridorder::bottomleft_to_topright)
            : w(_w)
            , h(_h)
            , dx(_dx)
            , offset(_offset)
        ...
        void init()
        {
            ...
            if constexpr (std::is_signed_v<I> == true) {
                ...
                std::int64_t test = this->w * this->h;
                ...
                std::uint64_t test = this->w * this->h;
                if (test > std::numeric_limits<I>::max()) {
                    throw std::runtime_error ("Use a larger capacity type for I");
                }
            }
        ...
        sm::vec<C, 2> operator[] (const I index) const
        {
            return index >= this->n() ? sm::vec<C, 2>{std::numeric_limits<C>::max(), std::numeric_limits<C>::max()} : this->v_c[index];
        }
        ...
        I index_lookup (const sm::vec<C, 2>& _coord) const
        {
            I index = I{0};
            sm::vec<C, 2> xyf = ((_coord - this->offset) / this->dx);
            xyf[0] = std::round (xyf[0]); // theres no vec::round() function at the time of writing
            xyf[1] = std::round (xyf[1]);

            if (order == sm::gridorder::topleft_to_bottomright
                || order == sm::gridorder::topleft_to_bottomright_colmaj) {
            ...
            sm::vec<I, 2> xyi = xyf.template as<I>();

            if (order == sm::gridorder::bottomleft_to_topright
                || order == sm::gridorder::topleft_to_bottomright) {
        ...
        sm::vec<C, 2> coord_lookup (const I index) const
        {
            return index >= this->n() ? sm::vec<C, 2>{std::numeric_limits<C>::max(), std::numeric_limits<C>::max()} : this->v_c[index];
        }

        //! Return the coordinate in polar coordinates
        sm::vec<C, 2> polar_lookup (const I index) const
        {
            sm::vec<C, 2> c = this->coord_lookup (index);
            return sm::vec<C, 2>{ c.length(), c.angle() };
        }

        //! Compute and return the coordinate with the given index
        sm::vec<C, 2> coord (const I index) const
        {
            if (index >= this->n()) { return sm::vec<C, 2>{std::numeric_limits<C>::max(), std::numeric_limits<C>::max()}; }
            sm::vec<C, 2> loc = this->offset;
            if (order == sm::gridorder::bottomleft_to_topright) {
        ...
        sm::vec<C, 2> coord_ne (const I index) const
        {
            I idx = index_ne (index);
            if (idx < this->n()) { return (*this)[idx]; }
            return sm::vec<C, 2>{std::numeric_limits<C>::max(), std::numeric_limits<C>::max()};
        }
        ...
        sm::vec<C, 2> coord_nw (const I index) const
        {
        ...
        sm::vec<C, 2> coord_nn (const I index) const
        {
        ...
        sm::vec<C, 2> coord_ns (const I index) const
        {
        ...
        sm::vec<C, 2> coord_nne (const I index) const
        {
            I idx = this->index_nne (index);
            return idx < this->n() ? (*this)[idx] : sm::vec<C, 2>{std::numeric_limits<C>::max(), std::numeric_limits<C>::max()};
        }
        ...
        sm::vec<C, 2> coord_nnw (const I index) const
        {
        ...
        sm::vec<C, 2> coord_nse (const I index) const
        {
        ...
        sm::vec<C, 2> coord_nsw (const I index) const
        {
        ...
        sm::vec<C, 4> extents() const { return sm::vec<C, 4>{ xmin(), xmax(), ymin(), ymax() }; }

        //! Return the coordinates of the centre of the grid
        sm::vec<C, 2> centre() const { return sm::vec<C, 2>{ xmax() - xmin(), ymax() - ymin() } * C{0.5}; }

        //! Return the y coordinates for each row in the grid
        sm::vvec<C> get_abscissae() const
        {
            sm::vvec<C> abscissae (w, C{0});
        ...
        sm::vvec<C> get_ordinates() const
        {
            sm::vvec<C> ordinates (h, C{0});
            if (order == gridorder::bottomleft_to_topright || order == gridorder::topleft_to_bottomright) {
         ...
         */
        I shift_index (const I ind, const sm::vec<std::int32_t, 2> delta) const
        {
        ...
        sm::vvec<float> resample_image (const sm::vvec<float>& image_data,
                                        ...
                                        const sm::vec<float, 2>& image_offset) const
        {
            if (this->order != sm::gridorder::bottomleft_to_topright) {
                throw std::runtime_error ("grid::resample_image: resampling assumes image has sm::gridorder::bottomleft_to_topright, so your grid should, too.");
            }

            sm::vvec<float> expr_resampled(this->w * this->h, 0.0f);
            ...
            sm::vec<float, 2> image_dims = { 1.0f, 0.0f };
            ...
            sm::vec<float, 2> dist_per_pix = image_dims / (image_pixelsz - 1u);
...
#pragma omp parallel for // parallel on this outer loop gives best result (5.8 s vs 7 s)
            for (typename std::vector<float>::size_type xi = 0u; xi < this->v_c.size(); ++xi) {
                ...
                for (std::uint32_t i = 0; i < csz; ++i) {
                    ...
                    sm::vec<float, 2> posn = (dist_per_pix * idx) + image_offset;
                    // Distance from input pixel to output pixel:
                    sm::vec<float, 2> _v_c = this->v_c[xi] - posn;
         ...
         */
        void indices_in_radius (const sm::vec<C,2> loc,
                                const C radius,
                                sm::vvec<I>& inds_in_radius) const
        {
            sm::vvec<I> inds_in_circle;
            sm::vvec<I> inds_in_previous_circle;
            std::set<I> seen;
         ...
         */
        void find_nearest_neighbours (const sm::vvec<I>& inds, sm::vvec<I>& neighbour_inds) const
        {
