...
export module sm.bootstrap;

export import sm.vec;
import sm.vvec;

export namespace sm::bootstrap
{
    // Resample B sets from data and place them in resamples
    template <typename T>
    void resample_with_replacement (const sm::vvec<T>& data,
                                    std::vector<sm::vvec<T>>& resamples, const std::uint32_t B)
    {
        std::uint32_t data_n = data.size();
        resamples.resize (B);
        for (std::uint32_t i = 0; i < B; ++i) {
            sm::vvec<T> resample (data_n);
            sm::vvec<std::uint32_t> randindices (data_n);
    ...
    template <typename T>
    T error_of_mean (const sm::vvec<T>& data, const std::uint32_t B)
    {
        std::vector<sm::vvec<T>> resamples;
        sm::bootstrap::resample_with_replacement<T> (data, resamples, B);
        sm::vvec<T> r_mean (B, T{0});
        for (std::uint32_t i = 0; i < B; ++i) {
    ...
    template <typename T>
    T error_of_mean (const std::vector<T>& data, const std::uint32_t B)
    {
        sm::vvec<T> vdata;
        vdata.set_from (data);
        return bootstrap::error_of_mean<T> (vdata, B);
    }

    // Compute a bootstapped standard error of the SD of the data with B resamples
    template <typename T>
    T error_of_std (const sm::vvec<T>& data, const std::uint32_t B)
    {
        std::vector<sm::vvec<T>> resamples;
        sm::bootstrap::resample_with_replacement<T> (data, resamples, B);
        sm::vvec<T> r_std (B, T{0});
        for (std::uint32_t i = 0; i < B; ++i) {
    ...
    template <typename T>
    T error_of_std (const std::vector<T>& data, const std::uint32_t B)
    {
        sm::vvec<T> vdata;
        vdata.set_from (data);
        return bootstrap::error_of_std<T> (vdata, B);
    }
    ...
    template <typename T>
    sm::vec<T, 2> ttest_equalityofmeans (const sm::vvec<T>& _zdata,
                                         const sm::vvec<T>& _ydata, const std::uint32_t B)
    {
        constexpr bool debug_bstrap = false;

        // Ensure that the group which we name zdata is the larger one.
        sm::vvec<T> zdata = _zdata;
        sm::vvec<T> ydata = _ydata;
        if (_zdata.mean() <= _ydata.mean()) { std::swap (ydata, zdata); }

        std::uint32_t n = zdata.size();
        std::uint32_t m = ydata.size();
        ...
        sm::vvec<T> ztilda = zdata - zmean + xmean;
        sm::vvec<T> ytilda = ydata - ymean + xmean;
        if constexpr (debug_bstrap) {
        ...
        std::vector<sm::vvec<T>> zstar;
        bootstrap::resample_with_replacement<T> (ztilda, zstar, B);
        if constexpr (debug_bstrap) {
            std::cout << "zstar size after resample: " << zstar.size() << std::endl;
        }
        std::vector<sm::vvec<T>> ystar;
        bootstrap::resample_with_replacement<T> (ytilda, ystar, B);

        // Create vectors of the means of these resamples:
        sm::vvec<T> zstarmeans (B, T{0});
        sm::vvec<T> ystarmeans (B, T{0});
        ...
        sm::vvec<T> zvariances (B, T{0});
        sm::vvec<T> yvariances (B, T{0});
        for (std::uint32_t i = 0; i < B; ++i) {
        ...
        sm::vvec<T> top = zstarmeans - ystarmeans;
        sm::vvec<T> bot = (yvariances/static_cast<T>(m) + zvariances/static_cast<T>(n)).sqrt();
        sm::vvec<T> txstar = top / bot;
        if constexpr (debug_bstrap) {
        ...
        return sm::vec<T, 2> ({asl, minasl});
    }
    // std::vector version of ttest_equalityofmeans()
    template <typename T>
    sm::vec<T, 2> ttest_equalityofmeans (const std::vector<T>& _zdata,
                                         const std::vector<T>& _ydata, const std::uint32_t B)
    {
        sm::vvec<T> vzdata;
        vzdata.set_from (_zdata);
        sm::vvec<T> vydata;
        vydata.set_from (_ydata);
        return bootstrap::ttest_equalityofmeans<T> (vzdata, vydata, B);
    }
}
