...
export namespace sm
{
    ...
    struct quaternion
    {
         ...
         * unit quaternion.
         */
        static constexpr F unit_thresh() noexcept
        {
        ...
        constexpr bool checkunit() const noexcept
        {
            ...
            if (sm::cem::abs(metric) > sm::quaternion<F>::unit_thresh()) {
                rtn = false;
            }
            return rtn;
        }

        //! Return a quaternion that contains the elements of this quaternion in type \tparam T
        template<typename T>
        constexpr quaternion<T> as() const noexcept
        {
            quaternion<T> qr;
            qr.w = static_cast<T>(this->w);
            qr.x = static_cast<T>(this->x);
            qr.y = static_cast<T>(this->y);
            qr.z = static_cast<T>(this->z);
            return qr;
        }

        //! Return this quaternion in single precision, float format
        constexpr quaternion<float> as_float() const noexcept { return this->as<float>(); }

        //! Return this quaternion in double precision, float format
        constexpr quaternion<double> as_double() const noexcept { return this->as<double>(); }
        ...
        // Interpolate between this quaternion and the quaternion q2. spherical linear interpolation
        // This code was adapted from Eigen (thanks Eigen!)
        //
        // \return the interpolated quaternion, or a special quaternion with a
        // std::numeric_limits<F>::max() value indicating error. if rtn.w is max, then t was out of
        // range. If rtn.x is max, then *this is not a normalized quaternion. If rtn.y is max, then
        ...
        constexpr quaternion<F> slerp (const quaternion<F>& q2, const F t) const noexcept
        {
            ...
            //sm::vec<F> new_coeffs = scale0 * my_coeffs + scale1 * q2_coeffs;
            my_coeffs *= scale0;
            q2_coeffs *= scale1;
            my_coeffs += q2_coeffs;

            interpolated.w = my_coeffs[0];
            interpolated.x = my_coeffs[1];
            interpolated.y = my_coeffs[2];
            interpolated.z = my_coeffs[3];
         ...
         * around the axis given by \a axis_x, \a axis_y, \a axis_z. The axis *must* be of length 1.
         */
         ...
         *
         * By default, renorm_axis is true, and so this axis is renormalized before being
         * passed to the core rotate method. You *may* set renorm_axis to false to save some
         * computations, but if you do, then the first 3 elements of your axis *must* encode a 3D
         * vector of length 1, otherwise, this function will not yield the rotation you expect!
         */
        template <typename V, bool renorm_axis = true>
        ...
        constexpr void rotate (const V& axis, const F& angle) noexcept
        {
            if constexpr (renorm_axis == true) {
                const F oneovermag = F{1} / sm::cem::sqrt (axis[0] * axis[0] + axis[1] * axis[1] + axis[2] * axis[2]);
                this->rotate (axis[0] * oneovermag, axis[1] * oneovermag , axis[2] * oneovermag, angle);
            } else {
                this->rotate (axis[0], axis[1], axis[2], angle);
            }
        }
         ...
         *  3  7 11 15
         */
        constexpr sm::vec<F, 16> rotation_matrix() const noexcept
        {
            sm::vec<F, 16> mat;
         ...
         */
        constexpr void rotation_matrix (sm::vec<F, 16>& mat) const noexcept
        {
            ...
            mat /= this->norm_squared();
        }

        //! Obtain rotation matrix assuming this IS a unit quaternion
        constexpr sm::vec<F, 16> unit_rotation_matrix() const noexcept
        {
            sm::vec<F, 16> mat;
            this->unit_rotation_matrix (mat);
            return mat;
        }

        //! Populate 4x4 transform matrix \a mat with the rotation represented by this quaternion,
        //! *assuming it's a unit quaternion*.
        constexpr void unit_rotation_matrix (sm::vec<F, 16>& mat) const noexcept
        {
        ...
        //! Unary negate operator. -q represents the same rotation as q!
        constexpr quaternion<F> operator-() const noexcept
        {
            return quaternion<F>{ -this->w, -this->x, -this->y, -this->z };
        }

        //! glTF reads quaternion strings as x,y,z,w
        std::string str_gltf() const
        {
            std::stringstream ss;
