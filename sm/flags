// -*- C++ -*-
/*!
 * This file is part of sebsjames/maths, a library of maths code for modern C++
 *
 * See https://github.com/sebsjames/maths
 *
 * A Boolean flags class
 *
 * You can use sm::flags<E> rather like std::bitset<10>. The difference is that E may
 * be (and must be) an enum class, which does not work with a bitset without ugly static
 * casting.
 *
 * Usage:
 *
 * First create your enum class
 *
 * enum class myflags : uint64_t { // up to 64 flags are possible with this underlying type
 *     flag_one, // Name the class and flags in some way that is right for your application
 *     flag_two  // It is not necessary to be explicit about the values of each flag
 * };
 *
 * (It is up to you to ensure that your enum class does not contain more flags than there are bits
 * in the underlying type!)
 *
 * sm::flags<myflags> fl;
 *
 * Set a flag:
 *
 * fl.set (myflags::flag_one);
 *
 * Test a flag is set:
 *
 * bool flag_one_is_set = fl.test (myflags::flag_one);
 *
 * Test several flags in one call to test with an initializer list:
 *
 * bool flags_one_AND_two_set = fl.test ({myflags::flag_one, myflags::flag_two});
 *
 * You can also use the alias all_of:
 *
 * bool flags_one_AND_two_set = fl.all_of ({myflags::flag_one, myflags::flag_two});
 *
 * Or use any_of() for an OR test:
 *
 * bool flags_one_OR_two_set = fl.any_of ({myflags::flag_one, myflags::flag_two});
 *
 * Clear/reset a flag:
 *
 * fl.reset (myflags::flag_one);
 *
 * Flip a flag:
 *
 * fl.flip (myflags::flag_one);
 *
 * You can stream your flags object:
 *
 * enum class myflags : uint8_t { one, two, three, four };
 * std::cout << sm::flags<myflags>{myflags::one} << std::endl;
 * std::cout << sm::flags<myflags>{myflags::three} << std::endl;
 *
 * Gives output:
 *
 * 00000001b
 * 00000100b
 *
 * The output is a binary string representation, with '1' and '0' characters followed by a 'b'. The
 * number of digits depends on the size of the underlying type (in this case, there are 8 bits in
 * uint8_t). The binary string shows the first flag as the least significant bit to the right. For
 * the example above, we get 00000001b for myflags::one and 00000100b for myflags::three:
 *
 *
 * This code was adapted from an idea in the Vulkan code base via
 * https://gist.github.com/fschoenberger/54c5342f220af510e1f78308a8994a45
 *
 * Author: Seb James
 * Date: Jan 2025
 */
module;

#include <compare>
#include <type_traits>
#include <initializer_list>
#include <ostream>

export module sm.flags;

export namespace sm
{
    template <typename E> requires std::is_enum_v<E> struct flags;
    template <typename E> std::ostream& operator<< (std::ostream&, const flags<E>&);

    template <typename E> requires std::is_enum_v<E>
    struct flags
    {
        using I = std::underlying_type_t<E>;

        // constructors
        constexpr flags() noexcept : bits(I{0}) {}
        constexpr flags(E flag) noexcept : bits( I{1} << static_cast<I>(flag) ) {}
        constexpr flags(const flags<E>& rhs) noexcept = default;
        constexpr explicit flags(I _bits) noexcept : bits(_bits) {}

        // logical operator
        constexpr bool operator!() const noexcept { return !this->bits; }

        // bitwise operators
        constexpr flags<E> operator&(const flags<E>& rhs) const noexcept { return flags<E>(this->bits & rhs.bits); }
        constexpr flags<E> operator|(const flags<E>& rhs) const noexcept { return flags<E>(this->bits | rhs.bits); }
        constexpr flags<E> operator^(const flags<E>& rhs) const noexcept { return flags<E>(this->bits ^ rhs.bits); }
        constexpr flags<E> operator~() const noexcept { return flags<E>(bits ^ I{-1}); }

        // assignment operators
        constexpr flags<E>& operator=(const flags<E>& rhs) noexcept = default;

        constexpr flags<E>& operator=(const I& rhs) noexcept
        {
            this->bits = rhs;
            return *this;
        }

        constexpr flags<E>& operator|=(const flags<E>& rhs) noexcept
        {
            this->bits |= rhs.bits;
            return *this;
        }
        constexpr flags<E>& operator&=(const flags<E>& rhs) noexcept
        {
            this->bits &= rhs.bits;
            return *this;
        }
        constexpr flags<E>& operator^=(const flags<E>& rhs) noexcept
        {
            this->bits ^= rhs.bits;
            return *this;
        }

        // Set from a bit
        constexpr flags<E>& operator|=(const E& rhs) noexcept
        {
            this->bits |= (I{1} << static_cast<I>(rhs));
            return *this;
        }
        // Set from a bit (std::bitset-like function name)
        //
        // NB: If static_cast<I>(flag) is greater than the number of bits in the type I, then no
        // flag will be set. That is, it is up to the programmer to ensure that the enum E does not
        // have more flags than bits in its underlying type I.
        constexpr void set (const E& flag, bool value = true) noexcept
        {
            if (value == true) {
                this->bits |= (I{1} << static_cast<I>(flag));
            } else {
                this->bits &= ~(I{1} << static_cast<I>(flag));
            }
        }

        // Set several bits in a single call
        constexpr void set (std::initializer_list<const E> flags, bool value = true) noexcept
        {
            I tbits = I{0};
            for (auto flag : flags) { tbits |= (I{1} << static_cast<I>(flag)); }
            if (value == true) {
                this->bits |= tbits;
            } else {
                this->bits &= ~(tbits);
            }
        }

        // Flip a bit (in enum form)
        constexpr flags<E>& operator^=(const E& rhs) noexcept
        {
            this->bits ^= (I{1} << static_cast<I>(rhs));
            return *this;
        }

        // Unset from a bit (std::bitset-like function name)
        constexpr void reset (const E& flag) noexcept
        {
            this->bits &= ~(I{1} << static_cast<I>(flag));
        }

        // Reset several flags in one call
        constexpr void reset (std::initializer_list<const E> flags) noexcept
        {
            I tbits = I{0};
            for (auto flag : flags) { tbits |= (I{1} << static_cast<I>(flag)); }
            this->bits &= ~(tbits);
        }

        // Unset all (std::bitset-like function name)
        constexpr void reset() noexcept { this->bits = I{0}; }

        // Flip a flag (std::bitset-like function name)
        constexpr void flip (const E& flag) noexcept
        {
            this->bits ^= (I{1} << static_cast<I>(flag));
        }

        // Test a flag  (std::bitset-like function name)
        constexpr bool test (const E& flag) const noexcept
        {
            return (this->bits & (I{1} << static_cast<I>(flag))) > I{0} ? true : false;
        }

        // Test that multiple flags are set with an intializer list. True only if ALL are set
        constexpr bool test (std::initializer_list<const E> flags) const noexcept
        {
            I tbits = I{0};
            for (auto flag : flags) { tbits |= (I{1} << static_cast<I>(flag)); }
            return (this->bits & tbits) == tbits;
        }

        // Test that multiple flags are set with an intializer list. True only if ALL are
        // set. Companion to flags::any_of (and sub-calls flags::test (std::initializer_list<const E>))
        constexpr bool all_of (std::initializer_list<const E> flags) const noexcept { return this->test (flags); }

        // True if any of the multiple flags in the intializer list are set. True if ANY are set
        constexpr bool any_of (std::initializer_list<const E> flags) const noexcept
        {
            I tbits = I{0};
            for (auto flag : flags) { tbits |= (I{1} << static_cast<I>(flag)); }
            return (this->bits & tbits) != I{0};
        }

        // Get the underlying bits
        constexpr I get() const noexcept { return this->bits; }

        // Set bits from number of type I
        constexpr void set (const I& _bits) { this->bits = _bits; }

        // Return the number of flags set true
        constexpr I count() const noexcept
        {
            I n = this->bits;
            I c = I{0};
            while (n) { n &= n--, ++c; } // Kernighan's algorithm
            return c;
        }

        // Return true if any flags are set
        constexpr bool any() const noexcept { return this->count() > I{0}; }

        // Return true if no flags are set
        constexpr bool none() const noexcept { return this->count() == I{0}; }

        // Note: there is no flags::all() to match std::bitset::all() because it is not currently
        // possible in C++ to know how many enumerated values there are in the enum class E.

        // cast operators
        explicit constexpr operator bool() const noexcept { return !!this->bits; }
        explicit constexpr operator I() const noexcept { return this->bits; }

        friend std::ostream& operator<< <E> (std::ostream& os, const flags<E>& flags);
    private:
        I bits = I{0};
    };

    template <typename E> requires std::is_enum_v<E>
    std::ostream& operator<< (std::ostream& os, const flags<E>& flags)
    {
        using I = std::underlying_type_t<E>;
        constexpr I sz = sizeof(I) * I{8};
        I bits = flags.get();
        for (I i = sz; i > I{0}; --i) {
            // '0' is ASCII 0x30; '1' is ASCII 0x31
            os << static_cast<char>((bits >> (i - I{1}) & I{1}) + 0x30);
        }
        os << 'b';
        return os;
    }

    // bitwise operators
    template <typename E> requires std::is_enum_v<E>
    constexpr flags<E> operator&(E flag, const flags<E>& flags) noexcept
    {
        return flags.operator&(flag);
    }

    template <typename E> requires std::is_enum_v<E>
    constexpr flags<E> operator|(E flag, const flags<E>& flags) noexcept
    {
        return flags.operator|(flag);
    }

    template <typename E> requires std::is_enum_v<E>
    constexpr flags<E> operator^(E flag, const flags<E>& flags) noexcept
    {
        return flags.operator^(flag);
    }

    // bitwise operators on E
    template <typename E> requires std::is_enum_v<E>
    inline constexpr flags<E> operator&(E lhs, E rhs) noexcept
    {
        return flags<E>(lhs) & rhs;
    }

    template <typename E> requires std::is_enum_v<E>
    inline constexpr flags<E> operator|(E lhs, E rhs) noexcept
    {
        return flags<E>(lhs) | rhs;
    }

    template <typename E> requires std::is_enum_v<E>
    inline constexpr flags<E> operator^(E lhs, E rhs) noexcept
    {
        return flags<E>(lhs) ^ rhs;
    }

    template <typename E> requires std::is_enum_v<E>
    inline constexpr flags<E> operator~(E flag) noexcept
    {
        return ~(flags<E>(flag));
    }

} // namespace sm
