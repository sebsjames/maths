...
export namespace sm::algo
{
     ...
     */
    template<typename T, std::int32_t boxside, std::int32_t w, bool onlysum = false, typename T_o = T>
    void boxfilter_2d (const sm::vvec<T>& data, sm::vvec<T_o>& result)
    {
        ...
        static constexpr std::int32_t halfbox = boxside / 2;
        static constexpr std::int32_t halfbox_p1 = halfbox + 1;

        sm::vec<T_o, w> colsum;
        colsum.zero();
        T_o rowsum = T{0};

        std::int32_t h = data.size() / w;

        // process data row by row
        for (std::int32_t y = -halfbox; y < h; ++y) {
            ...
            if (y >= 0) {
                ...
                for (std::int32_t i = -halfbox_p1; i < 0; ++i) { rowsum += colsum[i + w]; }
                for (std::int32_t i = 0; i < halfbox; ++i) { rowsum += colsum[i]; }

                // 3. Compute the sum along the row, and write this into result
                std::int32_t l = -halfbox_p1;
                std::int32_t r = halfbox;
                for (std::int32_t x = 0; x < w; ++x) {
     ...
     */
    template<typename T, std::int32_t boxside, std::int32_t w, std::int32_t h, bool onlysum = false, typename T_o = T>
    void boxfilter_2d (const std::array<T, w * h>& data, std::array<T_o, w * h>& result)
    {
    ...
    void boxfilter_2d (const sm::vvec<T>& data, sm::vvec<T>& result, const std::int32_t w)
    {
        if constexpr (boxside % 2 == 0) {
            throw std::runtime_error ("boxfilter_2d was not designed for even box filter squares (set boxside template param. to an odd value)");
        }
