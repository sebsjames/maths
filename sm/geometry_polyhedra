 ...
 *
 * Geometry algorithms to generate icosahedrons and geodesic polyhedrons.
 */
module;
...
export module sm.geometry_polyhedra;
...
export namespace sm::geometry_ce
{
    ...
    struct polyhedron
    {
        ...
        sm::vec<sm::vec<F, 3>, n_verts> vertices;
        //! A list of the faces
        sm::vec<sm::vec<int, 3>, n_faces> faces;
    };
    ...
    {
        constexpr F phi = (F{1} + sm::mathconst<F>::root_5) / F{2};

        // Unit vectors in 3D
        constexpr sm::vec<F, 3> _ux = { F{1}, F{0}, F{0} };
        constexpr sm::vec<F, 3> _uz = { F{0}, F{0}, F{1} };


        sm::geometry_ce::polyhedron<F, 12, 20> ico;
        // Arranged 'in spiral order', going with positive angle in x/y plane (i.e. around z axis)
        ico.vertices = sm::vec< sm::vec<F, 3>, 12> {
            sm::vec<F, 3>{ F{-1}, phi,   F{0}  },

            sm::vec<F, 3>{ F{1},  phi,   F{0}  },
            sm::vec<F, 3>{ F{0},  F{1},  -phi  },
            sm::vec<F, 3>{ -phi,  F{0},  F{-1} },
            sm::vec<F, 3>{ -phi,  F{0},  F{1}  },
            sm::vec<F, 3>{ F{0},  F{1},  phi   },
        ...
        sm::quaternion<F> rotn1(_uz, -sm::cem::atan2(F{1}, phi));
        // ...and a rotation about the x axis:
        sm::quaternion<F> rotn2(_ux, sm::mathconst<F>::pi_over_2);
        ...
        ico.faces = sm::vec< sm::vec<int, 3>, 20>{
            sm::vec<int, 3>{0, 1, 2},
            sm::vec<int, 3>{0, 2, 3},
            sm::vec<int, 3>{0, 3, 4},
            sm::vec<int, 3>{0, 4, 5},
            ...
            sm::vec<int, 3>{10, 11, 6},
            sm::vec<int, 3>{6, 11, 7},
            sm::vec<int, 3>{7, 11, 8},
            sm::vec<int, 3>{8, 11, 9},
            sm::vec<int, 3>{9, 11, 10}
        };
     ...
     * sm::geometry version. DO want to test for duplicate vertices.
     *
     ...
     *
     * \return the literal type object sm::geometry_ce::icosahedral_geodesic containing
     * vertices and faces of the geodesic.
     */
    template<typename F, int iterations>
    constexpr sm::geometry_ce::icosahedral_geodesic<F, iterations> make_icosahedral_geodesic()
    {
        sm::geometry_ce::icosahedral_geodesic<F, iterations> geo; // our return object

        // geo must be fully initialized in a constexpr function (this could be constructor in icosahedral_geodesic<>
        constexpr sm::vec<int, 3> fz = { 0, 0, 0 };
        constexpr sm::vec<F, 3> vz = { F{0}, F{0}, F{0} };
        for (std::uint32_t i = 0; i < geo.poly.faces.size(); ++i) { geo.poly.faces[i] = fz; }
        for (std::uint32_t i = 0; i < geo.poly.vertices.size(); ++i) { geo.poly.vertices[i] = vz; }

        // Start out with an icosahedron
        constexpr sm::geometry_ce::polyhedron<F, 12, 20> initial_ico = sm::geometry_ce::icosahedron<F>();
        ...
        for (int i = 0; i < iterations; ++i) {
            ...
            for (int f = 0; f < _n_faces; ++f) { // Loop over existing faces
                sm::vec<F, 3> va = (geo.poly.vertices[geo.poly.faces[f][1]] + geo.poly.vertices[geo.poly.faces[f][0]]) / 2.0f;
                sm::vec<F, 3> vb = (geo.poly.vertices[geo.poly.faces[f][2]] + geo.poly.vertices[geo.poly.faces[f][1]]) / 2.0f;
                sm::vec<F, 3> vc = (geo.poly.vertices[geo.poly.faces[f][0]] + geo.poly.vertices[geo.poly.faces[f][2]]) / 2.0f;
                va.renormalize();
                vb.renormalize();
                vc.renormalize();
                ...
                for (int v = 0; v < _n_verts; ++v) {
                    if (sm::cem::abs(geo.poly.vertices[v][0] - va[0]) < thresh * std::numeric_limits<F>::epsilon()
                        && sm::cem::abs(geo.poly.vertices[v][1] - va[1]) < thresh * std::numeric_limits<F>::epsilon()
                        && sm::cem::abs(geo.poly.vertices[v][2] - va[2]) < thresh * std::numeric_limits<F>::epsilon()) {
                ...
                int b = -1;
                for (int v = 0; v < _n_verts; ++v) {
                    if (sm::cem::abs(geo.poly.vertices[v][0] - vb[0]) < thresh * std::numeric_limits<F>::epsilon()
                        && sm::cem::abs(geo.poly.vertices[v][1] - vb[1]) < thresh * std::numeric_limits<F>::epsilon()
                        && sm::cem::abs(geo.poly.vertices[v][2] - vb[2]) < thresh * std::numeric_limits<F>::epsilon()) {
                ...
                int c = -1;
                for (int v = 0; v < _n_verts; ++v) {
                    if (sm::cem::abs(geo.poly.vertices[v][0] - vc[0]) < thresh * std::numeric_limits<F>::epsilon()
                        && sm::cem::abs(geo.poly.vertices[v][1] - vc[1]) < thresh * std::numeric_limits<F>::epsilon()
                        && sm::cem::abs(geo.poly.vertices[v][2] - vc[2]) < thresh * std::numeric_limits<F>::epsilon()) {
                ...
                // 4 new triangles.
                sm::vec<int, 3> newface = { geo.poly.faces[f][0], a, c };
...
export namespace sm::geometry
{
    ...
    struct polyhedron
    {
        ...
        sm::vvec<sm::vec<F, 3>> get_face_centres() const
        {
            sm::vvec<sm::vec<F, 3>> fcentres(this->faces.size(), { F{0}, F{0}, F{0} });
            for (std::uint32_t i = 0; i < this->faces.size(); ++i) {
        ...
        sm::vvec<sm::vvec<sm::vec<F, 3>>> get_neighbour_vectors() const
        {
            sm::vvec<sm::vvec<sm::vec<F, 3>>> vneighb_vectors_per_vertex (this->vneighbours.size());
            for (std::uint32_t i = 0; i < this->vneighbours.size(); ++i) {
                for (auto n : this->vneighbours[i]) { // vneighbour[i] is a set<int>
                    sm::vec<F, 3> _v = this->vertices[n] - this->vertices[i];
    ...
    struct icosahedral_geodesic
    {
        //! The base polyhedron that defines this icosahedral geodesic polyhedron
        geometry::polyhedron <F> poly;

        //! The 12 vertices with five-fold symmetry
        std::set<std::int32_t> fivefold_vertices;

        /*!
         * Get a vvec of vvecs of vecs. There are as many vvecs of vecs as there are vertices in
         * the geodesic. For each one, there will be 3 vecs, the 'R, G and B' basis vectors for
         * the hexagonal-like mesh.
         *
         * Note: In this function I use the default number of elements, N, for a sm::vec<F,
         * N> which is N=3. vec<F> is equivalent to vec<F, 3>. I also have some function-wide
         * using statements to keep the code a bit shorter.
         */
        sm::vvec< sm::vvec<std::tuple<sm::vec<F>, std::int32_t>> > get_neighbour_hexdir_vectors() const
        {
            using sm::vec;
            using sm::vvec;
            using std::tuple;
            ...
            vvec<vvec<tuple<vec<F>, std::int32_t>>> vneighb_vectors_per_vertex (this->poly.vneighbours.size());
            for (auto& vn : vneighb_vectors_per_vertex) { vn.reserve (6); } // There can be up to 6 neighbours
             ...
             * vectors. It uses a reference vector 'ref_vec' to figure out which neighbour
             * vector should be 'R'. The 'G' and 'B' basis vectors are then selected
             * sequentially with increasing anticlockwise angle about the vertex normal.
             *
             ...
             */
            auto _select_hex_basis_vectors = [&vneighb_vectors_per_vertex] (const std::int32_t i,
                                                                            const vec<F>& ref_vec,
                                                                            const vec<F>& nrm_vec)
            {
                constexpr F Fmax = std::numeric_limits<F>::max();
                vvec<tuple<vec<F>, std::int32_t>> rgb_vectors (3, {vec<F>{ F{0}, F{0}, F{0} }, -1});

                std::int32_t vnsz = static_cast<std::int32_t>(vneighb_vectors_per_vertex[i].size()); // 5 or 6 neighbours?

                vvec<F> angles(vnsz, F{0});
                F min_angle = Fmax;                 // Find the minimum angle
                std::int32_t min_angle_index = -1;           // index of min_angle *into vneighb_vectors_per_vertex[i]*
                std::int32_t min_angle_vertex_index = -1;    // index of the min_angle vertex *into poly.vertices*

                // First loop simply finds rgb_vector[0], which is the vector most aligned with ref_vec
                for (std::int32_t j = 0; j < vnsz; ++j) {
                    // Compute each angle wrt to ref_vec
                    auto vec_ij = std::get<vec<F>>(vneighb_vectors_per_vertex[i][j]);
                    angles[j] = vec_ij.angle (ref_vec);
                    if (angles[j] < min_angle) {
                        min_angle_index = j;
                        min_angle_vertex_index = std::get<std::int32_t>(vneighb_vectors_per_vertex[i][j]);
                        min_angle = angles[j];
                    }
                }
                //std::int32_t r_index = min_angle_index; // This is not the original index of the neighbour
                rgb_vectors[0] = { std::get<vec<F>>(vneighb_vectors_per_vertex[i][min_angle_index]), min_angle_vertex_index };

                // Now find the next two, in anticlockwise order.
                angles.zero();
                std::map<F, tuple<vec<F>, std::int32_t>> vec_map; // A map with key as angle, to order vectors easily
                for (std::int32_t j = 0; j < vnsz; ++j) {
                    auto vtx_ij = std::get<vec<F>>(vneighb_vectors_per_vertex[i][j]);
                    if (j != min_angle_index) {
                        // Compute each angle wrt to the reference (rgb_vectors[0])
                        angles[j] = vtx_ij.angle (std::get<vec<F>>(rgb_vectors[0]));
                        // The angle to the normal of the cross product should be nearly 0 or nearly pi
                        vec<F> cross_prod = vtx_ij.cross (std::get<vec<F>>(rgb_vectors[0]));
                        bool in_line = cross_prod.angle (nrm_vec) < mc::pi_over_2;
                        // This ensures angles will order nicely (as the key in the map). It assumes vectors lie on a plane (which is nearly true)
                        angles[j] = in_line == false ? angles[j] : mc::two_pi - angles[j];
                    } // else angle[j] == 0
                ...
                // ...then swap into vneighb_vectors_per_vertex. Done.
                std::swap (vneighb_vectors_per_vertex[i], rgb_vectors);
            }; // end of _select_hex_basis_vectors()

            // Unit vector in x direction
            constexpr vec<F> ux = { F{1}, F{0}, F{0} };
            // Algorithm choices
            constexpr bool align_all_R_with_ux = true;
            constexpr bool align_each_R_with_a_neighbour = !align_all_R_with_ux;

            for (std::int32_t i = 0; i < static_cast<std::int32_t>(this->poly.vneighbours.size()); ++i) {

                // Compute the neighbour vectors
                for (std::int32_t n : this->poly.vneighbours[i]) { // vneighbour[i] is a set<std::int32_t>
                    vec<F> _v = this->poly.vertices[n] - this->poly.vertices[i];
                    vneighb_vectors_per_vertex[i].push_back ({_v,  n}); // n IS index into poly.vertices
                }

                vec<F> nrm = this->poly.vertices[i]; // no need to renormalize

                if constexpr (align_all_R_with_ux) {

                    // For each vertex, our reference is ux. This is quite good in large regions.
                    _select_hex_basis_vectors (i, ux, nrm);

                } else if constexpr (align_each_R_with_a_neighbour) {

                    // Do first wrt ux, and subsequent wrt any neighbour. Works ok.
                    if (i == 0) {
                        _select_hex_basis_vectors (i, ux, nrm);
                    } else {
                        // From i and down, see if it's a neighbour. If so, use it.
                        for (std::int32_t id = i-1; id >= 0; --id) {
                            // Is vertex j a neighbour?
                            if (this->poly.vneighbours[i].count(id)) {
                                // yes, so use it
                                _select_hex_basis_vectors (i, std::get<vec<F>>(vneighb_vectors_per_vertex[id][0]), nrm);
                            } // else loop to next j
                        }
                    }

                } else {
                    throw std::runtime_error ("get_neighbour_hexdir_vectors: check your constexpr bools");
                }
            }
    ...
    {
        icosahedral_geodesic_info (const std::int32_t iterations)
        {
            double d = std::round (std::pow (4, iterations));
            if (d <= static_cast<double>(std::numeric_limits<std::int32_t>::max()) && d >= 0.0) {
                const std::int32_t T = static_cast<std::int32_t>(d);
                this->n_vertices = 10 * T + 2;
                this->n_faces = 20 * T;
            }
        }
        std::int32_t n_vertices = 0;
     ...
     */
    template<typename F>
    polyhedron<F> icosahedron()
    {
        sm::geometry::polyhedron<F> ico;
        ico.vertices.resize(12);
        ico.faces.resize(20);
        sm::geometry_ce::polyhedron<F, 12, 20> initial_ico = sm::geometry_ce::icosahedron<F>();
     ...
     */
    template<typename F>
    sm::geometry::icosahedral_geodesic<F> make_icosahedral_geodesic (const std::int32_t iterations)
    {
        sm::geometry::icosahedral_geodesic<F> geo;
        // Start out with an icosahedron
        geo.poly = sm::geometry::icosahedron<F>();

        // As geo is currently an icosahedron, all vertices in geo are five-fold at this point
        for (std::int32_t v = 0; v < static_cast<std::int32_t>(geo.poly.vertices.size()); ++v) { geo.fivefold_vertices.insert(v); }
        ...
        auto _vtx_cmp = [](sm::vec<F, 3> a, sm::vec<F, 3> b)
        {
        ...
        // sm::vec is key to map, as we will have a very custom sorting function. This
        // requires some care with the sorting function used by the std::map
        std::map<sm::vec<F, 3>, std::int32_t, decltype(_vtx_cmp)> vertices_map(_vtx_cmp);
        std::map<std::int32_t, std::int32_t> idx_remap;
        ...
        for (std::int32_t i = 0; i < iterations; ++i) {
            ...
            std::map<sm::vec<F, 3>, // Comparing on the vertex key
                     sm::vec<std::int32_t, 3>, decltype(_vtx_cmp)> faces_map(_vtx_cmp);
            std::int32_t count = 0;
                                                  ...
                                                  const std::int32_t newface_num)
            {
                ...
                if (static_cast<std::size_t>(count) != faces_map.size()) { throw std::runtime_error ("count != faces_map.size()"); }
            };
            ...
            for (const auto f : geo.poly.faces) { // faces contains indexes into vertices.
                ...
                sm::vec<F, 3> va = (geo.poly.vertices[f[1]] + geo.poly.vertices[f[0]]) / 2.0f;
                sm::vec<F, 3> vb = (geo.poly.vertices[f[2]] + geo.poly.vertices[f[1]]) / 2.0f;
                sm::vec<F, 3> vc = (geo.poly.vertices[f[0]] + geo.poly.vertices[f[2]]) / 2.0f;
                ...
                sm::vec<std::int32_t, 3> newface = { f[0], a, c }; // indices in old money here
            ...
            std::int32_t k = 0;
            idx_remap.clear();
            std::set<std::int32_t> ffv; // temporary storage for a new fivefold_vertices set
