...
export namespace sm
{
    ...
    struct histo
    {
         ...
         * workflow is Construct -> access results in histo::bins histo::binwidth histo::counts
         * histo::proportions and histo::densities.
         *
         ...
         * histo::counts histo::proportions and histo::densities.
         *
        ...
        void init (const Container<H, Allocator>& data, std::size_t n, const sm::range<H>& manual_datarange)
        {
            ...
            this->proportions.zero();
            this->densities.resize (n, T{0});
            this->densities.zero();
            ...
            this->densities = this->proportions / this->binwidth;
        }

        //! Initialize without a manual data range. Can be used to re-init the histo
        template < template <typename, typename> typename Container, typename Allocator=std::allocator<H> >
        ...
        //! The counts computed as probability densities [density = count / (sum(counts) * binwidth)].
        sm::vvec<T> densities;
    };
}
