...
int main()
{
    ...
    constexpr morph::quaternion<float> q1 (morph::vec<float>{0,1,0}, morph::mathconst<float>::pi_over_4);

    // A transformation matrix, which is initialized as the identity matrix
    constexpr morph::mat44<float> t;
    ...
    t.rotate (q1);
    t.translate (morph::vec<float, 3>{ 0, 0, 2 });
    ...
    t.scale (morph::vec<float, 3>{ 0.5f, 2.0f, 0.25f });

    // An initial vector
    morph::vec<float, 3> v1 = { 1, 0, 0 };

    // Apply the transformation to v1; multiply v1 by the transformation matrix. There
    // are operator* overloads for mat44<T> * morph::vec<T, 3> and for mat44<T> *
    // morph::vec<T, 4>, but both return vec<T, 4>:
    //
    morph::vec<float, 4> v_4d = t * v1;
    // which you can then reduce with vec<>::less_one_dim():
    morph::vec<float, 3> v_3d = (t * v1).less_one_dim();
