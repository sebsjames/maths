...
namespace sj
{
     ...
     * sj::Visual. The matrix data is stored in mat44::mat, an array of 16
    ...
    public:
        ...
        constexpr sj::vec<F, 4> row (unsigned int idx) const noexcept
        {
            sj::vec<F, 4> r = {F{0}, F{0}, F{0}, F{0}};
            if (idx > 3U) { return r; }
            r[0] = this->mat[idx];
            r[1] = this->mat[idx+4];
            r[2] = this->mat[idx+8];
        ...
        constexpr sj::vec<F, 4> col (unsigned int idx) const noexcept
        {
            sj::vec<F, 4> c = {F{0}, F{0}, F{0}, F{0}};
            if (idx > 3U) { return c; }
            idx *= 4U;
            c[0] = this->mat[idx];
            c[1] = this->mat[++idx];
        ...
        //! Compute a sjing transformation to turn simplex(ABCD) into simplex(EFGH). Avoid
        ...
        constexpr void translate (const sj::vec<T, 3>& dv) noexcept
        {
            this->mat[12] += dv[0];
            this->mat[13] += dv[1];
            this->mat[14] += dv[2];
        ...
        template<typename T> requires std::is_arithmetic_v<T>
        constexpr void pretranslate (const sj::vec<T, 3>& dv) noexcept
        {
            mat44<T> trans_mat;
            trans_mat [12] = dv[0];
            trans_mat [13] = dv[1];
        ...
        template<typename T> requires std::is_arithmetic_v<T>
        constexpr void scale (const sj::vec<T, 3>& scl) noexcept
        {
            this->mat[0] *= scl[0];
            this->mat[1] *= scl[0];
            this->mat[2] *= scl[0];
        ...
        constexpr mat44<F> pure_rotation (const sj::quaternion<T>& q) noexcept
        {
        ...
        template <typename T = float> requires std::is_arithmetic_v<T>
        constexpr void rotate (const sj::quaternion<T>& q) noexcept
        {
        ...
        //! Rotate an angle theta radians about axis (specified as sj::vec)
        template <typename T> requires std::is_arithmetic_v<T>
        constexpr void rotate (const sj::vec<T, 3>& axis, const T& theta) noexcept
        {
            sj::quaternion<T> q;
            q.rotate (axis, theta);
            this->rotate<T> (q);
        ...
        template <typename T = float> requires std::is_arithmetic_v<T>
        constexpr void prerotate (const sj::quaternion<T>& q) noexcept
        {
            mat44<F> m = this->pure_rotation (q);
            *this *= m;
        }
        ...
        //! Prerotate an angle theta radians about axis (specified as sj::vec)
        template <typename T> requires std::is_arithmetic_v<T>
        constexpr void prerotate (const sj::vec<T, 3>& axis, const T& theta) noexcept
        {
            sj::quaternion<T> q;
            q.rotate (axis, theta);
            this->prerotate<T> (q);
        ...
        constexpr sj::mat33<F> linear() noexcept
        {
            sj::mat33<F> x;
        ...
        constexpr sj::vec<F, 3> translation() noexcept
        {
        ...
        //! Do matrix times vector multiplication, v = mat * v1
        constexpr sj::vec<F, 4> operator* (const sj::vec<F, 4>& v1) const noexcept
        {
            sj::vec<F, 4> v;
            v[0] = this->mat[0] * v1.x()
                + this->mat[4] * v1.y()
        ...
        //! Do matrix times vector multiplication, v = mat * v1.
        constexpr sj::vec<F, 4> operator* (const sj::vec<F, 3>& v1) const noexcept
        {
            sj::vec<F, 4> v;
            v[0] = this->mat[0] * v1.x()
                + this->mat[4] * v1.y()
        ...
        constexpr void perspective (F fovDeg, F aspect, F zNear, F zFar) noexcept
        {
            ...
            F fovRad_ov2 = fovDeg * sj::mathconst<F>::pi_over_360; // fovDeg/2 converted to radians
            F sineFov = sj::math::sin (fovRad_ov2);
            if (sineFov == F{0}) { return; }
            F cotanFov = sj::math::cos (fovRad_ov2) / sineFov;
            F clip = zFar - zNear;
         ...
         */
        constexpr void orthographic (const sj::vec<F, 2>& lb, const sj::vec<F, 2>& rt,
