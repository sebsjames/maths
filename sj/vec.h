...
namespace sj
{
     ...
     *
     * Because sj::vec extends std::array, it works best when compiled with a
     ...
     * and sj::vec does not add any of its own constructors. Prior to c++-17,
    ...
    struct vec : public std::array<S, N>
    {
        ...
        constexpr void arange (const Sy start, const Sy2 stop, const Sy2 increment) noexcept
        {
            ...
            S num = sj::math::ceil((stop - start) / increment);
            if (num > S{0}) {
        ...
        constexpr void renormalize() noexcept
        {
            ...
            const Sy denom = sj::math::sqrt (std::accumulate (this->begin(), this->end(), Sy{0}, add_squared));
            if (denom != Sy{0}) {
        ...
        {
            static_assert ((N%2==0), "N must be even to call sj::vec::rotate_pairs");
            S tmp_el = S{0};
        ...
        constexpr bool checkunit() const noexcept
        {
            ...
            if (sj::math::abs(metric) > sj::vec<Sy>::unitThresh()) {
                return false;
            }
            return true;
        }
        ...
        constexpr Sy length() const noexcept
        {
            ...
            if constexpr (std::is_integral<std::decay_t<Sy>>::value == true) {
                return static_cast<Sy>(std::round(sj::math::sqrt(std::accumulate(this->begin(), this->end(), Sy{0}, add_squared))));
            } else {
                return sj::math::sqrt(std::accumulate(this->begin(), this->end(), Sy{0}, add_squared));
            }
        }
         ...
         *
         * If S typed elements are sj::vecs or sj::vvecs, then return the sum of the squares
        ...
        {
            auto abs_compare = [](S a, S b) { return (sj::math::abs(a) < sj::math::abs(b)); };
        ...
        constexpr sj::range<S> range() const noexcept
        {
            auto mm = std::minmax_element (this->begin(), this->end());
            return sj::range<S>(*mm.first, *mm.second);
        }
        ...
        constexpr bool has_inf() const noexcept
        {
            if constexpr (std::numeric_limits<S>::has_infinity) {
                return std::any_of (this->cbegin(), this->cend(), [](S i){return sj::math::isinf(i);});
            } else {
                return false;
            }
        }
        ...
        constexpr bool has_nan() const noexcept
        {
            if constexpr (std::numeric_limits<S>::has_quiet_NaN || std::numeric_limits<S>::has_signaling_NaN) {
                return std::any_of (this->cbegin(), this->cend(), [](S i){return sj::math::isnan(i);});
            } else {
                return false;
            }
        }
        ...
        constexpr void replace_nan_with (const S replacement) noexcept
        {
            ...
            for (auto& i : *this) { if (sj::math::isnan(i)) { i = replacement; } }
        }
        ...
        constexpr void replace_nan_or_inf_with (const S replacement) noexcept
        {
            ...
            for (auto& i : *this) { if (sj::math::isnan(i) || sj::math::isinf(i)) { i = replacement; } }
        }
        ...
        constexpr Sy std() const noexcept
        {
            ...
            return sj::math::sqrt (this->variance<Sy>());
        }
        ...
        {
            vec<S, N> rtn{};
            auto raise_to_p = [p](S elmnt) -> S { return static_cast<S>(sj::math::pow (elmnt, p)); };
            std::transform (this->begin(), this->end(), rtn.begin(), raise_to_p);
            return rtn;
        }
        //! Raise each element to the power p
        constexpr void pow_inplace (const S& p) noexcept { for (auto& i : *this) { i = static_cast<S>(sj::math::pow (i, p)); } }
        ...
        constexpr vec<S, N> pow (const vec<Sy, N>& p) const noexcept
        {
            auto pi = p.begin();
            vec<S, N> rtn{};
            auto raise_to_p = [pi](S elmnt) mutable -> S { return static_cast<S>(sj::math::pow (elmnt, (*pi++))); };
            std::transform (this->begin(), this->end(), rtn.begin(), raise_to_p);
            return rtn;
        }
        template<typename Sy=S>
        ...
        {
            auto pi = p.begin();
            for (auto& i : *this) { i = static_cast<S>(sj::math::pow (i, (*pi++))); }
        }
        ...
        {
            vec<S, N> rtn{};
            auto _floor = [](S elmnt) -> S { return (sj::math::floor (elmnt)); };
            std::transform (this->begin(), this->end(), rtn.begin(), _floor);
            return rtn;
        }
        constexpr void floor_inplace() noexcept { for (auto& i : *this) { i = sj::math::floor(i); } }

        //! Return the floor-or-ceiling of the vector's elements - i.e. apply trunc()
        constexpr vec<S, N> trunc() const noexcept
        {
            vec<S, N> rtn{};
            auto _trunc = [](S elmnt) -> S { return (sj::math::trunc(elmnt)); };
            std::transform (this->begin(), this->end(), rtn.begin(), _trunc);
            return rtn;
        }
        constexpr void trunc_inplace() noexcept { for (auto& i : *this) { i = sj::math::trunc(i); } }

        //! Return the ceiling of the vec
        constexpr vec<S, N> ceil() const noexcept
        {
            vec<S, N> rtn{};
            auto _ceil = [](S elmnt) -> S { return (sj::math::ceil (elmnt)); };
            std::transform (this->begin(), this->end(), rtn.begin(), _ceil);
            return rtn;
        }
        constexpr void ceil_inplace() noexcept { for (auto& i : *this) { i = sj::math::ceil(i); } }
        ...
        {
            vec<S, N> rtn{};
            auto sqrt_element = [](S elmnt) -> S { return static_cast<S>(sj::math::sqrt (elmnt)); };
            std::transform (this->begin(), this->end(), rtn.begin(), sqrt_element);
            return rtn;
        }
        //! Replace each element with its own square root
        constexpr void sqrt_inplace() noexcept { for (auto& i : *this) { i = static_cast<S>(sj::math::sqrt (i)); } }
        ...
        {
            vec<S, N> rtn{};
            auto log_element = [](S elmnt) -> S { return static_cast<S>(sj::math::log (elmnt)); };
            std::transform (this->begin(), this->end(), rtn.begin(), log_element);
            return rtn;
        }
        //! Replace each element with its own natural log
        constexpr void log_inplace() noexcept { for (auto& i : *this) { i = static_cast<S>(sj::math::log (i)); } }
        ...
        {
            vec<S, N> rtn{};
            auto log_element = [](S elmnt) -> S { return static_cast<S>(sj::math::log10 (elmnt)); };
            std::transform (this->begin(), this->end(), rtn.begin(), log_element);
            return rtn;
        }
        //! Replace each element with its own log to base 10
        constexpr void log10_inplace() noexcept { for (auto& i : *this) { i = static_cast<S>(sj::math::log10 (i)); } }
        ...
        {
            vec<S, N> rtn{};
            auto exp_element = [](S elmnt) -> S { return static_cast<S>(sj::math::exp (elmnt)); };
            std::transform (this->begin(), this->end(), rtn.begin(), exp_element);
            return rtn;
        }
        //! Replace each element with its own natural exponential
        constexpr void exp_inplace() noexcept { for (auto& i : *this) { i = static_cast<S>(sj::math::exp (i)); } }
        ...
        {
            vec<S, N> rtn{};
            auto abs_element = [](S elmnt) -> S { return static_cast<S>(sj::math::abs(elmnt)); };
            std::transform (this->begin(), this->end(), rtn.begin(), abs_element);
            return rtn;
        }
        //! Replace each element with its own absolute value
        constexpr void abs_inplace() noexcept { for (auto& i : *this) { i = static_cast<S>(sj::math::abs(i)); } }
        ...
        /*!
         * Use something like this as a compare function when storing sj::vecs in
         * a std::set:
         *
         ...
         * unique vecs.
         *
         * This can also be an issue with using a sj::vec as a key to an std::map. So similarly
         * do something like this to create a map with a key type sj::vec<int, 2> and a value
         * type std::string:
         *
         *    auto _cmp = [](sj::vec<int,2> a, sj::vec<int,2> b){return a.lexical_lessthan(b);};
         *    std::map<sj::vec<int, 2>, std::string, decltype(_cmp)> themap(_cmp);
         */
        template<typename Sy=S>
        ...
        constexpr vec<S, Ny> cartesian_to_spherical() const noexcept
        {
            ...
            S rho = this->length();
            spherical[0] = rho;                                  // rho
            spherical[1] = sj::math::atan2 ((*this)[1], (*this)[0]);  // theta
            spherical[2] = sj::math::acos ((*this)[2] / rho);         // phi
            return spherical;
        }
        ...
        {
            S _angle = sj::math::atan2 ((*this)[1], (*this)[0]);
            return _angle;
        }
        ...
        {
            S l = this->length();
            (*this)[0] = sj::math::cos (_ang);
            (*this)[1] = sj::math::sin (_ang);
            (*this) *= l > S{0} ? l : S{1};
        }
