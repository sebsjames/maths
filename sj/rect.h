...
namespace sj {
    ...
    public:
        ...
        //! Load the data for this rect from a sj::hdfdata file
        ...
        float distanceFrom (const sj::vec<float, 2>& cartesianPoint) const
        {
            float deltax = cartesianPoint[0] - x;
            float deltay = cartesianPoint[1] - y;
            return std::sqrt (deltax*deltax + deltay*deltay);
         ...
         * the variables recorded for this rect. It's used in sj::CartGrid to
        ...
        sj::vec<float, 3> position() const
        {
            sj::vec<float,3> rtn = { { this->x, this->y, this->z } };
            return rtn;
        }
         ...
         * rows. (unlikely to be useful, but included to match API of sj::Hex)
         */
