 ...
 * (sj::Visual, sj::HexGridVisual, etc).
 *
...
namespace sj {
    ...
    public:
         ...
         * an axis.
         */
        constexpr quaternion (const sj::vec<F, 3>& axis, const F angle) noexcept : w(F{1}), x(F{0}), y(F{0}), z(F{0}) { this->rotate (axis, angle); }
        ...
        {
            F oneovermag = F{1} / sj::math::sqrt (w*w + x*x + y*y + z*z);
            this->w *= oneovermag;
            this->x *= oneovermag;
            this->y *= oneovermag;
            this->z *= oneovermag;
        ...
        constexpr bool checkunit() noexcept
        {
            ...
            if (sj::math::abs(metric) > sj::quaternion<F>::unitThresh()) {
                rtn = false;
            }
            return rtn;
        }
        ...
        {
            return (sj::math::abs(this->w - rhs.w) < std::numeric_limits<F>::epsilon()
                    && sj::math::abs(this->x - rhs.x) < std::numeric_limits<F>::epsilon()
                    && sj::math::abs(this->y - rhs.y) < std::numeric_limits<F>::epsilon()
                    && sj::math::abs(this->z - rhs.z) < std::numeric_limits<F>::epsilon());
        }
        ...
        constexpr sj::vec<F, N> rotate_vec (const sj::vec<Fy, N>& v_r) const noexcept
        {
            sj::quaternion<F> v_quat ( F{0}, static_cast<F>(v_r[0]), static_cast<F>(v_r[1]), static_cast<F>(v_r[2]) );
            sj::quaternion<F> v_rotated = (*this * v_quat) * this->conjugate();
            return { v_rotated.x, v_rotated.y, v_rotated.z };
        }

        //! Rotate a vector v_r by this quaternion, returning the resulting rotated vector
        template <typename Fy=F, std::size_t N = 3> requires (N == 3 || N == 4)
        constexpr sj::vec<F, N> operator* (const sj::vec<Fy, N>& v_r) const noexcept
        {
        ...
        constexpr F magnitude() const noexcept { return sj::math::sqrt (w*w + x*x + y*y + z*z); }
        //! Return the norm of the quaternion (aka the magnitude)
        constexpr F norm() const noexcept { return sj::math::sqrt (w*w + x*x + y*y + z*z); }
        //! Sometimes you'll want the norm squared. Save the sj::math::sqrt and a multiplication.
        ...
        constexpr void set_rotation (const vec<F>& axis, const F& angle) noexcept
        {
            ...
            F cosHalf = sj::math::cos(halfangle);
            F sinHalf = sj::math::sin(halfangle);
            vec<F> ax = axis;
            ax.renormalize();
         ...
         */
        template <typename V>
        requires (std::is_same_v<V, std::array<F, 3>> || std::is_same_v<V, sj::vec<F, 3>>)
         ...
         */
        constexpr sj::vec<F, 4> axis_angle() const noexcept
        {
            sj::vec<F, 4> aa{F{0}};
            aa[3] =  2 * sj::math::acos (this->w);
            F s = sj::math::sqrt (F{1} - this->w * this->w);
            aa[0] = this->x / s;
            aa[1] = this->y / s;
            aa[2] = this->z / s;
            return aa;
