...
namespace sj
{
    ...
    struct histo
    {
         ...
         *
         * \tparam Container A container (std::vector, std::array, sj::vvec, etc) of data values
         * of type H
         *
        ...
        {
            sj::range<H> r { std::numeric_limits<H>::max(), std::numeric_limits<H>::max() };
            this->init (data, n, r);
        }
         ...
         */
        template < template <typename, typename> typename Container, typename Allocator=std::allocator<H> >
        histo (const Container<H, Allocator>& data, std::size_t n, const sj::range<H>& manual_datarange)
        {
         ...
         */
        template < template <typename, typename> typename Container, typename Allocator=std::allocator<H> >
        void init (const Container<H, Allocator>& data, std::size_t n, const sj::range<H>& manual_datarange)
        {
            ...
            if (manual_datarange.min == std::numeric_limits<H>::max()
                ...
                this->datarange = sj::range<H>::get_from (data);
            } else {
                // Check manual_datarange
                sj::range<H> actual_datarange = sj::range<H>::get_from (data);
                if (!manual_datarange.contains (actual_datarange)) {
                    throw std::runtime_error ("sj::histo: Make sure the manual_datarange is *larger* than the data's own datarange");
                }
                this->datarange = manual_datarange;
            }
            if (this->datarange.span() == H{0}) {
                throw std::runtime_error ("sj::histo: range span is 0, can't make a histogram");
            }
            ...
            for (auto datum : data) {
                ...
                if (std::abs(bin_proportion - T{1}) < std::numeric_limits<T>::epsilon()) {
                    ...
                    throw std::runtime_error ("sj::histo: shouldn't see proportion > 1");
