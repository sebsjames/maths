 ...
 * \file
 *
 * This file contains the definition for sj::Grid, a simple Cartesian Grid
 * class. This is the runtime-modifiable version of sj::Gridct<>
 *
 * \author Seb James
 * \date February 2024
 */
...
namespace sj
{
     ...
     *
     * Use this class instead of sj::Gridct<> if you need to change the grid parameters
    ...
    {
    private:
        // Members that are template arguments in sj::grid. Set only via constructor or setters

        //! w Number of elements that the grid is wide
        I w = I{1};
        //! h Number of elements that the grid is high
        I h = I{1};
        //! dx A two element sj::vec providing the horizontal distance between
        ...
        gridorder order = sj::gridorder::bottomleft_to_topright;
    ...
    public:
        ...
        //! Setter for dx
        void set_dx (const sj::vec<C, 2> _dx) { this->dx = _dx; this->init(); }
        //! Setter for g_offset
        void set_offset (const sj::vec<C, 2> _offset) { this->offset = _offset; this->init(); }

        //! Setter for most of the grid parameters to be carried out all in one function
        void set_grid_params (const sj::vec<I, 2> dims,
                              const sj::vec<C, 2> spacing,
                              const sj::vec<C, 2> grid_offset)
        {
            this->w = dims[0];
            this->h = dims[I{1}];
            this->dx = spacing;
        ...
        sj::vec<I, 2> get_dims() const { return sj::vec<I, 2>{this->w, this->h}; }
        sj::vec<C, 2> get_dx() const { return this->dx; }
        sj::vec<C, 2> get_offset() const { return this->offset; }
        ...
        static sj::vec<I, 2> suggest_dims (const I num_elements, const bool allow_extra = false)
        {
            sj::vec<I, 2> w_h = { std::numeric_limits<I>::max(), std::numeric_limits<I>::max() };
            ...
            if (!factors.empty()) {
                sj::vvec<C> factors_minus_sqrt = factors.template as<C>() - std::sqrt(static_cast<C>(num_elements));
                ...
                if (num_elements <= one_by_most) {
                    ...
                    for (I j = num_elements + I{1}; j < num_elements + possible_additional; ++j) {
                        w_h = sj::grid<I, C>::suggest_dims (j, false);
                        if (w_h != sj::vec<I, 2>{ std::numeric_limits<I>::max(), std::numeric_limits<I>::max() }) {
                            // success!
                            break;
                        }
                    }
        ...
        bool rowmaj() const
        {
            return (this->order == sj::gridorder::bottomleft_to_topright
                    || this->order == sj::gridorder::topleft_to_bottomright) ? true : false;
        }
        ...
        {
            std::stringstream ss;
            if (order == sj::gridorder::bottomleft_to_topright) {
            ...
            } else if (order == sj::gridorder::topleft_to_bottomright) {
            ...
            } else if (order == sj::gridorder::bottomleft_to_topright_colmaj) {
            ...
            } else if (order == sj::gridorder::topleft_to_bottomright_colmaj) {
        ...
        grid (const I _w, const I _h,
              ...
              const gridorder _order = sj::gridorder::bottomleft_to_topright)
            : w(_w)
            , h(_h)
            , dx(_dx)
            , offset(_offset)
        ...
        sj::vec<C, 2> operator[] (const I index) const
        {
            return index >= this->n() ? sj::vec<C, 2>{std::numeric_limits<C>::max(), std::numeric_limits<C>::max()} : this->v_c[index];
        }
        ...
        I index_lookup (const sj::vec<C, 2>& _coord) const
        {
            I index = I{0};
            sj::vec<C, 2> xyf = ((_coord - this->offset) / this->dx);
            xyf[0] = std::round (xyf[0]); // theres no vec::round() function at the time of writing
            xyf[1] = std::round (xyf[1]);

            if (order == sj::gridorder::topleft_to_bottomright
                || order == sj::gridorder::topleft_to_bottomright_colmaj) {
            ...
            sj::vec<I, 2> xyi = xyf.template as<I>();

            if (order == sj::gridorder::bottomleft_to_topright
                || order == sj::gridorder::topleft_to_bottomright) {
        ...
        sj::vec<C, 2> coord_lookup (const I index) const
        {
            return index >= this->n() ? sj::vec<C, 2>{std::numeric_limits<C>::max(), std::numeric_limits<C>::max()} : this->v_c[index];
        }

        //! Compute and return the coordinate with the given index
        sj::vec<C, 2> coord (const I index) const
        {
            if (index >= this->n()) { return sj::vec<C, 2>{std::numeric_limits<C>::max(), std::numeric_limits<C>::max()}; }
            sj::vec<C, 2> loc = this->offset;
            if (order == sj::gridorder::bottomleft_to_topright) {
        ...
        sj::vec<C, 2> coord_ne (const I index) const
        {
            I idx = index_ne (index);
            if (idx < this->n()) { return (*this)[idx]; }
            return sj::vec<C, 2>{std::numeric_limits<C>::max(), std::numeric_limits<C>::max()};
        }
        ...
        sj::vec<C, 2> coord_nw (const I index) const
        {
        ...
        sj::vec<C, 2> coord_nn (const I index) const
        {
        ...
        sj::vec<C, 2> coord_ns (const I index) const
        {
        ...
        sj::vec<C, 2> coord_nne (const I index) const
        {
            I idx = this->index_nne (index);
            return idx < this->n() ? (*this)[idx] : sj::vec<C, 2>{std::numeric_limits<C>::max(), std::numeric_limits<C>::max()};
        }
        ...
        sj::vec<C, 2> coord_nnw (const I index) const
        {
        ...
        sj::vec<C, 2> coord_nse (const I index) const
        {
        ...
        sj::vec<C, 2> coord_nsw (const I index) const
        {
        ...
        sj::vec<C, 4> extents() const { return sj::vec<C, 4>{ xmin(), xmax(), ymin(), ymax() }; }

        //! Return the coordinates of the centre of the grid
        sj::vec<C, 2> centre() const { return sj::vec<C, 2>{ xmax() - xmin(), ymax() - ymin() } * C{0.5}; }

        //! Return the y coordinates for each row in the grid
        sj::vvec<C> get_abscissae() const
        {
            sj::vvec<C> abscissae (w, C{0});
        ...
        sj::vvec<C> get_ordinates() const
        {
            sj::vvec<C> ordinates (h, C{0});
            if (order == gridorder::bottomleft_to_topright || order == gridorder::topleft_to_bottomright) {
         ...
         */
        I shift_index (const I ind, const sj::vec<int, 2> delta) const
        {
         ...
         */
        sj::vvec<float> resample_image (const sj::vvec<float>& image_data,
                                           const unsigned int image_pixelwidth,
                                           const sj::vec<float, 2>& image_scale,
                                           const sj::vec<float, 2>& image_offset) const
        {
            if (this->order != sj::gridorder::bottomleft_to_topright) {
                throw std::runtime_error ("grid::resample_image: resampling assumes image has sj::gridorder::bottomleft_to_topright, so your grid should, too.");
            }

            sj::vvec<float> expr_resampled(this->w * this->h, 0.0f);
            ...
            sj::vec<float, 2> image_dims = { 1.0f, 0.0f };
            ...
            sj::vec<float, 2> dist_per_pix = image_dims / (image_pixelsz - 1u);
...
#pragma omp parallel for // parallel on this outer loop gives best result (5.8 s vs 7 s)
            for (typename std::vector<float>::size_type xi = 0u; xi < this->v_c.size(); ++xi) {
                ...
                for (unsigned int i = 0; i < csz; ++i) {
                    ...
                    sj::vec<float, 2> posn = (dist_per_pix * idx) + image_offset;
                    // Distance from input pixel to output pixel:
                    sj::vec<float, 2> _v_c = this->v_c[xi] - posn;
         ...
         */
        void indices_in_radius (const sj::vec<C,2> loc,
                                const C radius,
                                sj::vvec<I>& inds_in_radius) const
        {
            sj::vvec<I> inds_in_circle;
            sj::vvec<I> inds_in_previous_circle;
            std::set<I> seen;
         ...
         */
        void find_nearest_neighbours (const sj::vvec<I>& inds, sj::vvec<I>& neighbour_inds) const
        {
