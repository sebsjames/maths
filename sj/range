 ...
 * which to return minmax values for use in sj::vec and sj::vvec. An option would
...
namespace sj
{
    ...
    struct range
    {
        ...
        constexpr range (const sj::range_init _range_init) noexcept
        {
            if (_range_init == sj::range_init::for_search) { this->search_init(); }
        }
        ...
        {
            return (sj::math::abs(this->min - static_cast<T>(rhs.min)) < std::numeric_limits<T>::epsilon()
                    && sj::math::abs(this->max - static_cast<T>(rhs.max)) < std::numeric_limits<T>::epsilon());
        }
        ...
        {
            if constexpr (sj::number_type<T>::value == 2) { // range is complex
            ...
            } else if constexpr (sj::number_type<T>::value == 1) { // range is scalar
                this->min = std::numeric_limits<T>::max();
                this->max = std::numeric_limits<T>::lowest();
            } else {
                []<bool flag = false>() { static_assert(flag, "sj::range::search_init does not support this type"); }();
            }
        }
        ...
        constexpr bool update (const T& d) noexcept
        {
            ...
            if constexpr (sj::number_type<T>::value == 2) { // range is complex
                ...
                []<bool flag = false>() { static_assert(flag, "sj::range::update does not support this type"); }();
            }
            return changed;
        }
        ...
        constexpr bool includes (const T& v) const noexcept
        {
            if constexpr (sj::number_type<T>::value == 2) { // range is complex
                ...
                []<bool flag = false>() { static_assert(flag, "sj::range::includes does not support this type"); }();
            }
        }

        // If the range other 'fits inside' this range, then this range contains (or encompasses) the range other.
        constexpr bool contains (const sj::range<T>& other) const noexcept
        {
            if constexpr (sj::number_type<T>::value == 2) { // range is complex
                ...
                []<bool flag = false>() { static_assert(flag, "sj::range::contains does not support this type"); }();
            }
        }
        ...
        static sj::range<typename C::value_type> get_from (const C& values)
        {
            using Tc = typename C::value_type;

            if constexpr (sj::number_type<T>::value == 0) { // vector elements
                ...
                using T_el = std::remove_reference_t<decltype(*std::begin(std::declval<Tc&>()))>;
                sj::range<Tc> r (std::numeric_limits<Tc>::max(), std::numeric_limits<Tc>::min());
                T_el maxlen = 0;
            ...
            } else if constexpr (sj::number_type<T>::value == 2) { // complex elements
                ...
                sj::range<Tc> r ({std::numeric_limits<T_el>::max(), std::numeric_limits<T_el>::max() }, T{0, 0});
                for (auto v : values) {
            ...
            } else if constexpr (sj::number_type<T>::value == 1) { // scalar elements
                sj::range<Tc> r (std::numeric_limits<T>::max(), std::numeric_limits<T>::lowest());
                for (auto v : values) {
