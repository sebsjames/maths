/*!
 * \file
 * \brief An N dimensional vector class template, sj::vvec which derives from std::vector.
 *
 * \author Seb James
 * \date May 2020
 */
...
namespace sj
{
     ...
     * on.
     *
     * This class is better for writing neural networks than sj::vec, whose size
    ...
    struct vvec : public std::vector<S, Al>
    {
         ...
         * sj::vvec, sj::vec etc and even with std::set (though not with std::map).
         *
         ...
         */
        template <typename C>
        requires (sj::is_copyable_container<C>::value && !std::is_same<std::decay_t<C>, S>::value)
        ...
        template <typename Sy=S> requires (!sj::is_copyable_container<Sy>::value)
         ...
         * or sj::vec<T, N>
         */
        template <typename C> requires (sj::is_copyable_container<C>::value)
        ...
        void shuffle()
        {
            std::random_device rd; // we have access to these via #include <sj/random.h>
        ...
        void rotate_pairs()
        {
            std::size_t N = this->size();
            if (N%2!=0) {
                throw std::runtime_error ("vvec size must be even to call sj::vvec::rotate_pairs");
            }
            S tmp_el = S{0};
        ...
        bool checkunit() const noexcept
        {
            ...
            if (std::abs(metric) > sj::vvec<S>::unitThresh()) {
                return false;
            }
            return true;
        }
        ...
        template <typename Sy=S> requires sj::is_copyable_fixedsize<std::decay_t<Sy>>::value
        ...
        //! (sj::vvec<sj::vec<T, N>>). Note that the enclosed non-scalar thing must have
        //! function length_lessthan (as sj::vec does).
        template <typename Sy=S> requires sj::is_copyable_fixedsize<std::decay_t<Sy>>::value
        S max() const noexcept
        {
        ...
        //! vvec of vecs version of argmax returns the index of the maximum length sj::vec
        template <typename Sy=S> requires sj::is_copyable_fixedsize<std::decay_t<Sy>>::value
        std::size_t argmax() const noexcept
        {
            auto themax = std::max_element (this->begin(), this->end(), [](S a, S b){return a.length_lessthan(b);});
        ...
        sj::range<S> minmax() const { return this->range<test_for_nans>(); }
        ...
        sj::range<S> range() const
        {
            sj::range<S> r;
            if constexpr (test_for_nans) {
                if (this->has_nan()) {
                    // Deal with non-numbers by removing them
                    sj::vvec<S> sans_nans = this->prune_nan();
        ...
        // The extent if S is scalar is just the same as range; a sj::range<S> is returned.
        template <typename Sy=S> requires std::is_scalar_v<std::decay_t<Sy>>
        sj::range<S> extent() const { return this->range(); }

        /*!
         * For a vvec of vecs/arrays extent (that is, S is a sj::vec or std::array),
         ...
         * (sj::is_copyable_fixedsize).
         */
        template <typename Sy=S> requires sj::is_copyable_fixedsize<std::decay_t<Sy>>::value
        sj::range<S> extent() const noexcept
        {
            constexpr S s = {};                  // A dummy variable whose size is stored as sz
            constexpr std::size_t sz = s.size(); // Should work for S=std::array or sj::vec
        ...
        template <typename Sy=S>
        sj::vvec<std::size_t> find (const Sy& val) const
        {
            sj::vvec<std::size_t> indices;
            std::size_t i = 0;
            for (i = 0; i < this->size(); i++) {
        ...
        //
        // Use something like this as a compare function when storing sj::vvec in
        // a std::set:
        //
         ...
         */
        template <typename Sy=S>
        requires (std::is_scalar_v<std::decay_t<Sy>> || sj::is_copyable_fixedsize<std::decay_t<Sy>>::value)
