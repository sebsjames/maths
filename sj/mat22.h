...
namespace sj {
     ...
     * stored in mat22::mat, an array (sj::vec) of 4 floating point numbers.
     *
    ...
    public:
         ...
         * sj::mat44 (which is OpenGL friendly).
         */
        alignas(sj::vec<F, 4>) sj::vec<F, 4> mat;
        ...
        static std::string str (const sj::vec<F, 4>& arr) noexcept
        {
            std::stringstream ss;
            ss <<"[ "<< arr[0]<<" , "<<arr[2]<<" ;\n";
            ss <<"  "<< arr[1]<<" , "<<arr[3]<<" ]\n";
        ...
        sj::vec<F, 2> row (unsigned int idx) const noexcept // unsigned char would be enough capacity!
        {
            sj::vec<F, 2> r = {F{0}, F{0}};
            if (idx > 1U) { return r; }
            r[0] = this->mat[idx];
            r[1] = this->mat[idx+2];
            return r;
        ...
        sj::vec<F, 2> col (unsigned int idx) const noexcept
        {
            sj::vec<F, 3> c = {F{0}, F{0}};
            if (idx > 1U) { return c; }
            idx *= 2U;
            c[0] = this->mat[idx];
            c[1] = this->mat[++idx];
        ...
        sj::vec<F, 4> transpose (const sj::vec<F, 4>& matrx) const noexcept
        {
            sj::vec<F, 4> tposed;
            tposed[0] = matrx[0];
            tposed[1] = matrx[2];
        ...
        static F determinant (sj::vec<F, 4> cm) noexcept
        {
        ...
        sj::vec<F, 4> adjugate() const noexcept
        {
            sj::vec<F, 4> adj = { this->mat[3], -this->mat[1], -this->mat[2], this->mat[0] };
            return adj;
        }
        ...
        void operator*= (const sj::vec<F, 4>& m2) noexcept
        {
            sj::vec<F, 4> result;
            // Top row
        ...
        //! Return this->mat * m2
        mat22<F> operator* (const sj::vec<F, 4>& m2) const noexcept
        {
            mat22<F> result;
            // Top row
        ...
        sj::vec<F, 2> operator* (const sj::vec<F, 2>& v1) const noexcept
        {
            sj::vec<F, 2> v = {
