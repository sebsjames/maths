...
namespace sj
{
    ...
    struct bezcurvepath
    {
        ...
        sj::vec<F, 2> initialCoordinate = { F{0}, F{0} };
        ...
        F getEndToEnd() const
        {
            ...
            sj::vec<F, 2> cend = this->curves.back().getFinalPointScaled();
            return (cend - initialCoordinate).length();
        }

        //! Compute & return the centroid of the passed in set of positions.
        static sj::vec<F, 2> getCentroid (const std::vector<bezcoord<F>>& points)
        {
            sj::vec<F, 2> c = {F{0}, F{0}};
