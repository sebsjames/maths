...
namespace sj
{
    ...
    public:
        ...
        //! Load the data for this hex from a sj::hdfdata file
         ...
         */
        template <typename F>
        float distanceFrom (const sj::vec<F, 2> cartesianPoint) const
        {
         ...
         */
        float distanceFrom (const sj::bezcoord<float>& cartesianPoint) const
        {
         ...
         * the variables recorded for this hex. It's used in sj::hexgrid to populate
        ...
        float y = 0.0f;
        // Getter for (x,y) as a sj::vec
        sj::vec<float, 2> x_y() { return sj::vec<float, 2>({this->x, this->y}); }
        ...
        float getLR() const
        {
            float lr = this->d * sj::mathconst<float>::one_over_root_3;
            return lr;
        }

        //! Compute and return the area of the hex
        float getArea() const { return (this->d * this->d * sj::mathconst<float>::root_3_over_2); }

        //! The vertical distance between hex centres on adjacent rows.
        float getV() const
        {
            float v = this->d * sj::mathconst<float>::root_3_over_2;
            return v;
        }
        ...
        {
            float v = this->d * sj::mathconst<float>::one_over_2_root_3;
            return v;
        }
        ...
        float getTwoV() const
        {
            float tv = this->d * sj::mathconst<float>::sqrt_of_3;
            return tv;
        }
         ...
         */
        sj::vec<float, 2> get_vertex_coord (unsigned short ni) const
        {
            sj::vec<float, 2> rtn = { 0.0f, 0.0f };
            switch (ni) {
            case HEX_VERTEX_POS_NE:
            {
         ...
         * argument.
         */
        sj::vec<float, 2> get_vertex_coord (unsigned int ni) const
        {
            sj::vec<float, 2> rtn = { -2.0f, -2.0f };
            if (ni > 5) { return rtn; }
         ...
         * argument.
         */
        sj::vec<float, 2> get_vertex_coord (int ni) const
        {
            sj::vec<float, 2> rtn = { -3.0f, -3.0f };
            if (ni > 5) {
                rtn[0] = -4.0f;
                return rtn;
            }
         ...
         */
        template <typename F>
        bool compare_vertex_coord (int ni, sj::vec<F, 2>& coord) const
        {
            sj::vec<float, 2> vc = this->get_vertex_coord (ni);
        ...
        template <typename F>
        bool contains_vertex (sj::vec<F, 2>& coord) const
        {
         ...
         */
        template <typename F>
        bool compare_coord (sj::vec<F, 2>& coord) const
        {
