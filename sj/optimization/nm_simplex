...
namespace sj::optimization
{
     ...
     * dimensionality of the search, and using sj::vec<T, N+1> as the type for
     * vertices.
     */
    template <typename T>
    class nm_simplex
    ...
    public:
        ...
        sj::vvec<unsigned int> vertex_order;
    ...
    public:
        ...
        //! each.
        nm_simplex (const sj::vvec<sj::vvec<T>>& initial_vertices)
        {
        ...
        nm_simplex (const sj::vec<T, 2>& v0,
                    const sj::vec<T, 2>& v1, const sj::vec<T, 2>& v2)
        {
            this->n = 2;
            this->allocate();
        ...
        void reset (const sj::vvec<sj::vvec<T>>& initial_vertices)
        {
            ...
            this->allocate();
            unsigned int i = 0U;
            for (sj::vvec<T>& v : this->vertices) { v = initial_vertices[i++]; }
            this->state = nm_simplex_state::NeedToComputeThenOrder;
        }
         ...
         *
         * auto myobj = [](const sj::vvec<T>& point) { return T{your_implementation_result}; };
         * simp.objective = myobj;
         */
        std::function<T(const sj::vvec<T>& point)> objective = {};
        ...
        sj::vvec<T> best_vertex() { return this->vertices[this->vertex_order[0]]; }
        ...
        void order()
        {
            ...
            if (this->downhill) {
                // Best is lowest
                sj::algo::bubble_sort_lo_to_hi<T> (this->values, this->vertex_order);
            } else {
                sj::algo::bubble_sort_hi_to_lo<T> (this->values, this->vertex_order);
            }
    ...
    private:
        ...
        void allocate()
        {
            ...
            for (sj::vvec<T>& v : this->vertices) { v.resize (this->n, T{0}); }
