...
#include <sj/vvec.h>
#include <sj/hex.h>
#include <sj/bezcurvepath.h> // requires arma
#include <sj/bezcoord.h>
...
namespace sj {
    ...
    public:
        ...
        void populate_d_neighbours()
        {
            ...
            std::list<sj::hex>::iterator hi = this->hexen.begin();
        ...
        {
            sj::hdfdata hgdata (path, std::ios::out | std::ios::trunc);
            hgdata.add_val ("/d", d);
            hgdata.add_val ("/v", v);
            ...
            std::list<sj::hex>::const_iterator h = this->hexen.begin();
        ...
        {
            sj::hdfdata hgdata (path, std::ios::in);
        ...
        {
            this->v = this->d * sj::mathconst<float>::root_3_over_2;
            this->init();
        }
         ...
         */
        sj::vec<float, 2> computeCentroid (const std::list<hex>& phexes)
        {
            sj::vec<float, 2> centroid = {0,0};
            for (auto h : phexes) {
                centroid[0] += h.x;
                centroid[1] += h.y;
            }
         ...
         * pos.
         */
        std::list<hex>::iterator findHexNearest (const sj::vec<float, 2>& pos)
        {
            std::list<sj::hex>::iterator nearest = this->hexen.end();
            std::list<sj::hex>::iterator hi = this->hexen.begin();
        ...
        std::list<hex>::iterator findhexAt (const sj::vec<int, 3>& rgbpos)
        {
            std::list<sj::hex>::iterator hi = this->hexen.begin(); // First hex in hexen is always 0,0,0
        ...
        void setBoundary (const std::list<hex>& phexes)
        {
            ...
            std::list<sj::hex>::iterator bpoint = this->hexen.begin();
            std::list<sj::hex>::iterator bpi = this->hexen.begin();
            while (bpi != this->hexen.end()) {
                std::list<sj::hex>::const_iterator ppi = phexes.begin();
            ...
            std::list<sj::hex>::iterator hi = bpoint;
         ...
         * this boundary. Finishes up by calling sj::hexgrid::discardOutside.
        ...
        void setBoundary (const bezcurvepath<float>& p, bool loffset = true)
        {
            ...
            if (!this->boundary.isNull()) {
                ...
                std::vector<sj::bezcoord<float>> bpoints = this->boundary.getPoints();
                this->setBoundary (bpoints, loffset);
            }
        }

        /*!
         * This sets a boundary, just as sj::hexgrid::setBoundary(const
         * sj::bezcurvepath<float> p, bool offset) does but WITHOUT discarding hexes
        ...
        {
            this->boundaryCentroid = sj::bezcurvepath<float>::getCentroid (bpoints);
            ...
            std::list<sj::hex>::iterator nearbyBoundaryPoint = this->hexen.begin(); // i.e the hex at 0,0
            bpi = bpoints.begin();
            while (bpi != bpoints.end()) {
            ...
            {
                std::set<unsigned int> seen;
                std::list<sj::hex>::iterator hi = nearbyBoundaryPoint;
         ...
         * sj::hexgrid::setBoundary(vector<sj::bezcoord<float>& bpoints, bool offset)
        ...
        void setBoundaryOnOuterEdge()
        {
            ...
            // Check that the boundary is contiguous.
            std::set<unsigned int> seen;
            std::list<sj::hex>::iterator hi = bpi;
        ...
        {
            std::list<sj::hex> bhexen_concrete;
        ...
        std::vector<bezcoord<float>> rectangleCompute (const float x, const float y,
                                                       const sj::vec<float, 2> c = {0.0f, 0.0f})
        {
            std::vector<sj::bezcoord<float>> bpoints;

            // Go to bottom left first
            sj::vec<float, 2> bleft = {-0.5f * x, -0.5f * y};
            sj::vec<float, 2> tright = {0.5f * x, 0.5f * y};
            bleft += c;
            tright += c;
            ...
            for (float x1 = bleft[0]; x1 < tright[0]; x1 += step) {
                sj::bezcoord<float> b(sj::vec<float, 2>{x1, bleft[1]});
                bpoints.push_back (b);
            }
            // Right
            for (float y1 = bleft[1]; y1 < tright[1]; y1 += step) {
                sj::bezcoord<float> b(sj::vec<float, 2>{tright[0], y1});
                bpoints.push_back (b);
            }
            // Top
            for (float x1 = tright[0]; x1 >= bleft[0]; x1 -= step) {
                sj::bezcoord<float> b(sj::vec<float, 2>{x1, tright[1]});
                bpoints.push_back (b);
            }
            // Left
            for (float y1 = tright[1]; y1 >= bleft[1]; y1 -= step) {
                sj::bezcoord<float> b(sj::vec<float, 2>{bleft[0], y1});
                bpoints.push_back (b);
            }
                                                           ...
                                                           const sj::vec<float, 2> c = {0.0f, 0.0f})
        {
            std::vector<sj::bezcoord<float>> bpoints;
            // Go to bottom left first
            sj::vec<float, 2> xy = {-(rw * this->d + gsw * this->d/2.0f), -gsw * this->v};
            xy += c;

            // 'Draw' bottom
            for (int i = 0; i < 2*(rw+re); ++i) {
                sj::bezcoord<float> b(xy);
                bpoints.push_back (b);
                xy[0] += this->d/2.0f;
            }
            // Right
                                                     ...
                                                     const sj::vec<float, 2> c = {0.0f, 0.0f})
        {
            ...
            for (double phi = 0.0; phi < sj::mathconst<double>::two_pi; phi+=delta_phi) {
                sj::vec<float, 2> xy_pt = {
                    static_cast<float>(a * std::cos (phi)),
                    static_cast<float>(b * std::sin (phi))
                };
                xy_pt += c;
                sj::bezcoord<float> b(xy_pt);
                bpoints.push_back (b);
            }
        ...
        float ellipsePerimeter (const float a, const float b)
        {
            ...
            double p = sj::mathconst<double>::pi * apb * sum;
        ...
        void setEllipticalBoundary (const float a, const float b,
                                    const sj::vec<float, 2> c = {0.0f, 0.0f}, bool offset=true)
        {
            std::vector<sj::bezcoord<float>> bpoints = ellipseCompute (a, b, c);
        ...
        void setCircularBoundary (const float a,
                                  const sj::vec<float, 2> c = {0.0f, 0.0f}, bool offset=true)
        {
            std::vector<sj::bezcoord<float>> bpoints = ellipseCompute (a, a, c);
            this->setBoundary (bpoints, offset);
        }
        ...
        void setRectangularBoundary (const float x, const float y,
                                     const sj::vec<float, 2> c = {0.0f, 0.0f}, bool offset=true)
        {
            std::vector<sj::bezcoord<float>> bpoints = rectangleCompute (x, y, c);
            this->setBoundary (bpoints, offset);
        }
        ...
        void setParallelogramBoundary (const int r, const int g,
                                       const sj::vec<float, 2> c = {0.0f, 0.0f}, bool offset=true)
        {
            std::vector<sj::bezcoord<float>> bpoints = parallelogramCompute (r, g, r, g, c);
            this->setBoundary (bpoints, offset);
        }
         ...
         * "long radius".
         */
        float getLR() const { return (this->d/sj::mathconst<float>::sqrt_of_3); }

        /*!
         * The vertical distance from the centre of the hex to the "north east" vertex
         * of the hex.
         */
        float getVtoNE() const { return (this->d/(2.0f*sj::mathconst<float>::sqrt_of_3)); }
         ...
         */
        float getHexArea() const { return (this->d * this->d * sj::mathconst<float>::root_3_over_2); }
        ...
        {
            std::list<sj::hex>::iterator h = this->hexen.begin();
            while (h != this->hexen.end()) {
                if (h->testFlags(HEX_IS_BOUNDARY) == true) {
                    ...
                    if (h->testFlags(HEX_INSIDE_BOUNDARY) == false) {
                        ...
                        std::list<sj::hex>::iterator bh = this->hexen.begin();
         ...
         * region. The idea is that client code will then use the vector of sj::hex* to work
         ...
         */
        std::vector<std::list<hex>::iterator> getRegion (bezcurvepath<float>& p, sj::vec<float, 2>& regionCentroid,
                                                         bool applyOriginalBoundaryCentroid = true)
        {
            p.computePoints (this->d/2.0f, true);
            std::vector<sj::bezcoord<float>> bpoints = p.getPoints();
         ...
         */
        std::vector<std::list<hex>::iterator> getRegion (std::vector<bezcoord<float>>& bpoints, sj::vec<float, 2>& regionCentroid,
                                                         bool applyOriginalBoundaryCentroid = true)
        {
            ...
            regionCentroid = sj::bezcurvepath<float>::getCentroid (bpoints);

            // A return object
            std::vector<std::list<sj::hex>::iterator> theRegion;
            ...
            std::list<sj::hex>::iterator nearbyRegionBoundaryPoint = this->hexen.begin(); // i.e the hex at 0,0
            typename std::vector<sj::bezcoord<float>>::iterator bpi = bpoints.begin();
            while (bpi != bpoints.end()) {
            ...
            {
                std::set<unsigned int> seen;
                std::list<sj::hex>::iterator hi = nearbyRegionBoundaryPoint;
            ...
            std::list<sj::hex>::iterator insideRegionhex = this->findHexNearest (regionCentroid);
         ...
         */
        sj::vvec<float> resampleImage (const sj::vvec<float>& image_data,
                                          const unsigned int image_pixelwidth,
                                          const sj::vec<float, 2>& image_scale,
                                          const sj::vec<float, 2>& image_offset)
        {
            unsigned int csz = image_data.size();
            sj::vec<unsigned int, 2> image_pixelsz = {image_pixelwidth, csz / image_pixelwidth};

            // Return data object for the resampled result
            sj::vvec<float> expr_resampled(this->num(), 0.0f);
            ...
            sj::vec<float, 2> dist_per_pix = image_scale / (image_pixelsz[0] - 1u);
            // This is an offset to centre the image wrt to the hexgrid
            sj::vec<float, 2> input_centering_offset = dist_per_pix * image_pixelsz * 0.5f;
            // Parameters for the Gaussian computation
            sj::vec<float, 2> params = 1.0f / (2.0f * dist_per_pix * dist_per_pix);
            sj::vec<float, 2> threesig = 3.0f * dist_per_pix;
...
#pragma omp parallel for // parallel on this outer loop gives best result (5.8 s vs 7 s)
            for (typename std::vector<float>::size_type xi = 0u; xi < this->d_x.size(); ++xi) {
                ...
                for (unsigned int i = 0; i < csz; ++i) {
                    ...
                    sj::vec<unsigned int, 2> idx = {(i % image_pixelsz[0]), (i / image_pixelsz[0])};
                    // Get the coordinates of the pixel at index i (in hexgrid units):
                    sj::vec<float, 2> posn = (dist_per_pix * idx) - input_centering_offset + image_offset;
        ...
        template <typename T>
        bool shiftdata (sj::vvec<T>& image_data, const sj::vec<float, 2>& dx)
        {
            unsigned int csz = image_data.size();
            sj::vvec<T> shifted(csz, T{0});
            ...
            vec<float, 2> rg = {
                (1.0f/this->d) * (dx[0] - dx[1] * sj::mathconst<float>::one_over_root_3),
                (1.0f/this->d) * (dx[1] * sj::mathconst<float>::two_over_root_3)
            };
            ...
            sw_loc = { (-d*0.5f), (-d*sj::mathconst<float>::one_over_2_root_3) };
            nw_loc = { (-d*0.5f), ( d*sj::mathconst<float>::one_over_2_root_3) };
            ne_loc = {  (d*0.5f), ( d*sj::mathconst<float>::one_over_2_root_3) };
            se_loc = {  (d*0.5f), (-d*sj::mathconst<float>::one_over_2_root_3) };
            n_loc =  {   0.0f   , ( d*sj::mathconst<float>::one_over_root_3)   };
            s_loc =  {   0.0f   , (-d*sj::mathconst<float>::one_over_root_3)   };
                                       ...
                                       const vec<float, 2> _p2, const vec<float, 2> _q2)
        {
            ...
            if (rxs == 0.0f && q_m_pxr == 0.0f) {
                ...
                // Get normal to p2-q2
                sj::mat22<float> rot90;
                rot90.rotate (sj::mathconst<float>::pi_over_2);
        ...
        vec<float, 19> compute_hex_overlap (vec<float, 2> shift)
        {
            ...
            sj::vec<float, 2> hv_ne = { sj::mathconst<float>::root_3_over_2 * lr, 0.5f * lr };
            sj::vec<float, 2> hv_n =  { 0, lr };
            sj::vec<float, 2> hv_nw = {-sj::mathconst<float>::root_3_over_2 * lr, 0.5f * lr};
            sj::vec<float, 2> hv_sw = {-sj::mathconst<float>::root_3_over_2 * lr, -0.5f * lr};
            sj::vec<float, 2> hv_s =  { 0, -lr };
            sj::vec<float, 2> hv_se = { sj::mathconst<float>::root_3_over_2 * lr, -0.5f * lr};
            ...
            if (!isct1.has_nan()) {
                ...
                overlap = this->compute_overlap_colinear3 (5);
                       ...
                       || (std::abs(shift.angle()    -   sj::mathconst<float>::pi_over_6) <= anglethreshold) // rest test for shift angle
                       || (std::abs(shift.angle()    +   sj::mathconst<float>::pi_over_6) <= anglethreshold) // rest test for shift angle
                       || (std::abs(shift.angle() - 3.0f*sj::mathconst<float>::pi_over_6) <= anglethreshold)
                       || (std::abs(shift.angle() + 3.0f*sj::mathconst<float>::pi_over_6) <= anglethreshold)
                       || (std::abs(shift.angle() - 5.0f*sj::mathconst<float>::pi_over_6) <= anglethreshold)
                       || (std::abs(shift.angle() + 5.0f*sj::mathconst<float>::pi_over_6) <= anglethreshold)) {
            ...
            if constexpr (debug_hexshift) {
                ...
                std::cout << "overlap sum: " << overlap.sum() << "\n";
            }
            if (overlap.sum() == 0.0f) {
        ...
        vec<float, 19> compute_overlap_colinear()
        {
            ...
            if (minpp < 2.0f*lr) {
                ...
                if (minpp >= lr) {
                    ...
                    float pw = (2.0f * lr - minpp) * sj::mathconst<float>::root_3_over_2;
        ...
        vec<float, 19> compute_overlap_colinear2 (const unsigned int _rotation)
        {
            ...
            vec<float, 19> rtn;
            rtn.zero();
            sj::mat22<float> rotn = this->setup_hexoverlap_geometry (_rotation);
            unit_60 = rotn * sj::vec<float, 2>({ 0.5f, sj::mathconst<float>::root_3_over_2 });
        ...
        vec<float, 19> compute_overlap_colinear3 (const unsigned int _rotation)
        {
            ...
            rtn.zero();
            sj::mat22<float> rotn = this->setup_hexoverlap_geometry (_rotation);
            unit_120 = rotn * sj::vec<float, 2>({ -0.5f, sj::mathconst<float>::root_3_over_2});
        ...
        sj::mat22<float> setup_hexoverlap_geometry (const unsigned int _rotation)
        {
            float lr = this->getLR();
            sj::mat22<float> rotn;
            // hexvectors from centre to points:
            ...
            case 1:
            {
                ...
                rotn.rotate (sj::mathconst<float>::pi_over_3);
                p5 = n_sft;
                p6 = n_loc + hv_n + hv_nw;
            ...
            case 2:
            {
                ...
                rotn.rotate (sj::mathconst<float>::two_pi_over_3);
                p5 = nw_sft;
            ...
            case 3:
            {
                ...
                rotn.rotate (sj::mathconst<float>::pi);
                p5 = sw_sft;
            ...
            case 4:
            {
                ...
                rotn.rotate (sj::mathconst<float>::four_pi_over_3);
                p5 = s_sft;
                p6 = s_loc + hv_s + hv_se;
            ...
            case 5:
            {
                ...
                rotn.rotate (sj::mathconst<float>::five_pi_over_3);
                p5 = se_sft;
        ...
        vec<float, 19> compute_overlap_far2 (const unsigned int _rotation)
        {
            ...
            // basis vectors
            unit_150 = { -sj::mathconst<float>::root_3_over_2, 0.5f };
            unit_60 = { 0.5f, sj::mathconst<float>::root_3_over_2 };
            ...
            if (i5_to_right == false) {
                ...
                sj::vec<float, 2> tmp = i1; i1 = i5; i5 = tmp;
                a1_tl = p4;
                a1_bl = q4;
        ...
        vec<float, 19> compute_overlap_far (const unsigned int _rotation)
        {
            ...
            unit_60 = {0.5f, sj::mathconst<float>::root_3_over_2};
            unit_300 = {0.5f, -sj::mathconst<float>::root_3_over_2};
            ...
            // Rectangle a1 area
            float vside = d * sj::mathconst<float>::one_over_root_3;
            float hside = (i2 - a1_tl).length();
            a1 = vside * hside;
        ...
        vec<float, 19> compute_overlap_corner (const unsigned int _rotation)
        {
            ...
            // Unit vectors
            vec<float, 2> uvv = {0.0f, 1.0f};
            vec<float, 2> unit_240 = {-0.5f, -sj::mathconst<float>::root_3_over_2};
            vec<float, 2> unit_120 = { -0.5f, sj::mathconst<float>::root_3_over_2};
            vec<float, 2> unit_30 = { sj::mathconst<float>::root_3_over_2, 0.5f };
            vec<float, 2> unit_210 = { -sj::mathconst<float>::root_3_over_2, -0.5f };
         ...
         */
        sj::vec<float, 2> boundaryCentroid = {0.0f, 0.0f};
         ...
         */
        sj::vec<float, 2> originalBoundaryCentroid = {0.0f, 0.0f};
    ...
    private:
        ...
        void init()
        {
            ...
            std::vector<std::list<sj::hex>::iterator> prevRingEven;
            std::vector<std::list<sj::hex>::iterator> prevRingOdd;

            // Swap pointers between rings.
            std::vector<std::list<sj::hex>::iterator>* prevRing = &prevRingEven;
            std::vector<std::list<sj::hex>::iterator>* nextPrevRing = &prevRingOdd;
            ...
            {
                std::list<sj::hex>::iterator h = this->hexen.end(); --h;
            ...
            for (unsigned int ring = 1; ring <= maxRing; ++ring) {
                ...
                std::vector<std::list<sj::hex>::iterator>* tmp = prevRing;
         ...
         */
        std::list<sj::hex>::iterator setBoundary (const sj::bezcoord<float>& point,
                                                     std::list<sj::hex>::iterator startFrom)
        {
            std::list<sj::hex>::iterator h = this->findHexNearPoint (point, startFrom);
        ...
        {
            this->bhexen.clear();
            std::list<sj::hex>::const_iterator bhi = this->hexen.begin();
            if (this->findBoundaryhex (bhi) == false) {
                // Found no boundary hex
                return false;
            }
            std::set<unsigned int> seen;
            std::list<sj::hex>::const_iterator hi = bhi;
        ...
        {
            bool rtn = false;
            std::list<sj::hex>::const_iterator hi_next;
            seen.insert (hi->vi);
        ...
        bool findBoundaryhex (std::list<hex>::const_iterator& hi) const
        {
            ...
            if (hi->has_ne()) {
                std::list<sj::hex>::const_iterator ci(hi->ne);
            ...
            if (hi->has_nne()) {
                std::list<sj::hex>::const_iterator ci(hi->nne);
                if (this->findBoundaryhex (ci) == true) {
                    hi = ci;
                    return true;
                }
            ...
            if (hi->has_nnw()) {
                std::list<sj::hex>::const_iterator ci(hi->nnw);
                if (this->findBoundaryhex (ci) == true) {
                    hi = ci;
                    return true;
                }
            ...
            if (hi->has_nw()) {
                std::list<sj::hex>::const_iterator ci(hi->nw);
                if (this->findBoundaryhex (ci) == true) {
                    hi = ci;
                    return true;
                }
            ...
            if (hi->has_nsw()) {
                std::list<sj::hex>::const_iterator ci(hi->nsw);
                if (this->findBoundaryhex (ci) == true) {
                    hi = ci;
                    return true;
                }
            ...
            if (hi->has_nse()) {
                std::list<sj::hex>::const_iterator ci(hi->nse);
                if (this->findBoundaryhex (ci) == true) {
                    hi = ci;
                    return true;
                }
        ...
        std::list<hex>::iterator findHexNearPoint (const bezcoord<float>& point, std::list<hex>::iterator startFrom)
        {
            ...
            std::list<sj::hex>::iterator h = startFrom;
                               ...
                               unsigned int insideFlag = HEX_INSIDE_BOUNDARY)
        {
            ...
            // Can debug first *count up* direction with sj::hex::neighbour_pos(diri)
                                     ...
                                     unsigned int insideFlag = HEX_INSIDE_BOUNDARY)
        {
            ...
            std::list<sj::hex>::iterator straight = first_inside;
                                        ...
                                        unsigned int insideFlag = HEX_INSIDE_BOUNDARY) const
        {
            ...
            for (unsigned short i = 0; i < 6 && gotnextneighbour == false; ++i) {
                ...
                if (bhi->has_neighbour(i) && bhi->get_neighbour(i)->testFlags(bdryFlag)) {
                    ...
                    std::list<sj::hex>::iterator cbhi = bhi->get_neighbour(i);
                    ...
                    for (unsigned short j = 0; j < 6; ++j) {
                        ...
                        // i's neighbour in dirn sj::hex::neighbour_pos(j) is the
                              ...
                              unsigned int insideFlag = HEX_INSIDE_BOUNDARY)
        {
            ...
            std::list<sj::hex>::iterator bhi(hi);
            while (bhi->testFlags (bdryFlag) == false && bhi->has_nne()) {
                bhi->setFlag (insideFlag);
                bhi = bhi->nne;
            }
            std::list<sj::hex>::iterator bhi_start = bhi;
            ...
            std::deque<std::list<sj::hex>::iterator> recently_seen;
        ...
        void discardOutsideBoundary()
        {
            ...
            std::list<sj::hex>::iterator centroidhex = this->findHexNearest (this->boundaryCentroid);
         ...
         * 'vertical' distance.
         */
        float v = 1.0f * sj::mathconst<float>::root_3_over_2;
