 ...
 * written/collected/adapted from internet examples for sj::Winder.
 *
 * \author Seb James
 * \date May 2020
 */
...
namespace sj {
    ...
    // Traits approach to testing for ability to access like an array (i.e. std::array, sj::vec)
    template<typename T>
    ...
    //! sj::has_size_const_method<T> tests whether a type T has a const size() method that returns size_t
     ...
     * std::array/sj::vec and other std containers, so it can be determined at
     ...
     *
     * If that passes, it's probably std::array or sj::vec.
     */
    template <typename T>
    ...
    // sj::value_type to allow us to write code that will accept float::value_type and std::vector<float>::value_type
    ...
    // that doesn't have value_type. For example sj::value_type_t<float> is float and
    // sj::value_type_t<std::array<float, 2>> is also float.
    template <class T> using value_type_t = typename sj::value_type<T>::type;
     ...
     *
     * You can use this trait test in template classes like sj::Scale for scalar/vector
     * implementations.
     *
    ...
    struct number_type {
        ...
        static constexpr bool const cplx = sj::is_complex<std::decay_t<T>>::value;
        static constexpr bool const container = sj::is_copyable_container<std::decay_t<T>>::value;
        // if container, check the element type
        static constexpr bool const container_of_scalars = container == true ? std::is_scalar<sj::value_type_t<T>>::value : false;
        static constexpr bool const container_of_complex = container == true ? sj::is_complex<sj::value_type_t<T>>::value : false;
