...
namespace sj {
    ...
    public:
        /*!
         * Construct, defining file access with the sj::file_access_mode enum.
         */
    ...
    public:
        ...
        void read_contained_vals (const char* path, Container<T, Allocator>& vals)
        {
            ...
            if constexpr (
                          ...
                          || std::is_same<std::decay_t<T>, sj::vec<float, 2>>::value == true
                          || std::is_same<std::decay_t<T>, sj::vec<double, 2>>::value == true
                          || std::is_same<std::decay_t<T>, sj::vec<int, 2>>::value == true
                          || std::is_same<std::decay_t<T>, sj::vec<long long int, 2>>::value == true
                          || std::is_same<std::decay_t<T>, sj::vec<unsigned int, 2>>::value == true
                          || std::is_same<std::decay_t<T>, sj::vec<unsigned long long int, 2>>::value == true
            ...
            if constexpr (std::is_same<std::decay_t<T>, float>::value == true
                          ...
                          || std::is_same<typename std::decay<T>::type, sj::vec<float,2>>::value == true
                ...
                status = H5Dread (dataset_id, H5T_NATIVE_FLOAT, H5S_ALL, H5S_ALL, H5P_DEFAULT, &(invals[0]));
                                 ...
                                 || std::is_same<typename std::decay<T>::type, sj::vec<double,2>>::value == true
                ...
                status = H5Dread (dataset_id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, &(invals[0]));
                                 ...
                                 || std::is_same<typename std::decay<T>::type, sj::vec<int,2>>::value == true
                ...
                status = H5Dread (dataset_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, &(invals[0]));
                                 ...
                                 || std::is_same<typename std::decay<T>::type, sj::vec<short int,2>>::value == true
                ...
                status = H5Dread (dataset_id, H5T_NATIVE_SHORT, H5S_ALL, H5S_ALL, H5P_DEFAULT, &(invals[0]));
                                 ...
                                 || std::is_same<typename std::decay<T>::type, sj::vec<unsigned int,2>>::value == true
                ...
                status = H5Dread (dataset_id, H5T_NATIVE_UINT, H5S_ALL, H5S_ALL, H5P_DEFAULT, &(invals[0]));
                                 ...
                                 || std::is_same<typename std::decay<T>::type, sj::vec<unsigned short int,2>>::value == true
                ...
                status = H5Dread (dataset_id, H5T_NATIVE_USHORT, H5S_ALL, H5S_ALL, H5P_DEFAULT, &(invals[0]));
                                 ...
                                 || std::is_same<typename std::decay<T>::type, sj::vec<unsigned long long int,2>>::value == true
                ...
                status = H5Dread (dataset_id, H5T_NATIVE_ULLONG, H5S_ALL, H5S_ALL, H5P_DEFAULT, &(invals[0]));
                                 ...
                                 || std::is_same<typename std::decay<T>::type, sj::vec<long long int,2>>::value == true
        ...
        //! read_contained_vals for an array<T,N> (and by extension, a sj::vec<T,N>)
        template<typename T, std::size_t N>
        ...
        template<typename T, std::size_t N>
        void read_contained_vals (const char* path, sj::vvec<sj::vec<T, N>>& vals)
        {
            hid_t dataset_id = H5Dopen2 (this->file_id, path, H5P_DEFAULT);
            if (this->check_dataset_id (dataset_id, path) == -1) { return; }
            hid_t space_id = H5Dget_space (dataset_id);
         ...
         * void add_contained_vals (const char*, const std::vector<sj::vec<T, N>>&)
         */
        template <typename T>
        ...
        template<typename T>
        void add_contained_vals (const char* path, const sj::vvec<sj::vvec<T>>& vals)
        {
            if (vals.empty()) { return; }
            this->process_groups (path);
        ...
        //! add_contained_vals for vector<sj::vec<T, N>>
        template<typename T, std::size_t N>
        void add_contained_vals (const char* path, const std::vector<sj::vec<T, N>>& vals)
        {
            if (vals.empty()) { return; }
            this->process_groups (path);
            hsize_t dim_vecNdcoords[2];
        ...
        // Code duplication of the above template function for std::vector<sj::vec<T,N>> :(
        template<typename T, std::size_t N>
        void add_contained_vals (const char* path, const sj::vvec<sj::vec<T, N>>& vals)
        {
            if (vals.empty()) { return; }
            this->process_groups (path);
            hsize_t dim_vecNdcoords[2];
