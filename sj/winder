 ...
 * \file
 *
 * Provides sj::winder, a class to compute the winding number of a boundary with
...
namespace sj
{
     ...
     * access members in an array-like fashion (std::vector or sj::vec). For example:
     *
     *\code{c++}
     *  std::list<sj::vec<float, 2>> path;
     *  // Code which populates path goes here
     *  sj::winder w(path);
     *  sj::vec<float, 2> pixel = {0.7, 0.6};
     *  int winding_number = w.wind (pixel);
     *\endcode
     *
     * \tparam T the (2D) coordinate type (this might be cv::Point, sj::BezCoord,
     * sj::vvec, sj::vec, std::array or std::vector)
     *
    ...
    class winder
    {
        ...
        static_assert (sj::is_copyable_container<C>::value == true);
    ...
    public:
        ...
        int wind (const T& px) {
            ...
            double winding_no_d = std::round (this->angle_sum/sj::mathconst<double>::two_pi);
    ...
    private:
        ...
        //! method sj::winder::wind (const double&) may be called.
        void wind (const T& px, const T& bp) {
            ...
            if constexpr (array_access_possible<T>::value == true) {
                ...
                // cv::Point, sj::BezCoord should fall through here.
                if constexpr (has_subtraction<T>::value == true) {
        ...
        //! all the specialization is in sj::winder::wind (const T&, const T&).
        void wind (const double& raw_angle) {

            // Convert the raw angle which has range -pi -> 0 -> +pi into a tansformed angle with range 0->2pi:
            this->angle = raw_angle >= 0 ? raw_angle : (sj::mathconst<double>::two_pi + raw_angle);
            if constexpr (sj::debug_mode == true) { std::cout << "angle: " << this->angle << "\n"; }

            // Set the initial angle.
            if (this->angle_last == double{-100}) { this->angle_last = angle; }

            // Compute the change in angle, delta
            double delta = double{0}; // delta is 'angle change'
            if (this->angle == double{0}) {
                // Special treatment
                if (this->angle_last > sj::mathconst<double>::pi) {
                    // Clockwise to 0
                    delta = (sj::mathconst<double>::two_pi - this->angle_last);
                } else if (this->angle_last < sj::mathconst<double>::pi) {
                    // Anti-clockwise to 0
                ...
                if (this->angle_last > sj::mathconst<double>::pi && this->angle < sj::mathconst<double>::pi) {
                    // crossed from 2pi side to 0 side: Clockwise
                    delta = this->angle + (sj::mathconst<double>::two_pi - this->angle_last);
                } else if (this->angle_last < sj::mathconst<double>::pi_over_2 && this->angle > sj::mathconst<double>::three_pi_over_2) {
                    // crossed from 0 side to 2pi side: Anti-clockwise
                    delta = - this->angle_last - (sj::mathconst<double>::two_pi - this->angle);
        ...
        void reset() {
            this->angle_last = double{-100};
            this->angle = double{0};
            this->angle_sum = double{0};
        }
