...
namespace sj {
    ...
    public:
         ...
         */
        int indexFromCoord (const sj::vec<float, 2>& coord) const
        {
         ...
         */
        sj::vvec<sj::vec<float, 2>> shiftCoords (const sj::vvec<sj::vec<float, 2>>& cds,
                                                       const float x_shift, const float y_shift) const
        {
            ...
            sj::vvec<sj::vec<float, 2>> new_coords;
            sj::vec<float, 2> ctmp;
            for (auto c : cds) {
                ctmp[0] = c[0] + x_step;
         ...
         * are simply omitted from the return vvec.
         */
        sj::vvec<int> shiftIndiciesByMetric (const sj::vvec<int>& inds,
                                                const float x_shift, const float y_shift) const
        {
        ...
        std::vector<sj::vec<float, 3>> getCoordinates3() const
        {
            std::vector<sj::vec<float, 3>> coords (this->num());
        ...
        std::vector<sj::vec<float, 2>> getCoordinates2() const
        {
            std::vector<sj::vec<float, 2>> coords (this->num());
            for (unsigned int i = 0; i < this->num(); ++i) {
        ...
        sj::vvec<sj::vec<float, 3>> getCoords() const
        {
            sj::vvec<sj::vec<float, 3>> rtn (d_x.size(), {0,0,0});
        ...
        sj::vvec<sj::vec<float, 2>> getCoords2D() const
        {
            sj::vvec<sj::vec<float, 2>> rtn (d_x.size(), {0,0});
            for (unsigned int i = 0; i < d_x.size(); ++i) {
                rtn[i][0] = d_x[i];
                rtn[i][1] = d_y[i];
            }
        ...
        sj::range<int> xi_minmax;
        sj::range<int> yi_minmax;
        ...
        void populate_d_neighbours()
        {
            ...
            std::list<sj::rect>::iterator ri = this->rects.begin();
        ...
        {
            sj::hdfdata cgdata (path, std::ios::out | std::ios::trunc);
            cgdata.add_val ("/d", d);
            cgdata.add_val ("/v", v);
            ...
            std::list<sj::rect>::const_iterator r = this->rects.begin();
        ...
        {
            sj::hdfdata cgdata (path, std::ios::in);
        ...
        sj::vec<float, 2> centre_of_mass (sj::vvec<F>& data)
        {
            sj::vec<float, 2> com = {0,0};
            F datasum = F{0};
            for (unsigned int i = 0; i < this->num(); ++i) {
                com[0] += d_x[i] * data[i];
                com[1] += d_y[i] * data[i];
        ...
        sj::vec<float, 2> computeCentroid (const std::list<rect>& pRects)
        {
            sj::vec<float, 2> centroid = { 0.0f, 0.0f };
            for (auto r : pRects) {
                centroid[0] += r.x;
                centroid[1] += r.y;
            }
        ...
        void setBoundary (const std::list<rect>& pRects)
        {
            ...
            std::list<sj::rect>::iterator bpoint = this->rects.begin();
            std::list<sj::rect>::iterator bpi = this->rects.begin();
            while (bpi != this->rects.end()) {
                std::list<sj::rect>::const_iterator ppi = pRects.begin();
            ...
            std::list<sj::rect>::iterator ri = bpoint;
            ...
            if (this->domainShape == sj::griddomainshape::boundary) {
         ...
         * this boundary. Finishes up by calling sj::cartgrid::discardOutside.
        ...
        /*!
         * This sets a boundary, just as sj::cartgrid::setBoundary(const
         * sj::BezCurvePath<float> p, bool offset) does but WITHOUT discarding rects
        ...
        void setBoundary (std::vector<BezCoord<float>>& bpoints, bool loffset = true)
        {
            ...
            std::list<sj::rect>::iterator nearbyBoundaryPoint = this->rects.begin(); // i.e the rect at 0,0
            bpi = bpoints.begin();
            while (bpi != bpoints.end()) {
            ...
            {
                std::set<unsigned int> seen;
                std::list<sj::rect>::iterator hi = nearbyBoundaryPoint;
        ...
        sj::vec<float, 2> findmax (const sj::vvec<float>& image_data)
        {
            unsigned int idx = image_data.argmax();
            return sj::vec<float, 2>({this->d_x[idx], this->d_y[idx]});
        }
        ...
        // of view_angle.
        void resampleToPolar (const sj::vvec<float>& image_data,
                              sj::cartgrid& cg_polar, sj::vvec<float>& polar_data,
                              sj::vec<float, 2> view_pos, float view_angle, sj::scaling_function radscale = sj::scaling_function::Linear)
        {
            polar_data.zero();

            // distance per pixel in the image. This defines the Gaussian width (sigma) for the resample:
            sj::vec<float, 2> dist_per_pix = { this->d, this->v };
            sj::vec<float, 2> params = 1.0f / (2.0f * dist_per_pix * dist_per_pix);
            float assumecirc = params.mean();
            sj::vec<float, 2> polar_span = cg_polar.getSpan();

            sj::vec<unsigned int, 2> polar_span_pix = cg_polar.getSpanPix();
            ...
            float rad_per_dist = sj::mathconst<float>::two_pi/(polar_span[0]+cg_polar.getd());

            std::list<sj::rect>::iterator lastrect = this->rects.begin();
#pragma omp parallel for
            for (unsigned int xi = 0; xi < cg_polar.num(); ++xi) { // for each output pixel which is an r/phi pair

                float r = cg_polar.d_y[xi]; // Linear
                if (radscale == sj::scaling_function::Logarithmic) {
                ...
                if (phi_imframe > sj::mathconst<float>::pi) { phi_imframe -= sj::mathconst<float>::two_pi; }
                // x,y in the image frame associated with r,phi in the polar rep:
                sj::vec<float, 2> abs_xy_imframe = sj::vec<float, 2>({r * std::cos(phi_imframe),
                ...
                //std::list<sj::rect>::iterator nearest = this->findRectNearest (abs_xy_imframe);
                // or (if it's faster?):
                std::list<sj::rect>::iterator nearest = this->findRectNearPoint (abs_xy_imframe, lastrect);
                lastrect = nearest;

                // Now sum up contribution from nearest and its neighbours to polar_data[xi].

                // Closest pix
                std::list<sj::rect>::iterator curr = nearest;
                float dd = (abs_xy_imframe - sj::vec<float, 2>({curr->x, curr->y})).length();
         ...
         * sj::cartgrid::setBoundary(vector<sj::BezCoord<float>& bpoints, bool offset)
        ...
        void setBoundaryOnly (std::vector<BezCoord<float>>& bpoints, bool loffset)
        {
            ...
            {
                std::set<unsigned int> seen;
                std::list<sj::rect>::iterator ri = nearbyBoundaryPoint;
        ...
        void setBoundaryOnOuterEdge()
        {
            ...
            std::set<unsigned int> seen;
            std::list<sj::rect>::iterator ri = bpi;
        ...
        {
            std::list<sj::rect> brects_concrete;
        ...
        std::vector<BezCoord<float>> ellipseCompute (const float a, const float b,
                                                     const sj::vec<float, 2> c = {0.0f, 0.0f}) const
        {
            ...
            for (double phi = 0.0; phi < sj::mathconst<double>::two_pi; phi+=delta_phi) {
                sj::vec<float, 2> xy_pt = { static_cast<float>(a * std::cos (phi) + c[0]), static_cast<float>(b * std::sin (phi) + c[1]) };
        ...
        void setEllipticalBoundary (const float a, const float b,
                                    const sj::vec<float, 2> c = {0.0f, 0.0f}, const bool offset=true)
        {
        ...
        sj::vec<float, 4> get_extents() const
        {
            sj::vec<int, 4> extents;
            if constexpr (debug_cartgrid) {
                std::cout << "cartgrid::get_extents(): calling findBoundaryExtents()\n";
            }
            extents.set_from (this->findBoundaryExtents());
            sj::vec<float, 4> extents_mult = { this->d, this->d, this->v, this->v };
        ...
        sj::vec<float, 2> getSpan() const { return sj::vec<float, 2>({this->x_span, this->y_span}); }

        //! Get the x/y span in elements/pixels
        sj::vec<unsigned int, 2> getSpanPix() const
        {
            ...
            return sj::vec<unsigned int, 2>({ 1+_x_pixdist, 1+_y_pixdist });
        }
        ...
        {
            std::list<sj::rect>::iterator r = this->rects.begin();
            while (r != this->rects.end()) {
                if (r->testFlags(RECT_IS_BOUNDARY) == true) {
                    ...
                    if (r->testFlags(RECT_INSIDE_BOUNDARY) == false) {
                        ...
                        std::list<sj::rect>::iterator br = this->rects.begin();
        ...
        void populate_d_vectors (const std::array<int, 4>& extnts)
        {
            ...
            std::list<sj::rect>::iterator blr = this->rects.end();

            if (this->domainShape == sj::griddomainshape::rectangle) {
            ...
            this->xi_minmax = sj::range<int>(d_xi[0], d_xi[d_xi.size()-1]);
            this->yi_minmax = sj::range<int>(d_yi[0], d_yi[d_yi.size()-1]);
         ...
         * region. The idea is that client code will then use the vector of sj::rect* to work
                                                          ...
                                                          bool applyOriginalBoundaryCentroid = true)
        {
            ...
            // A return object
            std::vector<std::list<sj::rect>::iterator> theRegion;
            ...
            std::list<sj::rect>::iterator nearbyRegionBoundaryPoint = this->rects.begin(); // i.e the rect at 0,0
            ...
            {
                std::set<unsigned int> seen;
                std::list<sj::rect>::iterator hi = nearbyRegionBoundaryPoint;
            ...
            std::list<sj::rect>::iterator insideRegionRect = this->findRectNearest (regionCentroid);
            this->markRectsInside (insideRegionRect, RECT_IS_REGION_BOUNDARY, RECT_INSIDE_REGION);

            // Populate theRegion, then return it
            std::list<sj::rect>::iterator hi = this->rects.begin();
        ...
        void boxfilter_f (const sj::vvec<T>& data, sj::vvec<T>& result) const
        {
            if (result.size() != this->rects.size()) {
                throw std::runtime_error ("The result vector is not the same size as the cartgrid.");
            }
            ...
            // We are a wrapper:
            sj::algo::boxfilter_2d<T, boxside, onlysum> (data, result, this->w_px);
        }
    ...
    private:
        ...
        void init()
        {
            ...
            this->x_minmax = sj::range<float>(-halfCols * this->d, halfCols * this->d);
            this->y_minmax = sj::range<float>(-halfRows * this->v, halfRows * this->v);

            // The "vector iterator" - this is an identity iterator that is added to each rect in the grid.
            unsigned int vi = 0;

            std::vector<std::list<sj::rect>::iterator> prevRowEven;
            std::vector<std::list<sj::rect>::iterator> prevRowOdd;

            // Swap pointers between rows.
            std::vector<std::list<sj::rect>::iterator>* prevRow = &prevRowEven;
            std::vector<std::list<sj::rect>::iterator>* nextPrevRow = &prevRowOdd;
            ...
            for (int yi = -halfRows; yi <= halfRows; ++yi) {
                ...
                std::vector<std::list<sj::rect>::iterator>* tmp = prevRow;
                prevRow = nextPrevRow;
                nextPrevRow = tmp;
                nextPrevRow->clear();
            }
        ...
        void init2 (float x1, float y1, float x2, float y2)
        {
            ...
            this->x_minmax = sj::range<float>(x1, x2);
            this->y_minmax = sj::range<float>(y1, y2);
            ...
            for (int yi = _yi; yi <= _yf; ++yi) { // for each row
                ...
                if (this->domainWrap == sj::griddomainwrap::horizontal || this->domainWrap == sj::griddomainwrap::both) {
         ...
         */
        std::list<sj::rect>::iterator setBoundary (const sj::BezCoord<float>& point,
                                                     std::list<sj::rect>::iterator startFrom)
        {
            std::list<sj::rect>::iterator h = this->findRectNearPoint (point, startFrom);
        ...
        bool isInsideRectangularBoundary (const sj::vec<float, 2>& point)
        {
        ...
        {
            this->brects.clear();
            std::list<sj::rect>::const_iterator bhi = this->rects.begin();
            if (this->findBoundaryRect (bhi) == false) {
                // Found no boundary rect
                return false;
            }
            std::set<unsigned int> seen;
            std::list<sj::rect>::const_iterator hi = bhi;
        ...
        {
            bool rtn = false;
            std::list<sj::rect>::const_iterator ri_next;
            seen.insert (ri->vi);
         ...
         * information for later use.
         */
        std::list<rect>::iterator setRegionBoundary (const sj::vec<float, 2>& point, std::list<rect>::iterator startFrom)
        {
            std::list<sj::rect>::iterator h = this->findRectNearPoint (point, startFrom);
            h->setFlag (RECT_IS_REGION_BOUNDARY | RECT_INSIDE_REGION);
            return h;
        ...
        {
            bool rtn = false;
            std::list<sj::rect>::const_iterator hi_next;
            seen.insert (hi->vi);
        ...
        bool findBoundaryRect (std::list<rect>::const_iterator& ri) const
        {
            ...
            if (ri->has_ns()) {
                std::list<sj::rect>::const_iterator ci(ri->ns);
        ...
        std::list<rect>::iterator findRectNearPoint (const vec<float, 2>& point, std::list<rect>::iterator startFrom)
        {
            ...
            std::list<sj::rect>::iterator h = startFrom;
                               ...
                               unsigned int insideFlag = RECT_INSIDE_BOUNDARY)
        {
            ...
            // Can debug first *count up* direction with sj::rect::neighbour_pos(diri)
                                     ...
                                     unsigned int insideFlag = RECT_INSIDE_BOUNDARY)
        {
            ...
            std::list<sj::rect>::iterator straight = first_inside;
                                        ...
                                        unsigned int insideFlag = RECT_INSIDE_BOUNDARY) const
        {
            ...
            for (unsigned short i = 0; i < 6 && gotnextneighbour == false; ++i) {
                ...
                if (bhi->has_neighbour(i) && bhi->get_neighbour(i)->testFlags(bdryFlag)) {
                    ...
                    std::list<sj::rect>::iterator cbhi = bhi->get_neighbour(i);
                    ...
                    for (unsigned short j = 0; j < 6; ++j) {
                        ...
                        // i's neighbour in dirn sj::rect::neighbour_pos(j) is the
                              ...
                              unsigned int insideFlag = RECT_INSIDE_BOUNDARY)
        {
            ...
            std::list<sj::rect>::iterator bhi(hi);
            while (bhi->testFlags (bdryFlag) == false && bhi->has_nne()) {
                bhi->setFlag (insideFlag);
                bhi = bhi->nne;
            }
            std::list<sj::rect>::iterator bhi_start = bhi;
            ...
            std::deque<std::list<sj::rect>::iterator> recently_seen;
        ...
        void discardOutsideBoundary()
        {
            ...
            std::list<sj::rect>::iterator centroidRect = this->findRectNearest (this->boundaryCentroid);
         ...
         * pos.
         */
        std::list<rect>::iterator findRectNearest (const sj::vec<float, 2>& pos)
        {
            std::list<sj::rect>::iterator nearest = this->rects.end();
            std::list<sj::rect>::iterator ri = this->rects.begin();
            float dist = std::numeric_limits<float>::max();
            while (ri != this->rects.end()) {
                sj::vec<float, 2> dx = { pos[0] - ri->x, pos[1] - ri->y };
                float dl = dx.length();
                if (dl < dist) {
                    dist = dl;
                    nearest = ri;
        ...
        {
            std::list<sj::rect>::iterator bottomleft = this->rects.begin();
    ...
    public:
        ...
        sj::range<float> x_minmax;
        sj::range<float> y_minmax;
    };
