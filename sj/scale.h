 ...
 *
 * Classes created from the template class sj::scale will derive from one of the sj::ScaleImpl
 * <ntype, T, S> classes, where ntype is the 'number type' (0 means the numbers are mathematical
 * vectors like sj::vec, std::array or std::vector; 1 means that the numbers are scalars like
 * float, double or int)
 *
...
namespace sj {
    ...
    /*!
     * \brief Base class for sj::scale_impl
     *
     ...
     * double, etc or a type which can contain a vector such as std::array, sj::vec or
     * std::vector.
     *
    ...
    public:
        ...
        requires sj::is_copyable_container<Container>::value && sj::is_copyable_container<OContainer>::value
        void transform (const Container& data, OContainer& output)
        {
        ...
        requires sj::is_copyable_container<Container>::value && sj::is_copyable_container<OContainer>::value
        void inverse (const Container& data, OContainer& output)
        {
            std::size_t dsize = data.size();
            if (output.size() != dsize) {
        ...
        //! Transform a range
        sj::range<S> transform (const sj::range<T>& data)
        {
            return sj::range<S>{ this->transform_one (data.min), this->transform_one (data.max) };
        }

        //! Inverse transform a range
        sj::range<T> inverse (const sj::range<S>& data)
        {
            return sj::range<T>{ this->inverse_one (data.min), this->inverse_one (data.max) };
        }
        ...
        virtual void compute_scaling (const sj::range<T>& input_range) = 0;
         ...
         * will be something like \c std::vector<float> or \c std::list<sj::vec<double,2>>
         */
        template <typename Container> requires sj::is_copyable_container<Container>::value
        void compute_scaling_from_data (const Container& data)
        {
            using Tc = typename Container::value_type;
            sj::range<Tc> mm = sj::range<Tc>::get_from (data);
     ...
     * vector such as std::array or sj::vec.
     *
     ...
     *
     * \tparam ntype The 'number type' as contained in sj::number_type::value. 1 for vectors, 0
     ...
     * sj::vec or std::vector. It is from the type \a T that ntype is
    ...
    public:
        ...
        sj::range<S_el> output_range = sj::range<S_el>(S_el{0}, S_el{1});
        ...
        void compute_scaling (const sj::range<T>& input_range) { this->compute_scaling (input_range.min, input_range.max); }
        ...
        //! Get all the params
        sj::vvec<S_el> getParams() { return this->params; }
    ...
    public:
        //! The output range required. Change if you want to scale to something other than [0, 1]
        sj::range<S> output_range = sj::range<S>(S{0}, S{1});
        ...
        //! Get all the params
        sj::vvec<S> getParams() { return this->params; }
    ...
    protected:
        ...
        void compute_scaling_log (T input_min, T input_max)
        {
            ...
            if constexpr (sj::number_type<T>::value == 1) {
    ...
    class scale_impl<3, T, S> : public scale_impl_base<T, S> {
        void compute_scaling_linear ([[maybe_unused]] T input_min, [[maybe_unused]] T input_max) {
            []<bool flag = false>() { static_assert(flag, "sj::scale does not support scaling vectors of complex numbers"); }();
        }
    };
    ...
    class scale_impl<-1, T, S> : public scale_impl_base<T, S> {
        void compute_scaling_linear ([[maybe_unused]] T input_min, [[maybe_unused]] T input_max) {
            []<bool flag = false>() { static_assert(flag, "sj::scale does not support scaling non-number types"); }();
        }
    };
     ...
     *
     * sj::scale derives from scale_impl<N> depending on the type of T
     *
    ...
    struct scale : public scale_impl<sj::number_type<T>::value, T, S> {};
