...
namespace sj
{
    ...
    struct bezcoord
    {
        ...
        bezcoord (const sj::vec<F, 2>& r) : coord(r) {}
        //! Construct with coordinate and corresponding t parameter
        bezcoord (F t, const sj::vec<F, 2>& r) : coord(r), param(t) {}
        //! Construct with coord & t and also set the para remaining
        bezcoord (F t, const sj::vec<F, 2>& r, F remain) : coord(r), param(t), remaining(remain) {}
        ...
        void normalize()
        {
            bezcoord origin(sj::vec<F, 2>({0.0f,0.0f}));
