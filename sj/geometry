...
namespace sj::geometry_ce
{
    ...
    struct polyhedron
    {
        ...
        sj::vec<sj::vec<F, 3>, n_verts> vertices;
        //! A list of the faces
        sj::vec<sj::vec<int, 3>, n_faces> faces;
    };
    ...
    {
        constexpr F phi = (F{1} + sj::mathconst<F>::root_5) / F{2};

        // Unit vectors in 3D
        constexpr sj::vec<F, 3> _ux = { F{1}, F{0}, F{0} };
        constexpr sj::vec<F, 3> _uz = { F{0}, F{0}, F{1} };


        sj::geometry_ce::polyhedron<F, 12, 20> ico;
        // Arranged 'in spiral order', going with positive angle in x/y plane (i.e. around z axis)
        ico.vertices = sj::vec< sj::vec<F, 3>, 12> {
            sj::vec<F, 3>{ F{-1}, phi,   F{0}  },

            sj::vec<F, 3>{ F{1},  phi,   F{0}  },
            sj::vec<F, 3>{ F{0},  F{1},  -phi  },
            sj::vec<F, 3>{ -phi,  F{0},  F{-1} },
            sj::vec<F, 3>{ -phi,  F{0},  F{1}  },
            sj::vec<F, 3>{ F{0},  F{1},  phi   },
        ...
        sj::quaternion<F> rotn1(_uz, -sj::math::atan2(F{1}, phi));
        // ...and a rotation about the x axis:
        sj::quaternion<F> rotn2(_ux, sj::mathconst<F>::pi_over_2);
        ...
        ico.faces = sj::vec< sj::vec<int, 3>, 20>{
            sj::vec<int, 3>{0, 1, 2},
            sj::vec<int, 3>{0, 2, 3},
            sj::vec<int, 3>{0, 3, 4},
            sj::vec<int, 3>{0, 4, 5},
            ...
            sj::vec<int, 3>{10, 11, 6},
            sj::vec<int, 3>{6, 11, 7},
            sj::vec<int, 3>{7, 11, 8},
            sj::vec<int, 3>{8, 11, 9},
            sj::vec<int, 3>{9, 11, 10}
        };
     ...
     * sj::geometry version. DO want to test for duplicate vertices.
     *
     ...
     *
     * \return the literal type object sj::geometry_ce::icosahedral_geodesic containing
     * vertices and faces of the geodesic.
     */
    template<typename F, int iterations>
    constexpr sj::geometry_ce::icosahedral_geodesic<F, iterations> make_icosahedral_geodesic()
    {
        sj::geometry_ce::icosahedral_geodesic<F, iterations> geo; // our return object

        // geo must be fully initialized in a constexpr function (this could be constructor in icosahedral_geodesic<>
        constexpr sj::vec<int, 3> fz = { 0, 0, 0 };
        constexpr sj::vec<F, 3> vz = { F{0}, F{0}, F{0} };
        ...
        constexpr sj::geometry_ce::polyhedron<F, 12, 20> initial_ico = sj::geometry_ce::icosahedron<F>();
        ...
        for (int i = 0; i < iterations; ++i) {
            ...
            for (int f = 0; f < _n_faces; ++f) { // Loop over existing faces
                sj::vec<F, 3> va = (geo.poly.vertices[geo.poly.faces[f][1]] + geo.poly.vertices[geo.poly.faces[f][0]]) / 2.0f;
                sj::vec<F, 3> vb = (geo.poly.vertices[geo.poly.faces[f][2]] + geo.poly.vertices[geo.poly.faces[f][1]]) / 2.0f;
                sj::vec<F, 3> vc = (geo.poly.vertices[geo.poly.faces[f][0]] + geo.poly.vertices[geo.poly.faces[f][2]]) / 2.0f;
                va.renormalize();
                vb.renormalize();
                vc.renormalize();
                ...
                for (int v = 0; v < _n_verts; ++v) {
                    if (sj::math::abs(geo.poly.vertices[v][0] - va[0]) < thresh * std::numeric_limits<F>::epsilon()
                        && sj::math::abs(geo.poly.vertices[v][1] - va[1]) < thresh * std::numeric_limits<F>::epsilon()
                        && sj::math::abs(geo.poly.vertices[v][2] - va[2]) < thresh * std::numeric_limits<F>::epsilon()) {
                ...
                int b = -1;
                for (int v = 0; v < _n_verts; ++v) {
                    if (sj::math::abs(geo.poly.vertices[v][0] - vb[0]) < thresh * std::numeric_limits<F>::epsilon()
                        && sj::math::abs(geo.poly.vertices[v][1] - vb[1]) < thresh * std::numeric_limits<F>::epsilon()
                        && sj::math::abs(geo.poly.vertices[v][2] - vb[2]) < thresh * std::numeric_limits<F>::epsilon()) {
                ...
                int c = -1;
                for (int v = 0; v < _n_verts; ++v) {
                    if (sj::math::abs(geo.poly.vertices[v][0] - vc[0]) < thresh * std::numeric_limits<F>::epsilon()
                        && sj::math::abs(geo.poly.vertices[v][1] - vc[1]) < thresh * std::numeric_limits<F>::epsilon()
                        && sj::math::abs(geo.poly.vertices[v][2] - vc[2]) < thresh * std::numeric_limits<F>::epsilon()) {
                ...
                // 4 new triangles.
                sj::vec<int, 3> newface = { geo.poly.faces[f][0], a, c };
...
namespace sj::geometry
{
    ...
    struct polyhedron
    {
        ...
        sj::vvec<sj::vec<F, 3>> get_face_centres() const
        {
            sj::vvec<sj::vec<F, 3>> fcentres(this->faces.size(), { F{0}, F{0}, F{0} });
        ...
        sj::vvec<sj::vvec<sj::vec<F, 3>>> get_neighbour_vectors() const
        {
            sj::vvec<sj::vvec<sj::vec<F, 3>>> vneighb_vectors_per_vertex (this->vneighbours.size());
            for (unsigned int i = 0; i < this->vneighbours.size(); ++i) {
                for (auto n : this->vneighbours[i]) { // vneighbour[i] is a set<int>
                    sj::vec<F, 3> _v = this->vertices[n] - this->vertices[i];
    ...
    struct icosahedral_geodesic
    {
         ...
         *
         * Note: In this function I use the default number of elements, N, for a sj::vec<F,
         ...
         */
        sj::vvec< sj::vvec<std::tuple<sj::vec<F>,int>> > get_neighbour_hexdir_vectors() const
        {
            using sj::vec;
            using sj::vvec;
            using std::tuple;
            ...
            vvec<vvec<tuple<vec<F>,int>>> vneighb_vectors_per_vertex (this->poly.vneighbours.size());
                                                                            ...
                                                                            const vec<F>& nrm_vec)
            {
                ...
                vvec<tuple<vec<F>, int>> rgb_vectors (3, {vec<F>{ F{0}, F{0}, F{0} }, -1});

                int vnsz = static_cast<int>(vneighb_vectors_per_vertex[i].size()); // 5 or 6 neighbours?

                vvec<F> angles(vnsz, F{0});
                F min_angle = Fmax;                 // Find the minimum angle
                int min_angle_index = -1;           // index of min_angle *into vneighb_vectors_per_vertex[i]*
                int min_angle_vertex_index = -1;    // index of the min_angle vertex *into poly.vertices*
                ...
                for (int j = 0; j < vnsz; ++j) {
                    ...
                    if (angles[j] < min_angle) {
                        min_angle_index = j;
                        min_angle_vertex_index = std::get<int>(vneighb_vectors_per_vertex[i][j]);
                        min_angle = angles[j];
                    }
                }
                //int r_index = min_angle_index; // This is not the original index of the neighbour
                ...
                angles.zero();
                std::map<F, tuple<vec<F>, int>> vec_map; // A map with key as angle, to order vectors easily
                for (int j = 0; j < vnsz; ++j) {
            ...
            for (int i = 0; i < static_cast<int>(this->poly.vneighbours.size()); ++i) {
                ...
                for (int n : this->poly.vneighbours[i]) { // vneighbour[i] is a set<int>
    ...
    {
        sj::geometry::polyhedron<F> ico;
        ico.vertices.resize(12);
        ico.faces.resize(20);
        sj::geometry_ce::polyhedron<F, 12, 20> initial_ico = sj::geometry_ce::icosahedron<F>();
     ...
     */
    template<typename F>
    sj::geometry::icosahedral_geodesic<F> make_icosahedral_geodesic (const int iterations)
    {
        sj::geometry::icosahedral_geodesic<F> geo;
        // Start out with an icosahedron
        geo.poly = sj::geometry::icosahedron<F>();
        ...
        auto _vtx_cmp = [](sj::vec<F, 3> a, sj::vec<F, 3> b)
        {
        ...
        // sj::vec is key to map, as we will have a very custom sorting function. This
        // requires some care with the sorting function used by the std::map
        std::map<sj::vec<F, 3>, int, decltype(_vtx_cmp)> vertices_map(_vtx_cmp);
        ...
        for (int i = 0; i < iterations; ++i) {
            ...
            std::map<sj::vec<F, 3>, // Comparing on the vertex key
                     sj::vec<int, 3>, decltype(_vtx_cmp)> faces_map(_vtx_cmp);
            int count = 0;
            int fcount = 0;
            ...
            auto add_face = [&faces_map, &count] (const sj::vec<F, 3>& vA,
                                                  const sj::vec<F, 3>& vB,
                                                  const sj::vec<F, 3>& vC,
                                                  const sj::vec<int, 3>& newface,
                                                  const int newface_num)
            {
                sj::vec<F, 3> centroid = (vA + vB + vC) / 3.0f;
            ...
            for (const auto f : geo.poly.faces) { // faces contains indexes into vertices.
                ...
                sj::vec<F, 3> va = (geo.poly.vertices[f[1]] + geo.poly.vertices[f[0]]) / 2.0f;
                sj::vec<F, 3> vb = (geo.poly.vertices[f[2]] + geo.poly.vertices[f[1]]) / 2.0f;
                sj::vec<F, 3> vc = (geo.poly.vertices[f[0]] + geo.poly.vertices[f[2]]) / 2.0f;
                ...
                sj::vec<int, 3> newface = { f[0], a, c }; // indices in old money here
