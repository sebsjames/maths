...
namespace sj
{
    ...
    struct bootstrap {
        ...
        static void resample_with_replacement (const sj::vvec<T>& data,
                                               std::vector<sj::vvec<T>>& resamples, const unsigned int B)
        {
            unsigned int data_n = data.size();
            resamples.resize (B);
            for (unsigned int i = 0; i < B; ++i) {
                sj::vvec<T> resample (data_n);
                sj::vvec<unsigned int> randindices (data_n);
        ...
        static T error_of_mean (const sj::vvec<T>& data, const unsigned int B)
        {
            std::vector<sj::vvec<T>> resamples;
            sj::bootstrap<T>::resample_with_replacement (data, resamples, B);
            sj::vvec<T> r_mean (B, T{0});
            for (unsigned int i = 0; i < B; ++i) {
        ...
        static T error_of_std (const sj::vvec<T>& data, const unsigned int B)
        {
            std::vector<sj::vvec<T>> resamples;
            sj::bootstrap<T>::resample_with_replacement (data, resamples, B);
            sj::vvec<T> r_std (B, T{0});
            for (unsigned int i = 0; i < B; ++i) {
        ...
        static sj::vec<T, 2> ttest_equalityofmeans (const sj::vvec<T>& _zdata,
                                                       const sj::vvec<T>& _ydata, const unsigned int B)
        {
            // Ensure that the group which we name zdata is the larger one.
            sj::vvec<T> zdata = _zdata;
            sj::vvec<T> ydata = _ydata;
            ...
            sj::vvec<T> ztilda = zdata - zmean + xmean;
            sj::vvec<T> ytilda = ydata - ymean + xmean;
            if constexpr (debug_bstrap) {
            ...
            sj::vvec<T> zvariances (B, T{0});
            sj::vvec<T> yvariances (B, T{0});
            for (unsigned int i = 0; i < B; ++i) {
            ...
            sj::vvec<T> top = zstarmeans - ystarmeans;
            sj::vvec<T> bot = (yvariances/static_cast<T>(m) + zvariances/static_cast<T>(n)).sqrt();
            sj::vvec<T> txstar = top / bot;
            if constexpr (debug_bstrap) {
            ...
            return sj::vec<T, 2> ({asl, minasl});
        }
        // std::vector version of ttest_equalityofmeans()
        static sj::vec<T, 2> ttest_equalityofmeans (const std::vector<T>& _zdata,
