...
#include <sj/constexpr_math.h> // constexpr math functions from Keith O'Hara
#include <sj/range.h>
#include <sj/vec.h>
...
namespace sj::algo
{
    ...
    constexpr sj::range<int> significant_cols (const F& f)
    {
        sj::range<int> sigcols = { 0, 0 };

        // If NaN or infinity, return 0, 0. Or something else?
        if (sj::math::isnan(f) || sj::math::isinf(f)) { return sigcols; }

        // How many sig figures does the type (float, double...) support?
        constexpr F epsilon = std::numeric_limits<F>::epsilon();
        constexpr int type_sf = static_cast<int>(sj::math::floor (sj::math::log10 (F{1} / epsilon)));

        // copy absolute value of f
        F fcpy = f > F{0} ? f : -f;

        // Find the biggest column
        sigcols.max = static_cast<int>(sj::math::floor (sj::math::log10 (fcpy)));

        // Initialize smallest column
        sigcols.min = sigcols.max;

        // We will need to use 10^sigcols.max
        F tentothe = sj::math::pow (F{10}, sigcols.max);
        ...
        for (; sigcols.min > (sigcols.max - type_sf); --sigcols.min) {
            ...
            F colval = sj::math::floor(fcpy / tentothe);
            ...
            if (colval > F{8}) {
                ...
                if (cols_remaining > 0) {
                    F additional_cols = sj::math::pow (F{10}, cols_remaining);
                    if (sj::math::abs((fcpy - colval * tentothe) - tentothe) < (tentothe / additional_cols)) {
                        ++sigcols.min;
                        break;
                    }
                } else { // no cols left
    ...
    {
        sj::range<int> sc = sj::algo::significant_cols<F> (f);
        return sc.span();
    }
    ...
    {
        if (sj::math::isnan(f) || sj::math::isinf(f)) { return f; }
        // How many sig figures does the type (float, double...) support?
        constexpr F epsilon = std::numeric_limits<F>::epsilon();
        constexpr int type_sf = static_cast<int>(sj::math::floor (sj::math::log10 (F{1} / epsilon)));
        ...
        F fcpy = f > F{0} ? f : -f;
        // Find the biggest column
        int maxcol = static_cast<int>(sj::math::floor (sj::math::log10 (fcpy)));
        // We will need to use 10^maxcol
        F tentothe = sj::math::pow (F{10}, maxcol);
        ...
        for (int curcol = maxcol; curcol >= mincol && curcol > (maxcol - type_sf); --curcol) {
            ...
            // Add to rounded
            rounded += sj::math::pow (F{10}, curcol) * colval;
            ...
            if (curcol == mincol) {
                ...
                F colval_peek = sj::math::floor (fcpy_peek / tentothe);
                if (colval_peek >= F{5}) { // need to round curcol up
                    rounded += sj::math::pow (F{10}, curcol) * F{1};
                }
            }
    ...
    {
        using mc = sj::mathconst<T>;
        I absm = m < I{0} ? -m : m;
        return std::sqrt ( mc::one_over_four_pi * (T{2} * static_cast<T>(l) + T{1}) * (sj::algo::factorial<T, I>(l - absm) / sj::algo::factorial<T, I>(l + absm)));
    }
    ...
    T real_spherical_harmonic (const UI l, const I m, const T nlm, const T phi, const T theta)
    {
        ...
        T ylm = T{0};
        if (m > I{0}) {
            ylm = mc::root_2 * nlm * std::cos (m * phi) * sj::algo::Plm<T, UI, I> (l, m, std::cos (theta));
        } else if (m < I{0}) {
            ylm = mc::root_2 * nlm * std::sin (-m * phi) * sj::algo::Plm<T, UI, I> (l, -m, std::cos (theta));
        } else { // m == 0
            ylm = nlm * sj::algo::Plm<T, UI, I> (l, I{0}, std::cos (theta));
        }
        return ylm;
    }
    ...
    {
        return sj::algo::real_spherical_harmonic<T, UI, I>(l, m, sj::algo::Nlm<T, UI, I>(l, m), phi, theta);
    }
#endif // __APPLE__
    ...
    template<typename T>
    rotation_sense orientation (const sj::vec<T, 2>& p,
                                const sj::vec<T, 2>& q,
                                const sj::vec<T, 2>& r)
    {
    ...
    template<typename T>
    static bool onsegment (const sj::vec<T, 2>& p,
                           const sj::vec<T, 2>& q,
                           const sj::vec<T, 2>& r)
    {
     ...
     */
    template<typename T>
    static std::bitset<2> segments_intersect (const sj::vec<T, 2>& p1, const sj::vec<T, 2> q1,
                                              const sj::vec<T, 2>& p2, const sj::vec<T, 2> q2)
    {
        ...
        std::bitset<2> rtn;
        sj::rotation_sense p1q1p2 = sj::algo::orientation (p1, q1, p2);
        sj::rotation_sense p1q1q2 = sj::algo::orientation (p1, q1, q2);
        sj::rotation_sense p2q2p1 = sj::algo::orientation (p2, q2, p1);
        sj::rotation_sense p2q2q1 = sj::algo::orientation (p2, q2, q1);
        if (p1q1p2 != p1q1q2 && p2q2p1 != p2q2q1) { // They intersect
            ...
            if constexpr (debug_this) {
                ...
                if (p1q1p2 == sj::rotation_sense::colinear) {
                    std::cout << "p1q1p2 rotn sense is colinear. On segment? "
                              << (sj::algo::onsegment (p1, p2, q1) ? "T" : "F") << std::endl;
                } else if (p1q1q2 == sj::rotation_sense::colinear) {
                    std::cout << "p1q1q2 rotn sense is colinear On segment? "
                              << (sj::algo::onsegment (p1, q2, q1) ? "T" : "F") << std::endl;
                } else if (p2q2p1 == sj::rotation_sense::colinear) {
                    std::cout << "p2q2p1 rotn sense is colinear On segment? "
                              << (sj::algo::onsegment (p2, p1, q2) ? "T" : "F") << std::endl;
                } else if (p2q2q1 == sj::rotation_sense::colinear) {
                    std::cout << "p2q2q1 rotn sense is colinear On segment? "
                              << (sj::algo::onsegment (p2, q1, q2) ? "T" : "F") << std::endl;
                } else {
                    std::cout << "NO rotn sense is colinear\n";
                }
            }
            if (p1q1p2 == sj::rotation_sense::colinear && sj::algo::onsegment (p1, p2, q1)) { rtn.set(1, true); }
            else if (p1q1q2 == sj::rotation_sense::colinear && sj::algo::onsegment (p1, q2, q1)) { rtn.set(1, true); }
            else if (p2q2p1 == sj::rotation_sense::colinear && sj::algo::onsegment (p2, p1, q2)) { rtn.set(1, true); }
            else if (p2q2q1 == sj::rotation_sense::colinear && sj::algo::onsegment (p2, q1, q2)) { rtn.set(1, true); }
        }
     ...
     */
    template<typename T>
    static sj::vec<T, 2> crossing_point (const sj::vec<T, 2>& p1, const sj::vec<T, 2>& q1,
                                            const sj::vec<T, 2>& p2, const sj::vec<T, 2>& q2)
    {
        sj::vec<T, 2> p = p1;
        sj::vec<T, 2> r = p1 - q1;
    ...
    static sj::vec<T, 2> meansos (const C<T, Al>& values)
    {
        sj::vec<T, 2> meansos = {T{0},T{0}};
    ...
    static T covariance (const C<T, Al>& x, const C<T, Al>& y)
    {
        ...
        sj::vec<T, 2> ms_x = algo::meansos<C, T, Al> (x);
        sj::vec<T, 2> ms_y = algo::meansos<C, T, Al> (y);
        T cov = T{0};
    ...
    //! = mx + c') in an vec<T, 2>
    template < template <typename, typename> typename C, typename T, typename Al=std::allocator<T> >
    static sj::vec<T, 2> linregr (const C<T, Al>& x, const C<T, Al>& y)
    {
        sj::vec<T, 2> ms_x = algo::meansos<C, T, Al> (x);
        sj::vec<T, 2> ms_y = algo::meansos<C, T, Al> (y);
