/*
 * Constexpr math in the sj::math namespace
 *
...
namespace sj::math
{
    ...
    namespace internal // log
    {
        ...
        {
            return (log_mantissa (mantissa(x)) + sj::mathconst<T>::ln_10 * T(find_exponent (x, 0)));
        }
    ...
    {
        template<typename T>
        constexpr return_t<T> log10_check (const T x) noexcept { return return_t<T>(log(x) / sj::mathconst<T>::ln_10); }
    }
    template<typename T>
    ...
    namespace internal // exp
    {
        ...
        {
            return (static_cast<T>(pow_integral (sj::mathconst<T>::e, find_whole(x))) * exp_cf (find_fraction(x)));
        }
    ...
    namespace internal // atan
    {
        ...
        {
            if (max_order == 1) { return sj::mathconst<T>::pi_over_2 - T{1} / x_pow; }
            ...
            res += sj::mathconst<T>::pi_over_2 - T{1} / x;
            return res;
        }
        template<typename T>
    ...
    namespace internal // atan2
    {
        ...
        constexpr T atan2_compute (const T y, const T x) noexcept
        {
            return (any_nan(y, x) ? std::numeric_limits<T>::quiet_NaN() :
                    std::numeric_limits<T>::min() > math::abs(x) ? std::numeric_limits<T>::min() > math::abs(y) ? neg_zero(y) ? neg_zero(x) ? -sj::mathconst<T>::pi :
                    - T{0} : neg_zero(x) ? sj::mathconst<T>::pi : T{0} :
                    y > T{0} ? sj::mathconst<T>::pi_over_2 : -sj::mathconst<T>::pi_over_2 :
                    x < T{0} ? y < T{0} ? atan(y/x) - sj::mathconst<T>::pi :
                    atan(y/x) + sj::mathconst<T>::pi :
                    atan(y/x));
        }
    ...
    namespace internal
    {
        ...
        {
            return (std::numeric_limits<T>::min() > math::abs (x - sj::mathconst<T>::pi_over_2) ? T{1.633124e+16} :
                    tan_series_exp_long (x - sj::mathconst<T>::pi_over_2));
        }
        template<typename T>
        ...
        {
            return (x > sj::mathconst<T>::pi ? count > 1 ? std::numeric_limits<T>::quiet_NaN() :
                    tan_begin (x - sj::mathconst<T>::pi * math::floor (x / sj::mathconst<T>::pi), count+1 ) :
                    tan_cf_main(x));
        }
        template<typename T>
    ...
    namespace internal // cos
    {
        ...
        constexpr T cos_check (const T x) noexcept
        {
            return (math::isnan(x) ? std::numeric_limits<T>::quiet_NaN() :
                    ...
                    std::numeric_limits<T>::min() > math::abs(x - sj::mathconst<T>::pi_over_2) ? T{0} : // special cases: pi/2 and pi
                    std::numeric_limits<T>::min() > math::abs(x + sj::mathconst<T>::pi_over_2) ? T{0} :
                    std::numeric_limits<T>::min() > math::abs(x - sj::mathconst<T>::pi) ? T{-1} :
                    std::numeric_limits<T>::min() > math::abs(x + sj::mathconst<T>::pi) ? T{-1} : cos_compute (math::tan (x/T{2})));
        }
    }
    template<typename T>
    ...
    namespace internal // sin
    {
        ...
        constexpr T sin_check (const T x) noexcept
        {
            return (math::isnan(x) ? std::numeric_limits<T>::quiet_NaN() :
                    std::numeric_limits<T>::min() > math::abs(x) ? T{0} :
                    std::numeric_limits<T>::min() > math::abs(x - sj::mathconst<T>::pi_over_2) ? T{1} :
                    std::numeric_limits<T>::min() > math::abs(x + sj::mathconst<T>::pi_over_2) ? T{-1} :
                    std::numeric_limits<T>::min() > math::abs(x - sj::mathconst<T>::pi) ? T{0} :
                    std::numeric_limits<T>::min() > math::abs(x + sj::mathconst<T>::pi) ? T{-0} : sin_compute (math::tan (x / T{2})));
        }
    }
    template<typename T>
    ...
    namespace internal // acos
    {
        ...
        constexpr T acos_compute(const T x) noexcept
        {
            return (math::abs(x) > T{1} ? std::numeric_limits<T>::quiet_NaN() :
                    std::numeric_limits<T>::min() > math::abs(x -  T{1}) ? T{0} :
                    std::numeric_limits<T>::min() > math::abs(x) ? sj::mathconst<T>::pi_over_2 : atan (math::sqrt(T{1} - x * x) / x));
        }
        template<typename T>
        constexpr T acos_check (const T x) noexcept
        {
            return (math::isnan(x) ? std::numeric_limits<T>::quiet_NaN() :
                    x > T{0} ? acos_compute(x) : sj::mathconst<T>::pi - acos_compute(-x));
        }
    }
    template<typename T>
    ...
    namespace internal // asin
    {
        ...
        constexpr T asin_compute (const T x) noexcept
        {
            return (x > T{1} ? std::numeric_limits<T>::quiet_NaN() :
                    std::numeric_limits<T>::min() > math::abs(x -  T{1}) ? sj::mathconst<T>::pi_over_2 :
