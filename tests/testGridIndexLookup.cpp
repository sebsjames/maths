...
int do_test (sj::griddomainwrap wrap, const sj::vec<float, 2>& coord_shift)
{
    ...
    sj::vec<float, 2> dx = { 0.5f, 0.5f };
    sj::vec<float, 2> offset = { -0.5f, 1.0f };
    ...
    for (int yii = 0; yii < g_bltr.get_h(); yii++) {
        ...
        for (int xii = 0; xii < g_bltr.get_w(); xii++) {
            ...
            int i_l = g_bltr.index_lookup(sj::vec<float, 2>{xi, yi});
            if (i_l != idx_expected++) {
                --rtn;
                std::cout << "bltr coord lookup fails for ("<<xi<<","<<yi<<")\n";
            }
            sj::vec<float, 2> c = g_bltr.coord_lookup(i_l);
            if ((c - (sj::vec<float, 2>{xi, yi} - coord_shift)).abs().sum() > std::numeric_limits<float>::epsilon()) {
                std::cout << "g_bltr.coord_lookup("<<i_l<<") returned " << c << " not " << (sj::vec<float, 2>{xi, yi} - coord_shift) << "\n";
                --rtn;
            }
        }
    }
    ...
    for (int xii = 0; xii < g_bltr.get_w(); xii++) {
        ...
        for (int yii = 0; yii < g_bltr.get_h(); yii++) {
            ...
            int i_l =g_bltrc.index_lookup(sj::vec<float, 2>{xi, yi});
            if (i_l != idx_expected++) {
                --rtn;
                std::cout << "bltrc coord lookup fails for ("<<xi<<","<<yi<<")\n";
            }
            sj::vec<float, 2> c = g_bltrc.coord_lookup(i_l);
            if ((c - (sj::vec<float, 2>{xi, yi} - coord_shift)).abs().sum() > std::numeric_limits<float>::epsilon()) {
                std::cout << "g_bltrc.coord_lookup("<<i_l<<") returned " << c << " not " << (sj::vec<float, 2>{xi, yi} - coord_shift) << "\n";
                --rtn;
            }
        }
    }
    ...
    for (int yii = 0; yii < g_bltr.get_h(); yii++) {
        ...
        for (int xii = 0; xii < g_bltr.get_w(); xii++) {
            ...
            int i_l = g_tlbr.index_lookup(sj::vec<float, 2>{xi, yi});
            if (i_l != idx_expected++) {
                --rtn;
                std::cout << "tlbr coord lookup fails for ("<<xi<<","<<yi<<")\n";
            }
            sj::vec<float, 2> c = g_tlbr.coord_lookup(i_l);
            if ((c - (sj::vec<float, 2>{xi, yi} - coord_shift)).abs().sum() > std::numeric_limits<float>::epsilon()) {
                std::cout << "g_tlbr.coord_lookup("<<i_l<<") returned " << c << " not " << (sj::vec<float, 2>{xi, yi} - coord_shift) << "\n";
                --rtn;
            }
        }
    }
    ...
    for (int xii = 0; xii < g_bltr.get_w(); xii++) {
        ...
        for (int yii = 0; yii < g_bltr.get_h(); yii++) {
            ...
            int i_l = g_tlbrc.index_lookup(sj::vec<float, 2>{xi, yi});
            if (i_l != idx_expected++) {
                --rtn;
                std::cout << "tlbrc coord lookup fails for ("<<xi<<","<<yi<<")\n";
            }
            sj::vec<float, 2> c = g_tlbrc.coord_lookup(i_l);
            if ((c - (sj::vec<float, 2>{xi, yi} - coord_shift)).abs().sum() > std::numeric_limits<float>::epsilon()) {
                std::cout << "g_tlbrc.coord_lookup("<<i_l<<") returned " << c << " not " << (sj::vec<float, 2>{xi, yi} - coord_shift) << "\n";
                --rtn;
            }
        }
    }
...
int main()
{
    ...
    try {
        for (float shift = 0.0f; shift < 0.25f; shift += 0.01f) {
            ...
            rtn += do_test (wrap, sj::vec<float, 2>{shift, 0.0f});
            rtn += do_test (wrap, sj::vec<float, 2>{-shift, 0.0f});
            rtn += do_test (wrap, sj::vec<float, 2>{0.0f, shift});
            rtn += do_test (wrap, sj::vec<float, 2>{0.0f, -shift});
        }
    ...
    try {
        do_test (wrap, sj::vec<float, 2>{shift, 0.0f});
        --rtn;
    } catch (const std::exception& e) {
    ...
    //
    // sj::griddomainwrap::horizontal tests should work exactly the same
    //
    ...
    try {
        do_test (wrap, sj::vec<float, 2>{-0.51f, 0.0f});
        --rtn;
    ...
    try {
        do_test (wrap, sj::vec<float, 2>{0.0f, shift});
        --rtn;
