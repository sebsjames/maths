...
int main()
{
    ...
    sj::vvec<int> numbers = { 1, 1, 2, 3, 4, 4, 4 };
    sj::histo<int, float> h(numbers, 3);

    std::cout << "For data: " << numbers << " arranged into three bins:\n\n";

    sj::range<int> _datarange = h.datarange; // Data range in terms of first histo template param type
    ...
    sj::vvec<float> _bins = h.bins;
    std::cout << "bin centres are: " << _bins << std::endl;

    sj::vvec<float> _binedges = h.binedges;
    std::cout << "bin edges are: " << _binedges << std::endl;

    sj::vvec<std::size_t> _counts = h.counts;
    std::cout << "Counts are: " << _counts << std::endl;

    std::size_t csum = (_counts - sj::vvec<std::size_t>{ 2, 1, 4 }).sum();
    if (csum != 0u) { --rtn; }

    sj::vvec<float> _proportions = h.proportions;
