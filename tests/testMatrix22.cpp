...
int main()
{
    ...
    sj::mat22<float> miinv = mi.invert();
    std::cout << "mi\n" << mi << std::endl;
    std::cout << "mi.invert():\n" << miinv << std::endl;

    // Test multiplication
    sj::mat22<float> mult1;
    setMatrixSequence (mult1);
    std::cout << "mult1\n" << mult1 << std::endl;

    sj::mat22<float> mult2;
    mult2.mat = { 5, 4, 3, 2 };
    std::cout << "mult2\n" << mult2 << std::endl;

    sj::mat22<float> mult3 = mult1 * mult2;
    std::cout << "mult1 * mult2 =\n" << mult3 << std::endl;

    sj::mat22<float> mult3alt = mult1 * mult2.mat;
    std::cout << "mult1 * mult2.mat =\n" << mult3alt << std::endl;

    sj::mat22<float> mult2_t = mult2;
    mult2_t.transpose();
    ...
    sj::mat22<float> mult1save = mult1;
    mult1 *= mult2;
    ...
    // vec rotation
    sj::vec<double, 2> v1 = { 0.0, 0.1 };
    sj::mat22<double> rotn;
    rotn.rotate (sj::mathconst<double>::pi_over_3);
    sj::vec<double, 2> v1_rot = rotn * v1;
    std::cout << "v1: " << v1 << ", rotated pi/3 is: "  << v1_rot << std::endl;

    rotn.rotate (sj::mathconst<double>::two_pi_over_3);
    v1_rot = rotn * v1;
