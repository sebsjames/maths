...
int main()
{
    ...
    sj::vvec<sj::vvec<FLT>> i_vertices = {
        { 0.7, 0.0 },
        { 0.0, 0.6 },
        { -0.6, -1.0 }
    };
    // Create an optimiser object
    sj::NM_Simplex<FLT> simp(i_vertices);
    // Define your Rosenbrock Banana objective function
    auto banana = [](const sj::vvec<FLT>& point) {
        FLT x = point[0];
        FLT y = point[1];
    ...
    if (simp.stopreason != sj::NM_Simplex_Stop_Reason::TerminationThreshold) {
        if (simp.stopreason == sj::NM_Simplex_Stop_Reason::TooManyOperations) {
    ...
    sj::vvec<FLT> thebest = simp.best_vertex();
