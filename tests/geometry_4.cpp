...
int main()
{
    ...
    float d = sm::geometry::dist_to_lineseg (t0, t1, p);
    std::cout << "Distance from " << p << " to " << t0 << "-" << t1 << " = " << d << std::endl;
    if (d != 0.1f) { std::cout << "Fail 0\n"; --rtn; }

    p = {0.5, 0.0001, 0.002};
    d = sm::geometry::dist_to_lineseg (t0, t1, p);
    ...
    d = sm::geometry::dist_to_tri_edge (t0, t1, t2, p);
    float d2 = sm::geometry::dist_to_tri_edge_sq (t0, t1, t2, p);
    std::cout << "d to " << p << " = " << d << " sq: " << d2 << std::endl;
    if (d != 0.00200249860063f || d2 != 4.01000033889e-6f) { std::cout << "Fail 2\n";--rtn; }

    p = {0, 0.9, 0};
    d = sm::geometry::dist_to_tri_edge (t0, t1, t2, p);
    d2 = sm::geometry::dist_to_tri_edge_sq (t0, t1, t2, p);
    std::cout << "d to " << p << " = " << d << " sq: " << d2 << std::endl;
