...
int main()
{
	...
	sm::vec<float, 2> mc_rnd = sm::algo::ransac<std::vector, float>(x, y, 300, 0.45f, 70);
	float m_est_rnd = mc_rnd[0];
	float c_est_rnd = mc_rnd[1];

	std::cout << "True parameters: m=" << m_true << " c=" << c_true << std::endl;
	std::cout << "Linear regression estimate: m=" << m_linregr << " c=" << c_linregr << std::endl;
	std::cout << "RANSAC estimate (fixed seed): m=" << m_est << " c=" << c_est << std::endl;
	std::cout << "RANSAC estimate (random seed): m=" << m_est_rnd << " c=" << c_est_rnd << std::endl;
	...
	if (std::abs(m_est_rnd - m_true) > m_tol) { --rtn; }
	if (std::abs(c_est_rnd - c_true) > c_tol) { --rtn; }
