...
int main()
{
    ...
    sj::vvec<float> b = { 2, 3, 2, 4 };
    sj::vvec<float> r1expct = { 14, 17, 14 };
    sj::vvec<float> r1 = a.convolve (b); // wrapdata::none, centre_kernel::yes, resize_output::no
    if (r1 != r1expct) { rtn -= 1; }
    ...
    r2 = aa.convolve<sj::vvec<float>::wrapdata::wrap, // wrapping complexified if kernel > data
                     sj::vvec<float>::centre_kernel::yes,
                     sj::vvec<float>::resize_output::no> (b);
    ...
    r2 = aa;
    r2.convolve_inplace<sj::vvec<float>::wrapdata::wrap,
                        sj::vvec<float>::centre_kernel::yes,
                        sj::vvec<float>::resize_output::no> (b);
