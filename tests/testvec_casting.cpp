...
void f_const_ptr_v (const sj::vec<float, 3>* v1) { std::cout << "You can stream a vec: " << (*v1) << std::endl; }
...
void f_nonconst_ptr_v (sj::vec<float, 3>* v1) { std::cout << "You can stream a vec: " << (*v1) << std::endl; }
...
void f_const_ref_v (const sj::vec<float, 3>& v1) { std::cout << "You can stream a vec: " << v1 << std::endl; }
...
void f_nonconst_ref_v (sj::vec<float, 3>& v1) { std::cout << "You can stream a vec: " << v1 << std::endl; }
...
int main()
{
    ...
    // error: invalid 'const_cast' from type 'sj::vec<float, 3>*' to type 'std::array<float, 3>*'
    ...
    f_const_ptr_v (static_cast<sj::vec<float, 3>*>(&a1));
    f_const_ptr_v (reinterpret_cast<sj::vec<float, 3>*>(&a1));
    // error: cannot 'dynamic_cast' '& a1' (of type 'struct std::array<float, 3>*') to type 'struct sj::vec<float, 3>*' (source type is not polysjic)
    //f_const_ptr_v (dynamic_cast<sj::vec<float, 3>*>(&a1));
    f_const_ptr_v (static_cast< const sj::vec<float, 3>* >(&a1));
    f_const_ptr_v (reinterpret_cast< const sj::vec<float, 3>* >(&a1));
    ...
    f_nonconst_ptr_v (&v1);
    // an array pointer can be cast to a vec
    f_nonconst_ptr_v (static_cast<sj::vec<float, 3>*>(&a1));

    // casting of a const reference sj::vec to std::array mostly works
    ...
    // error: no matching function for call to 'sj::vec<float, 3>::vec(std::array<float, 3>&)'
    // f_const_ref_v (static_cast< sj::vec<float, 3> >(a1));

    // error: no matching function for call to 'sj::vec<float, 3>::vec(std::array<float, 3>&)'
    // f_const_ref_v (static_cast< const sj::vec<float, 3> >(a1));

    // ok (note we're casting to const vec<>&
    f_const_ref_v (static_cast< const sj::vec<float, 3>& >(a1));
    f_const_ref_v (reinterpret_cast< const sj::vec<float, 3>& >(a1));
    f_const_ref_v (static_cast< sj::vec<float, 3>& >(a1));
    f_const_ref_v (reinterpret_cast< sj::vec<float, 3>& >(a1));

    // error: invalid 'const_cast' from type 'std::array<float, 3>*' to type 'const sj::vec<float, 3>*'
    // f_const_ref_v (const_cast< const sj::vec<float, 3>& >(a1));

    // error: cannot 'dynamic_cast' 'a1' (of type 'struct std::array<float, 3>') to type 'const struct sj::vec<float, 3>&' (source type is not polysjic)
    // f_const_ref_v (dynamic_cast< const sj::vec<float, 3>& >(a1));


    // reinterpret cast is also invalid and doesn't compile:
    // error: invalid cast from type 'std::array<float, 3>' to type 'sj::vec<float, 3>'
    //
    // f_const_ref_v (reinterpret_cast< sj::vec<float, 3> >(a1));
    // f_const_ref_v (reinterpret_cast< const sj::vec<float, 3> >(a1));

    std::cout << "\nCasting to non-const ref args\n";
    // casting of a non-const reference sj::vec to std::array is not ok
    f_nonconst_ref_a (a1);
    ...
    f_nonconst_ref_v (static_cast< sj::vec<float, 3>& >(a1));

    // error: cannot 'dynamic_cast' 'a1' (of type 'struct std::array<float, 3>') to type 'struct sj::vec<float, 3>&' (source type is not polysjic)
    // f_nonconst_ref_v (dynamic_cast< sj::vec<float, 3>& >(a1));
