...
int main()
{
    ...
    sj::quaternion<float> q;
    std::cout << q << std::endl;
    sj::quaternion<float> q_save = q;
    q.renormalize();
    std::cout << q << std::endl;

    if (q != q_save) { rtn++; }

    float angularSpeed = 0.2 * sj::mathconst<float>::deg2rad;
    sj::vec<float> rotationAxis = {1.0f, 0.0f, 0.0f};
    sj::quaternion<float> rotationquaternion(rotationAxis, angularSpeed);
    sj::quaternion<float> rq_expected (float{1}, angularSpeed/float{2}, float{0}, float{0});
    std::cout << "quaternion(" << rotationAxis<< ", " << angularSpeed<< ") constructor generates\n"
              << rotationquaternion
              << "\nvs expected:\n" << rq_expected
              << std::endl;
    sj::vec<float, 4> rqerr;
    ...
    sj::quaternion<float> p = q;
    if (p == q) {  } else { rtn++; }
    if (p != q) { rtn++; }

    sj::quaternion<float> qq1 (1.0f, -2.0f, 3.0f, -4.0f);
    std::cout << std::endl << qq1 << " conjugate (q*): " << qq1.conjugate() << std::endl << std::endl;
    sj::quaternion<float> qq1conj (1.0f, 2.0f, -3.0f, 4.0f);
    if (qq1.conjugate() != qq1conj) { ++rtn; }


    sj::quaternion<float> qq1i = qq1.inverse();
    std::cout << qq1 << " inverse (q^-1 or 1/q): " << qq1i << std::endl << std::endl;

    sj::quaternion<float> qq2 (1.0f, 0.0f, 0.0f, 0.0f);
    sj::quaternion<float> qq2i = qq2.inverse();
    std::cout << qq2 << " inverse (q^-1 or 1/q): " << qq2i << std::endl << std::endl;

    sj::quaternion<float> qiqi = qq1i * qq1;
    std::cout << "qq1i * qq1 = " << qiqi << std::endl << std::endl; // 1,0,0,0
    sj::quaternion<float> qident;
    if (qident == qiqi) {
        std::cout << "Good\n";
    } else {
        --rtn;
    ...
    sj::quaternion<float> q1(sj::vec<float>({1,0,0}), mc::pi_over_3);
    sj::quaternion<float> q2(sj::vec<float>({0,1,0}), mc::pi_over_4);
    sj::quaternion<float> q3 = q1 * q2;
    ...
    sj::quaternion<float> qcpc = q1.conjugate() * q2.conjugate();
    sj::quaternion<float> pqconj = (q2 * q1).conjugate();
    sj::quaternion<float> qpconj = (q1 * q2).conjugate();
    if (qcpc == pqconj) {
        std::cout << "Good\n";
    } else {
        --rtn;
    ...
    // Rotation matrices
    sj::quaternion<float> qfm (1, 2, -3, 4); // NOT unit.
    ...
    sj::vec<float, 16> vmatA;
    vmatA.set_from (matA);
    sj::vec<float, 16> vmatB;
    vmatB.set_from (matB);
    //sj::vec<float, 16> vmatC;
    //vmatC.set_from (matC);
    ...
    sj::vec<float, 16> vmatAA;
    vmatAA.set_from (matAA);
    sj::vec<float, 16> vmatBB;
    vmatBB.set_from (matBB);
    //sj::vec<float, 16> vmatCC;
    ...
    sj::vec<float> myaxis = { 1, 2, 3 };
    float myangle = 0.12f;
    myaxis.renormalize();
    sj::quaternion<float> qaa (myaxis, myangle);
    sj::vec<float, 4> aa = qaa.axis_angle();
    ...
    sj::quaternion<float> q_unitf (1.0f, 2.0f, -3.0f, 4.0f); // NOT unit.
    q_unitf.renormalize();
    sj::rand_uniform<float> rngf;
    sj::vec<float> vecf;
    float amountf;
    sj::range<float> metric_rangef (0.0f, 0.0f);
    for (unsigned int i = 0; i < 1000000; ++i) {
        rngf.get(vecf);
        vecf.renormalize();
        amountf = rngf.get();
        q_unitf.rotate (vecf, amountf);
        float metricf = sj::math::abs(float{1} - (q_unitf.w*q_unitf.w + q_unitf.x*q_unitf.x + q_unitf.y*q_unitf.y + q_unitf.z*q_unitf.z));
    ...
    if (metric_rangef.max > sj::quaternion<float>::unitThresh()) {
        --rtn;
    }

    sj::quaternion<double> q_unitd (1.0, 2.0, -3.0, 4.0); // NOT unit.
    q_unitd.renormalize();
    sj::rand_uniform<double> rngd;
    sj::vec<double> vecd;
    double amountd;
    sj::range<double> metric_ranged (0.0f, 0.0f);
    for (unsigned int i = 0; i < 1000000; ++i) {
        rngd.get(vecd);
        vecd.renormalize();
        amountd = rngd.get();
        q_unitd.rotate (vecd, amountd);
        double metricd = sj::math::abs(double{1} - (q_unitd.w*q_unitd.w + q_unitd.x*q_unitd.x + q_unitd.y*q_unitd.y + q_unitd.z*q_unitd.z));
        // how big is metric? Should be about 0.
        metric_ranged.update (metricd);
    }
    std::cout << "metric_range (double): " << metric_ranged << std::endl;
    // metric_range should be smaller than unitThresh
    if (metric_ranged.max > sj::quaternion<double>::unitThresh()) {
        --rtn;
    }
