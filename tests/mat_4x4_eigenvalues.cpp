...
int main()
{
    ...
    std::cout << "=== mat<F, 4> Eigenvalue Tests ===\n\n";
    ...
    // Test 2: Identity matrix
    {
        ...
        bool all_ones = true;
        for (size_t i = 0; i < 4; ++i) {
            if (std::abs(eigenvalues[i].real() - 1.0) > 1e-10 ||
                std::abs(eigenvalues[i].imag()) > 1e-10) {
                all_ones = false;
                break;
            }
        }
    ...
    // Test 5: Individual eigenvector method
    {
        ...
        if (normalized) {
            ...
            std::cout << "  Eigenvector normalization failed (norm^2 = " << norm_sq << ")\n";
            --rtn;
        }
    }
    ...
    // Test 9: 4x4 Rotation matrix (90deg rotation in xy-plane)
    ...
    {
        std::cout << "\nTest 9: Rotation matrix (90deg in xy-plane)\n";
        sm::mat<double, 4> R;
        R.set_identity();
        ...
        std::cout << "  Matrix: Rotation by 90deg in xy-plane\n";
    ...
    // Note: Verifying via trace/determinant properties
    {
        ...
        double s1 = std::sin (sm::mathconst<double>::pi_over_4);
        B[0] = c1;   B[4] = -s1;
        B[1] = s1;   B[5] = c1;

        // Second 2x2 block: 60deg rotation
        double c2 = std::cos (sm::mathconst<double>::pi_over_3);
        double s2 = std::sin (sm::mathconst<double>::pi_over_3);
        B[10] = c2;  B[14] = -s2;
        B[11] = s2;  B[15] = c2;

        std::cout << "  Matrix: Block diagonal with 45deg and 60deg rotations\n";
        std::cout << "  Expected: e^(i*45deg), e^(-i*45deg), e^(i*60deg), e^(-i*60deg) (analytically)\n";
