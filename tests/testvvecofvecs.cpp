...
int main()
{
    ...
    sj::vvec<sj::vec<float, 2>> vV2 = vV/2.0f;
    if (vV2[1][0] != 1.5f) { --rtn; }

    // Add/subtract vectors
    std::cout << "  (vvec of vecs) / + (1,-1) : " << vV + sj::vec<float, 2>({1,-1}) << std::endl;
    vV2 = vV + sj::vec<float, 2>({1,-1});
    if (vV2[1][0] != 4.0f) { --rtn; }
    ...
    sj::vvec<sj::vec<double, 3>> vvshrt = { {-0,-0,6.78819124e-05}, {-0,1.78819124e-05,1.78819124e-05}, {0,6.78819124e-05,0}, {0,2,0}, {7.34092391e-05,0,0}, {6.78819124e-05,0,0}, {-6.78819124e-05,-0,0} };
    ...
    sj::range<sj::vec<double, 3>> vextnts = vvshrt.extent();
    std::cout << "vextnts = " << vextnts << std::endl;

    if (vextnts.min == sj::vec<double, 3>{-6.78819124e-05, -0, 0}
        && vextnts.max == sj::vec<double, 3>{7.34092391e-05, 2, 6.78819124e-05}) {
        // Good
    } else {
        --rtn;
    }
    ...
    sj::range<float> vfr = sj::vvec<float>{1, 2, 3, 4}.extent();
    std::cout << "scalar range: " << vfr << std::endl;
    vfr = sj::vvec<float>{1, 2, -3, 4}.extent();
    std::cout << "scalar range: " << vfr << std::endl;

    // Test vector extent with an array of ints
    sj::vvec<std::array<int, 2>> vvai = {
        {-1, 1},
        {-3, 4},
        {-6, 2},
        {5,-4},
    ...
    // something odd
    try {
        sj::vvec<sj::vvec<float>> vvvvf2 = {{-1,1},{-2,5,3}};
        sj::vvec<float>  vfac  = { 1, 2, 3 };
    ...
    sj::vvec<sj::vec<int, 2>> vvfm = { {2, 3}, {4, 5} };
    sj::vec<int, 2> factor = {10, 100};
    sj::vvec<sj::vec<int, 2>> vvfm_result = (vvfm * factor);
    std::cout << vvfm << " * " << factor << " = " << vvfm_result << std::endl;
    if ((vvfm_result[0] == sj::vec<int, 2>{20, 300}
         && vvfm_result[1] == sj::vec<int, 2>{40, 500}) == false) {
        --rtn;
    }
