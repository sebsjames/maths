...
int main()
{
    ...
    sm::mat22<float> rot;
    rot.rotate(M_PI / 4.0f);  // 45 degree rotation
    sm::vec<std::complex<float>, 2> lambdas_rot = rot.eigenvalues();
    std::cout << "  Matrix: 45° rotation" << std::endl;
    ...
    // Test 5: Eigenpairs and verification (Av = λv)
    std::cout << "Test 5: Eigenpair verification (Av = λv)" << std::endl;
    ...
    std::cout << "  eigenvector(λ₀) verification error: " << error_B << std::endl;
    ...
    std::cout << "  Matrix: [[0,-1],[1,0]] (90° rotation)" << std::endl;
