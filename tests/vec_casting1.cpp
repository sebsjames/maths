...
void f_const_ptr_v (const sm::vec<float, 3>* v1) { std::cout << "You can stream a vec: " << (*v1) << std::endl; }
...
void f_nonconst_ptr_v (sm::vec<float, 3>* v1) { std::cout << "You can stream a vec: " << (*v1) << std::endl; }
...
void f_const_ref_v (const sm::vec<float, 3>& v1) { std::cout << "You can stream a vec: " << v1 << std::endl; }
...
void f_nonconst_ref_v (sm::vec<float, 3>& v1) { std::cout << "You can stream a vec: " << v1 << std::endl; }
...
int main()
{
    ...
    // error: invalid 'const_cast' from type 'sm::vec<float, 3>*' to type 'std::array<float, 3>*'
    ...
    // error: cannot 'dynamic_cast' '& a1' (of type 'struct std::array<float, 3>*') to type 'struct sm::vec<float, 3>*' (source type is not polysmic)
    //f_const_ptr_v (dynamic_cast<sm::vec<float, 3>*>(&a1));
    ...
    // casting of a const reference sm::vec to std::array mostly works
    ...
    // error: no matching function for call to 'sm::vec<float, 3>::vec(std::array<float, 3>&)'
    ...
    // error: invalid 'const_cast' from type 'std::array<float, 3>*' to type 'const sm::vec<float, 3>*'
    // f_const_ref_v (const_cast< const sm::vec<float, 3>& >(a1));

    // error: cannot 'dynamic_cast' 'a1' (of type 'struct std::array<float, 3>') to type 'const struct sm::vec<float, 3>&' (source type is not polysmic)
    // f_const_ref_v (dynamic_cast< const sm::vec<float, 3>& >(a1));


    // reinterpret cast is also invalid and doesn't compile:
    // error: invalid cast from type 'std::array<float, 3>' to type 'sm::vec<float, 3>'
    //
    // f_const_ref_v (reinterpret_cast< sm::vec<float, 3> >(a1));
    // f_const_ref_v (reinterpret_cast< const sm::vec<float, 3> >(a1));

    std::cout << "\nCasting to non-const ref args\n";
    // casting of a non-const reference sm::vec to std::array is not ok
    f_nonconst_ref_a (a1);
    ...
    // error: cannot 'dynamic_cast' 'a1' (of type 'struct std::array<float, 3>') to type 'struct sm::vec<float, 3>&' (source type is not polysmic)
    // f_nonconst_ref_v (dynamic_cast< sm::vec<float, 3>& >(a1));
