...
namespace morph {
    ...
    public:
        ...
        requires morph::is_copyable_container<Container>::value && morph::is_copyable_container<OContainer>::value
         ...
         */
        template <typename Container> requires morph::is_copyable_container<Container>::value
        void compute_scaling_from_data (const Container& data)
        {
            using Tc = typename Container::value_type;
            morph::range<Tc> mm = morph::range<Tc>::get_from (data);
