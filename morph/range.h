...
namespace morph {
    ...
    struct range
    {
        ...
        template <typename C> requires morph::is_copyable_container<C>::value
        static morph::range<typename C::value_type> get_from (const C& values)
        {
            using Tc = typename C::value_type;

            if constexpr (morph::number_type<T>::value == 0) { // vector elements
                ...
                morph::range<Tc> r (std::numeric_limits<Tc>::max(), std::numeric_limits<Tc>::min());
                T_el maxlen = 0;
            ...
            } else if constexpr (morph::number_type<T>::value == 2) { // complex elements
                ...
                morph::range<Tc> r ({std::numeric_limits<T_el>::max(), std::numeric_limits<T_el>::max() }, T{0, 0});
                for (auto v : values) {
            ...
            } else if constexpr (morph::number_type<T>::value == 1) { // scalar elements
                morph::range<Tc> r (std::numeric_limits<T>::max(), std::numeric_limits<T>::lowest());
                for (auto v : values) {
